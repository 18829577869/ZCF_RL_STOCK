...
STOCK_LIST = [
    ...
    {'code': 'sz.002475', 'name': '立讯精密', 'model': 'ppo_stock_v7_301389.zip', 'rank': 6, 'sharpe': 2.43, 'return': 68.03, 'drawdown': 10.64, 'strategy': '🟢 均衡型'},  # 使用隆扬电子模型，夏普2.43，收益率+68.03%，回撤10.64%
    ...
    # ===== 金融控股板块 =====
    {'code': 'sh.600120', 'name': '浙江东方', 'model': 'ppo_stock_v7_600120.zip', 'rank': 0, 'sharpe': 0.81, 'return': 7.90, 'drawdown': 4.23, 'strategy': '🔵 稳健型'},  # V11专用模型，夏普0.81，收益率+7.90%，回撤4.23%
]
...
CANDIDATE_MODELS = [  # 候选模型列表（包含所有股票的专用模型）
    ...
    {
        'name': '000425模型',
        'paths': ['ppo_stock_v7_000425.zip', 'models_v7_000425/best/best_model.zip'],
        'description': '徐工机械000425专用模型'
    },
    {
        'name': '301017模型',
...
# V18新增：初始化预测结果汇总列表（用于汇总第二天预测涨幅≥1%和≥2%的股票）
    ...
    for stock_info in STOCK_LIST:
        ...
        with OutputCapture() as output_capture:
            ...
            try:
                ...
                latest_time = None
                stock_name = get_stock_name(STOCK_CODE)
                # V18新增：在详细输出开始处添加HTML锚点（用于超链接跳转）
                anchor_id = STOCK_CODE.replace('.', '_').replace(':', '_')
                print(f'<a name="{anchor_id}"></a>')
                print(f"\n{'='*70}")
                ...
                if price_suggestions:
                    ...
                    if ENABLE_BACKTEST:
                        ...
                        if v17_suggestions:
                            ...
                            # ========== V18新增：收集预测结果到汇总列表 ==========
                            try:
                                # 从V17建议中提取第二天预测价格和涨跌幅
                                next_day_prediction = v17_suggestions.get('next_day_prediction')
                                ...
                                # 收集预测数据
                                if next_day_prediction is not None or next_day_change_pct is not None:
                                    summary_item = {
                                        'stock_code': STOCK_CODE,
                                        'stock_name': stock_name,
                                        'current_price': current_price,
                                        ...
                                        'ppo_action': ppo_action if 'ppo_action' in locals() else None,
                                        'ppo_operation': map_action_to_operation(ppo_action) if ('ppo_action' in locals() and ppo_action is not None) else '未知',
                                    }
...
# ========== V18新增：汇总第二天预测涨幅和跌幅≥1%和≥2%的股票 ==========
if all_predictions_summary:
    # 筛选涨幅和跌幅
    stocks_up_2pct = [s for s in all_predictions_summary if s.get('next_day_change_pct') is not None and s['next_day_change_pct'] >= 2.0]
    stocks_up_1pct = [s for s in all_predictions_summary if s.get('next_day_change_pct') is not None and s['next_day_change_pct'] >= 1.0 and s['next_day_change_pct'] < 2.0]
    stocks_down_1pct = [s for s in all_predictions_summary if s.get('next_day_change_pct') is not None and s['next_day_change_pct'] <= -1.0 and s['next_day_change_pct'] > -2.0]
    stocks_down_2pct = [s for s in all_predictions_summary if s.get('next_day_change_pct') is not None and s['next_day_change_pct'] <= -2.0]
    
    # 按涨幅/跌幅绝对值从高到低排序
    stocks_up_2pct.sort(key=lambda x: x['next_day_change_pct'], reverse=True)
    stocks_up_1pct.sort(key=lambda x: x['next_day_change_pct'], reverse=True)
    stocks_down_1pct.sort(key=lambda x: x['next_day_change_pct'])  # 跌幅从小到大（-1.5% < -1.0%）
    stocks_down_2pct.sort(key=lambda x: x['next_day_change_pct'])  # 跌幅从小到大（-3.0% < -2.0%）
    
    # 生成汇总信息
    summary_text = "\n" + "=" * 70 + "\n"
    summary_text += "📊 V18批量预测汇总：第二天预测涨跌幅汇总\n"
    summary_text += "=" * 70 + "\n\n"
    ...
    if stocks_up_2pct:
        ...
        for s in stocks_up_2pct:
            ...
            pred_display = f"{s['next_day_prediction']:.2f}" if s['next_day_prediction'] else "N/A"
            change_pct_display = f"{s['next_day_change_pct']:+.2f}%"
            ppo_op_display = s.get('ppo_operation', '未知') or '未知'
            summary_text += f"{stock_name_link:<30} | {stock_code_display:<15} | {current_price_display:>12} | {pred_display:>12} | {change_pct_display:>12} | {ppo_op_display:<12}\n"
        summary_text += "-" * 100 + "\n\n"
    else:
    ...
    if stocks_down_1pct:
        ...
        summary_text += "-" * 100 + "\n"
        summary_text += f"{'股票名称':<30} | {'股票代码':<15} | {'当前价格':<12} | {'预测价格':<12} | {'预测跌幅':<12} | {'PPO动作':<12}\n"
        summary_text += "-" * 100 + "\n"
