# get_stock_data_v11_300811.py - V11铂科新材300811专用版
# -*- coding: utf-8 -*-
"""
V11 铂科新材300811专用版：
1. 专门针对铂科新材300811进行数据获取和训练
2. 包含铂科新材300811及其相关股票，确保更好的针对性
3. 初始资金2万元（匹配实盘操作）
4. 数据用于V11全功能集成版训练
"""
...
# V11 铂科新材300811专用股票列表
# 包含铂科新材300811及其相关股票，确保更好的针对性
stocks = [
    # === 核心标的：铂科新材300811 ===
    {"code": "sz.300811", "name": "铂科新材", "start_date": "2019-12-27", 
     "category": "新材料", "volatility": "中", "style": "平衡", "priority": "核心"},
    
    # === 新材料/磁性材料板块（相关股票）===
    {"code": "sz.002056", "name": "横店东磁", "start_date": "2006-08-02", 
     "category": "新材料", "volatility": "中", "style": "平衡", "priority": "相关"},
    {"code": "sz.300655", "name": "晶瑞股份", "start_date": "2017-05-23", 
     "category": "新材料", "volatility": "中", "style": "平衡", "priority": "相关"},
    {"code": "sz.002384", "name": "东山精密", "start_date": "2010-04-09", 
     "category": "新材料", "volatility": "中", "style": "平衡", "priority": "相关"},
    {"code": "sz.300699", "name": "光威复材", "start_date": "2017-09-01", 
     "category": "新材料", "volatility": "中", "style": "平衡", "priority": "相关"},
    
    # === 电子材料/元器件板块（相关股票）===
    {"code": "sz.300661", "name": "圣邦股份", "start_date": "2017-06-06", 
     "category": "电子", "volatility": "中", "style": "平衡", "priority": "相关"},
    {"code": "sz.002916", "name": "深南电路", "start_date": "2017-12-13", 
     "category": "电子", "volatility": "中", "style": "平衡", "priority": "相关"},
...
print("="*70)
print("V11 铂科新材300811专用版 - 数据获取")
print("="*70)
print(f"总共 {len(stocks)} 只标的")
print(f"  核心标的: 铂科新材300811")
...
for stock in stocks:
    ...
    try:
        ...
        if len(train_data) < 100:
            ...
            fail_count += 1
            if code == "sz.300811":
                print(f"  [严重] 铂科新材300811训练数据不足！")
            continue
        
        # 保存到专用目录（V11可以使用V7的数据格式）
        os.makedirs('stockdata_v7_300811/train', exist_ok=True)
        os.makedirs('stockdata_v7_300811/test', exist_ok=True)
        
        train_file = f'stockdata_v7_300811/train/{code}.{name}.csv'
        test_file = f'stockdata_v7_300811/test/{code}.{name}.csv'
...
if success_count > 0:
    ...
    metadata_df.to_csv('stockdata_v7_300811/metadata_v7_300811.csv', index=False, encoding='utf-8-sig')
    print(f"\n元数据已保存: stockdata_v7_300811/metadata_v7_300811.csv")
    
    print("\n[完成] 可以开始训练：")
    print("  python train_v11_300811.py")
    print("\n[说明] 本版本专门针对铂科新材300811优化，初始资金2万元")
