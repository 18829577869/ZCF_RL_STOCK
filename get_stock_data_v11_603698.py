# get_stock_data_v11_603698.py - V11航天工程603698专用版
# -*- coding: utf-8 -*-
"""
V11 航天工程603698专用版：
1. 专门针对航天工程603698进行数据获取和训练
2. 包含航天工程603698及其相关股票，确保更好的针对性
3. 初始资金5万元（匹配实盘操作）
4. 数据用于V11全功能集成版训练
"""
...
# V11 航天工程603698专用股票列表
# 包含航天工程603698及其相关股票，确保更好的针对性
stocks = [
    # === 核心标的：航天工程603698 ===
    {"code": "sh.603698", "name": "航天工程", "start_date": "2015-01-27", 
     "category": "航天工程", "volatility": "中", "style": "平衡", "priority": "核心"},
    
    # === 航天工程板块（相关股票）===
    {"code": "sz.002025", "name": "航天电器", "start_date": "2004-07-26", 
     "category": "航天工程", "volatility": "中", "style": "平衡", "priority": "相关"},
    {"code": "sh.600343", "name": "航天动力", "start_date": "2003-04-08", 
     "category": "航天工程", "volatility": "中", "style": "平衡", "priority": "相关"},
    ...
    {"code": "sh.601012", "name": "隆基绿能", "start_date": "2012-04-11", 
     "category": "高端装备", "volatility": "高", "style": "激进", "priority": "配置"},
...
print("="*70)
print("V11 航天工程603698专用版 - 数据获取")
print("="*70)
print(f"总共 {len(stocks)} 只标的")
print(f"  核心标的: 航天工程603698")
...
success_count = 0
fail_count = 0
hangtian_gongcheng_success = False
...
for stock in stocks:
    ...
    try:
        ...
        if len(train_data) < 100:
            ...
            fail_count += 1
            if code == "sh.603698":
                print(f"  [严重] 航天工程603698训练数据不足！")
            continue
        
        # 保存到专用目录（V11可以使用V7的数据格式）
        os.makedirs('stockdata_v7_603698/train', exist_ok=True)
        os.makedirs('stockdata_v7_603698/test', exist_ok=True)
        
        train_file = f'stockdata_v7_603698/train/{code}.{name}.csv'
        test_file = f'stockdata_v7_603698/test/{code}.{name}.csv'
        ...
        if code == "sh.603698":
            hangtian_gongcheng_success = True
            print(f"  [核心] ✅ 航天工程603698数据获取成功！")
...
if hangtian_gongcheng_success:
    print(f"\n[核心] ✅ 航天工程603698数据获取成功！")
else:
    print(f"\n[严重] ❌ 航天工程603698数据获取失败！请检查！")
...
if success_count > 0:
    ...
    metadata_df.to_csv('stockdata_v7_603698/metadata_v7_603698.csv', index=False, encoding='utf-8-sig')
    print(f"\n元数据已保存: stockdata_v7_603698/metadata_v7_603698.csv")
    
    print("\n[完成] 可以开始训练：")
    print("  python train_v11_603698.py")
    print("\n[说明] 本版本专门针对航天工程603698优化，初始资金5万元")
