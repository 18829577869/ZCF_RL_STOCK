# get_stock_data_v11_002475.py - V11立讯精密002475专用版
# -*- coding: utf-8 -*-
"""
V11 立讯精密002475专用版：
1. 专门针对立讯精密002475进行数据获取和训练
2. 包含立讯精密002475及其相关股票，确保更好的针对性
3. 初始资金5万元（匹配实盘操作）
4. 数据用于V11全功能集成版训练
"""
...
# V11 立讯精密002475专用股票列表
# 包含立讯精密002475及其相关股票，确保更好的针对性
stocks = [
    # === 核心标的：立讯精密002475 ===
...
print("="*70)
print("V11 立讯精密002475专用版 - 数据获取")
print("="*70)
print(f"总共 {len(stocks)} 只标的")
print(f"  核心标的: 立讯精密002475")
...
for stock in stocks:
    ...
    try:
        ...
        if len(train_data) < 100:
            ...
            fail_count += 1
            if code == "sz.002475":
                print(f"  [严重] 立讯精密002475训练数据不足！")
            continue
        
        # 保存到专用目录（V11可以使用V7的数据格式）
        os.makedirs('stockdata_v7_002475/train', exist_ok=True)
        os.makedirs('stockdata_v7_002475/test', exist_ok=True)
        
        train_file = f'stockdata_v7_002475/train/{code}.{name}.csv'
        test_file = f'stockdata_v7_002475/test/{code}.{name}.csv'
...
if success_count > 0:
    ...
    metadata_df.to_csv('stockdata_v7_002475/metadata_v7_002475.csv', index=False, encoding='utf-8-sig')
    print(f"\n元数据已保存: stockdata_v7_002475/metadata_v7_002475.csv")
    
    print("\n[完成] 可以开始训练：")
    print("  python train_v11_002475.py")
    print("\n[说明] 本版本专门针对立讯精密002475优化，初始资金5万元")
