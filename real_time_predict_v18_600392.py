"""
-V18 实时预测系统 - 日K线均线信息版（盛和资源600392专用）
...
# 基础配置
MODEL_PATH = "ppo_stock_v7_300748.zip"  # 使用金力永磁专用PPO模型（盛和资源优选模型）
# 支持通过环境变量或命令行参数指定股票代码
STOCK_CODE = os.getenv('STOCK_CODE', 'sh.600392')  # 股票代码（默认：盛和资源600392）
...
try:
    ...
    STOCKAPI_AVAILABLE = False
CANDIDATE_MODELS = [  # 候选模型列表（按优先级排序）- 盛和资源600392优选金力永磁模型
    {
        'name': '300748模型（优选）',
        'paths': [
            'ppo_stock_v7_300748.zip',
            'models_v7_300748/best/best_model.zip'
        ],
        'description': '金力永磁300748模型（盛和资源600392优选模型，稀土永磁板块）'
    },
    {
        'name': '000831模型（相关）',
        'paths': [
            'ppo_stock_v7_000831.zip',
            'models_v7_000831/best/best_model.zip'
        ],
        'description': '中国稀土000831模型（稀土板块相关，备用）'
    },
    {
        'name': '通用模型',
        'paths': [
...
if ppo_model and PPO_AVAILABLE:
    try:
        ...
        if os.path.exists(test_data_dir):
            ...
            print(f"股票名称: {stock_name}")
            print(f"股票代码: {STOCK_CODE}")
            print(f"使用模型: 金力永磁300748模型 (ppo_stock_v7_300748.zip)")
            print(f"模型说明: 盛和资源600392优选使用金力永磁模型（稀土永磁板块）")
            print("-" * 70)
            print("🎯 核心回测指标:")
            print(f"   💡 提示: 回测数据需要运行对应的训练脚本生成")
            print(f"   📊 模型: 金力永磁300748专用模型")
            print(f"   🔗 相关: 中国稀土000831模型（备用）")
            print("-" * 70)
            print("💡 提示: 以上回测结果基于历史测试数据，仅供参考")
            print("=" * 70)
...
def get_daily_kline_df(stock_code, days=250):
    ...
    try:
        ...
        # 注意：300748是金力永磁模型代码，盛和资源优选使用此模型
        possible_csv_paths = [
            ...
            f'stockdata_v7_300748/test/{stock_code}.{stock_name}.csv',  # 300748金力永磁模型目录（优选）
            f'stockdata_v7_300748/train/{stock_code}.{stock_name}.csv',
            f'stockdata_v7_000831/test/{stock_code}.{stock_name}.csv',  # 000831中国稀土模型目录（备用）
            f'stockdata_v7_000831/train/{stock_code}.{stock_name}.csv',
