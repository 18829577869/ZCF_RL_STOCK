# train_v11_688208.py - V11道通科技688208专用训练
# -*- coding: utf-8 -*-
"""
V11 道通科技688208专用版特点：
1. 专门针对道通科技688208进行训练优化
2. 初始资金5万元（匹配实盘操作）
3. 优先使用道通科技688208进行训练和评估
4. 复用道通转债118013专用数据集中的全部标的，确保更好的泛化能力
...
# 说明：
# 道通科技的数据已经包含在 stockdata_v7_118013 中（作为正股），
# 因此这里直接复用该数据目录，无需重复获取。

# 扫描V7_118013训练数据（包含道通科技及相关标的）
...
def is_valid_data_file(file_path):
    ...
    try:
        ...
        # 但考虑到技术指标计算（MA20等），这里要求至少100条数据更安全
        min_required = 100
...
# 优先找到道通科技688208的文件（精确匹配，避免混淆）
daotong_stock_file = None
for f in stock_files:
    filename = os.path.basename(f)
    if 'sh.688208' in filename or '道通科技' in filename:
        daotong_stock_file = f
        break

print("="*70)
print("V11 道通科技688208专用版 - 训练启动")
print("="*70)
print(f"找到 {len(stock_files)} 只训练标的")
print(f"找到 {len(test_files)} 只测试标的")

if daotong_stock_file:
    print(f"✅ 核心标的: 道通科技688208 - {daotong_stock_file}")
else:
    print(f"⚠️  警告: 未找到道通科技688208的训练数据！")
...
if os.path.exists(metadata_file):
    ...
    # 检查道通科技688208
    daotong_stock_meta = metadata[metadata['code'] == 'sh.688208']
    if len(daotong_stock_meta) > 0:
        print(f"\n[核心] 道通科技688208元数据:")
        print(daotong_stock_meta[['name', 'category', 'volatility', 'style']].to_string(index=False))
else:
    print(f"\n[警告] 元数据文件不存在: {metadata_file}")

# V11_688208特殊配置：初始资金5万元（匹配实盘）
INITIAL_BALANCE_V11_688208 = 50000  # 5万初始资金，匹配实盘操作

def make_env():
    """随机选择标的创建环境，优先使用道通科技688208"""
    max_retries = 10  # 最多重试10次
    for _ in range(max_retries):
        try:
            # 30%概率使用道通科技688208，70%概率使用其他股票
            if daotong_stock_file and random.random() < 0.3:
                selected_file = daotong_stock_file
            else:
                selected_file = random.choice(stock_files)
            env = StockTradingEnv(selected_file, initial_balance=INITIAL_BALANCE_V11_688208)
            return env
    ...
    # 如果所有重试都失败，使用第一个有效文件
    if daotong_stock_file:
        return StockTradingEnv(daotong_stock_file, initial_balance=INITIAL_BALANCE_V11_688208)
    return StockTradingEnv(stock_files[0], initial_balance=INITIAL_BALANCE_V11_688208)
...
print("\n" + "="*70)
print("开始训练【V11 道通科技688208专用版】")
print("="*70)
print("核心特点：")
print("  [核心] 专门针对道通科技688208优化")
print("  [配置] 初始资金: 5万元（匹配实盘操作）")
print("  [配置] 复用道通转债118013专用数据集（包含正股+可转债+配置标的）")
print("  [策略] 训练时30%概率使用道通科技688208")
print("  [策略] 评估时优先使用道通科技688208")
...
# 回调
os.makedirs('./models_v7_688208/', exist_ok=True)
os.makedirs('./logs_v7_688208/eval/', exist_ok=True)

checkpoint_callback = CheckpointCallback(
    save_freq=100000 // 16,
    save_path='./models_v7_688208/',
    name_prefix='ppo_stock_v7_688208'
)

eval_callback = EvalCallback(
    eval_env,
    best_model_save_path='./models_v7_688208/best/',
    log_path='./logs_v7_688208/eval/',
    eval_freq=50000 // 16,
    deterministic=True,
    render=False
)

# PPO模型（针对道通科技688208优化，兼容V11）
model = PPO(
    "MlpPolicy",
    train_env,
    verbose=1,
    ...
    vf_coef=0.5,
    max_grad_norm=0.5,
    tensorboard_log="./logs_v7_688208/"
)

print("开始训练 2,500,000 步...")
print("💡 提示: 训练过程中会优先使用道通科技688208数据")
...
model.save("ppo_stock_v7_688208.zip")
print("\n[成功] 训练完成！模型已保存：ppo_stock_v7_688208.zip")
...
all_stats = []
category_stats = {}
daotong_stock_stats = None

# 优先测试道通科技688208
test_files_sorted = []
if daotong_stock_file:
    daotong_stock_test = None
    for test_file in test_files:
        if 'sh.688208' in test_file or '道通科技' in test_file:
            daotong_stock_test = test_file
            test_files_sorted.append(test_file)
            break

    for test_file in test_files:
        if test_file != daotong_stock_test:
            test_files_sorted.append(test_file)
else:
...
for test_file in test_files_sorted:
    ...
    try:
        env = StockTradingEnv(test_file, initial_balance=INITIAL_BALANCE_V11_688208)
        obs, _ = env.reset()
        done = False
        ...
        is_daotong_stock = ('sh.688208' in test_file or '道通科技' in test_file)
        if is_daotong_stock:
            stats['is_core'] = True
            daotong_stock_stats = stats
            print("="*70)
            print("🎯 [核心标的] 道通科技688208回测结果")
            print("="*70)
        else:
            stats['is_core'] = False
        ...
        core_mark = "🎯 [核心]" if is_daotong_stock else ""
...
if len(all_stats) > 0:
    ...
    if daotong_stock_stats:
        print("\n" + "="*70)
        print("🎯 [核心标的专项统计] 道通科技688208")
        print("="*70)
        print(f"最终净值: {daotong_stock_stats['final_net_worth']:,.2f} 元")
        print(f"总收益率: {daotong_stock_stats['total_return']:+.2f}%")
        print(f"最大回撤: {daotong_stock_stats['max_drawdown']:.2f}%")
        print(f"夏普比率: {daotong_stock_stats['sharpe_ratio']:.2f}")
        print(f"交易次数: {daotong_stock_stats['num_trades']}")
        print(f"胜率: {daotong_stock_stats['win_rate']:.2f}%")
        print(f"风险事件: {daotong_stock_stats['risk_events']} 次")
...
print("[完成] 所有测试完成！")
print("="*70)
print(f"[保存] 模型: ppo_stock_v7_688208.zip")
print(f"[日志] 训练日志: ./logs_v7_688208/")
print(f"[模型] 检查点: ./models_v7_688208/")
print(f"\n[提示] 使用 tensorboard --logdir=./logs_v7_688208/ 查看训练曲线")
print("\n[V11_688208特色] 专门针对道通科技688208优化，初始资金5万，匹配实盘操作！")
print("\n[使用] 训练完成后，可以使用以下命令进行实时预测：")
print("  python real_time_predict_v11_688208.py")
