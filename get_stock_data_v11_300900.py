# get_stock_data_v11_300900.py - V11广联航空300900专用版
# -*- coding: utf-8 -*-
"""
V11 广联航空300900专用版：
1. 专门针对广联航空300900进行数据获取和训练
2. 包含广联航空300900及其相关股票，确保更好的针对性
3. 初始资金2万元（匹配实盘操作）
4. 数据用于V11全功能集成版训练
"""
...
# V11 广联航空300900专用股票列表
# 包含广联航空300900及其相关股票，确保更好的针对性
stocks = [
    # === 核心标的：广联航空300900 ===
    {"code": "sz.300900", "name": "广联航空", "start_date": "2020-10-29", 
     "category": "航空航天", "volatility": "中", "style": "平衡", "priority": "核心"},
    
    # === 航空航天板块（相关股票）===
    {"code": "sh.600760", "name": "中航沈飞", "start_date": "1996-06-04", 
     "category": "航空航天", "volatility": "中", "style": "平衡", "priority": "相关"},
...
print("="*70)
print("V11 广联航空300900专用版 - 数据获取")
print("="*70)
print(f"总共 {len(stocks)} 只标的")
print(f"  核心标的: 广联航空300900")
...
success_count = 0
fail_count = 0
guanglian_hangkong_success = False
...
for stock in stocks:
    ...
    try:
        ...
        if len(train_data) < 100:
            ...
            fail_count += 1
            if code == "sz.300900":
                print(f"  [严重] 广联航空300900训练数据不足！")
            continue
        
        # 保存到专用目录（V11可以使用V7的数据格式）
        os.makedirs('stockdata_v7_300900/train', exist_ok=True)
        os.makedirs('stockdata_v7_300900/test', exist_ok=True)
        
        train_file = f'stockdata_v7_300900/train/{code}.{name}.csv'
        test_file = f'stockdata_v7_300900/test/{code}.{name}.csv'
        ...
        if code == "sz.300900":
            guanglian_hangkong_success = True
            print(f"  [核心] ✅ 广联航空300900数据获取成功！")
...
if guanglian_hangkong_success:
    print(f"\n[核心] ✅ 广联航空300900数据获取成功！")
else:
    print(f"\n[严重] ❌ 广联航空300900数据获取失败！请检查！")
...
if success_count > 0:
    ...
    metadata_df.to_csv('stockdata_v7_300900/metadata_v7_300900.csv', index=False, encoding='utf-8-sig')
    print(f"\n元数据已保存: stockdata_v7_300900/metadata_v7_300900.csv")
    
    print("\n[完成] 可以开始训练：")
    print("  python train_v11_300900.py")
    print("\n[说明] 本版本专门针对广联航空300900优化，初始资金2万元")
