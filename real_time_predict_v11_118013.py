"""
V11 实时预测系统 - 全功能集成版（道通转债118013专用）
整合 V7、V9、V10 的所有功能：
...
# 基础配置
MODEL_PATH = "ppo_stock_v7_118013.zip"  # V11使用专用PPO模型（道通转债118013）
STOCK_CODE = 'sh.118013'  # 股票代码（道通转债）
...
if PPO_AVAILABLE:
    try:
        if not os.path.exists(MODEL_PATH):
            possible_models = [
                "ppo_stock_v7_118013.zip",  # 118013专用模型（优先）
                "models_v7_118013/best/best_model.zip",
...
while True:
    try:
        ...
        # === 特殊处理：道通转债 118013 使用 akshare 日线，做「准实时」预测 ===
        if STOCK_CODE == 'sh.118013':
            try:
                ...
                print(f"   ⚠️  可转债日线数据获取失败: {e}")

        # 其他标的使用原有多数据源+5分钟K逻辑
        if df is None or len(df) == 0:
            if multi_source_manager and STOCK_CODE != 'sh.118013':
                try:
            ...
            if (df is None or len(df) == 0) and STOCK_CODE != 'sh.118013':
                try:
                    code_info = convert_stock_code(STOCK_CODE)
        ...
        try:
            ...
            if realtime_price and realtime_price > 0:
                print(f"   ✅ 已从实时行情接口获取价格: {realtime_price:.2f}  (获取时间: {request_ts.strftime('%Y-%m-%d %H:%M:%S')})")
            # 失败时不打印，避免频繁输出
        except Exception as e:
            # 静默处理，不打印错误
            pass
        ...
        if ENABLE_MULTI_MODEL_FUSION:
            ...
            stock_name = get_stock_name(STOCK_CODE)
            print(f"\n   ⭐ V11融合决策 [{stock_name}({STOCK_CODE})]: {final_operation} (置信度={confidence:.2f})")
