# get_stock_data_v11_300762.py - V11上海瀚讯300762专用版
# -*- coding: utf-8 -*-
"""
V11 上海瀚讯300762专用版：
1. 专门针对上海瀚讯300762进行数据获取和训练
2. 包含上海瀚讯300762及其相关股票，确保更好的针对性
3. 初始资金2万元（匹配实盘操作）
4. 数据用于V11全功能集成版训练
"""
...
# V11 上海瀚讯300762专用股票列表
# 包含上海瀚讯300762及其相关股票，确保更好的针对性
stocks = [
    # === 核心标的：上海瀚讯300762 ===
    {"code": "sz.300762", "name": "上海瀚讯", "start_date": "2019-04-04", 
     "category": "通信设备", "volatility": "中", "style": "平衡", "priority": "核心"},
    
    # === 通信设备板块（相关股票）===
    {"code": "sz.000063", "name": "中兴通讯", "start_date": "1997-11-18", 
     "category": "通信设备", "volatility": "高", "style": "激进", "priority": "相关"},
    {"code": "sz.002396", "name": "星网锐捷", "start_date": "2010-06-23", 
     "category": "通信设备", "volatility": "中", "style": "平衡", "priority": "相关"},
    {"code": "sz.300308", "name": "中际旭创", "start_date": "2012-04-10", 
     "category": "通信设备", "volatility": "中", "style": "平衡", "priority": "相关"},
    {"code": "sh.600050", "name": "中国联通", "start_date": "2002-10-09", 
     "category": "通信设备", "volatility": "中", "style": "平衡", "priority": "相关"},
    {"code": "sz.002281", "name": "光迅科技", "start_date": "2009-08-21", 
     "category": "通信设备", "volatility": "中", "style": "平衡", "priority": "相关"},
    
    # === 军工通信板块（相关股票）===
    {"code": "sz.002179", "name": "中航光电", "start_date": "2007-11-01", 
     "category": "军工通信", "volatility": "中", "style": "平衡", "priority": "相关"},
...
print("="*70)
print("V11 上海瀚讯300762专用版 - 数据获取")
print("="*70)
print(f"总共 {len(stocks)} 只标的")
print(f"  核心标的: 上海瀚讯300762")
...
for stock in stocks:
    ...
    try:
        ...
        if len(train_data) < 100:
            ...
            fail_count += 1
            if code == "sz.300762":
                print(f"  [严重] 上海瀚讯300762训练数据不足！")
            continue
        
        # 保存到专用目录（V11可以使用V7的数据格式）
        os.makedirs('stockdata_v7_300762/train', exist_ok=True)
        os.makedirs('stockdata_v7_300762/test', exist_ok=True)
        
        train_file = f'stockdata_v7_300762/train/{code}.{name}.csv'
        test_file = f'stockdata_v7_300762/test/{code}.{name}.csv'
        ...
        if code == "sz.300762":
            shanghai_hanxun_success = True
            print(f"  [核心] ✅ 上海瀚讯300762数据获取成功！")
...
if shanghai_hanxun_success:
    print(f"\n[核心] ✅ 上海瀚讯300762数据获取成功！")
else:
    print(f"\n[严重] ❌ 上海瀚讯300762数据获取失败！请检查！")
...
if success_count > 0:
    ...
    metadata_df.to_csv('stockdata_v7_300762/metadata_v7_300762.csv', index=False, encoding='utf-8-sig')
    print(f"\n元数据已保存: stockdata_v7_300762/metadata_v7_300762.csv")
    
    print("\n[完成] 可以开始训练：")
    print("  python train_v11_300762.py")
    print("\n[说明] 本版本专门针对上海瀚讯300762优化，初始资金2万元")
