# get_stock_data_v11_002851.py - V11麦格米特002851专用版
# -*- coding: utf-8 -*-
"""
V11 麦格米特002851专用版：
1. 专门针对麦格米特002851进行数据获取和训练
2. 包含麦格米特002851及其相关股票，确保更好的针对性
3. 初始资金2万元（匹配实盘操作）
4. 数据用于V11全功能集成版训练
"""
...
# V11 麦格米特002851专用股票列表
# 包含麦格米特002851及其相关股票，确保更好的针对性
stocks = [
    # === 核心标的：麦格米特002851 ===
    {"code": "sz.002851", "name": "麦格米特", "start_date": "2017-03-06", 
     "category": "工业自动化", "volatility": "中", "style": "平衡", "priority": "核心"},
    ...
    {"code": "sz.002594", "name": "比亚迪", "start_date": "2011-06-30", 
     "category": "新能源", "volatility": "中", "style": "平衡", "priority": "相关"},
    {"code": "sz.300014", "name": "亿纬锂能", "start_date": "2009-10-30", 
     "category": "新能源", "volatility": "中", "style": "平衡", "priority": "相关"},
    
    # === 工业控制/自动化板块（相关股票）===
    {"code": "sz.002179", "name": "中航光电", "start_date": "2007-11-01", 
     "category": "工业控制", "volatility": "中", "style": "平衡", "priority": "相关"},
    {"code": "sz.300726", "name": "宏达电子", "start_date": "2017-11-21", 
     "category": "电子元器件", "volatility": "中", "style": "平衡", "priority": "相关"},
...
print("="*70)
print("V11 麦格米特002851专用版 - 数据获取")
print("="*70)
print(f"总共 {len(stocks)} 只标的")
print(f"  核心标的: 麦格米特002851")
...
for stock in stocks:
    ...
    try:
        ...
        if len(train_data) < 100:
            ...
            fail_count += 1
            if code == "sz.002851":
                print(f"  [严重] 麦格米特002851训练数据不足！")
            continue
        
        # 保存到专用目录（V11可以使用V7的数据格式）
        os.makedirs('stockdata_v7_002851/train', exist_ok=True)
        os.makedirs('stockdata_v7_002851/test', exist_ok=True)
        
        train_file = f'stockdata_v7_002851/train/{code}.{name}.csv'
        test_file = f'stockdata_v7_002851/test/{code}.{name}.csv'
        ...
        if code == "sz.002851":
            maigemite_success = True
            print(f"  [核心] ✅ 麦格米特002851数据获取成功！")
...
if maigemite_success:
    print(f"\n[核心] ✅ 麦格米特002851数据获取成功！")
else:
    print(f"\n[严重] ❌ 麦格米特002851数据获取失败！请检查！")
...
if success_count > 0:
    ...
    metadata_df.to_csv('stockdata_v7_002851/metadata_v7_002851.csv', index=False, encoding='utf-8-sig')
    print(f"\n元数据已保存: stockdata_v7_002851/metadata_v7_002851.csv")
    
    print("\n[完成] 可以开始训练：")
    print("  python train_v11_002851.py")
    print("\n[说明] 本版本专门针对麦格米特002851优化，初始资金2万元")
