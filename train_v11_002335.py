# train_v11_002335.py - V11科华数据002335专用训练
# -*- coding: utf-8 -*-
"""
V11 科华数据002335专用版特点：
1. 专门针对科华数据002335进行训练优化
2. 初始资金5万元（匹配实盘/半实盘配置）
3. 优先使用科华数据002335进行训练和评估
...
# 扫描V7_002335训练数据（V11使用V7的数据格式）
train_dir = 'stockdata_v7_002335/train'
test_dir = 'stockdata_v7_002335/test'

if not os.path.exists(train_dir):
    print(f"[错误] 训练数据目录不存在: {train_dir}")
    print("请先运行: python get_stock_data_v11_002335.py")
    exit(1)
...
for f in stock_files:
    if '002335' in f or '科华数据' in f:
        kehua_file = f
        break

print("=" * 70)
print("V11 科华数据002335专用版 - 训练启动")
print("=" * 70)
print(f"找到 {len(stock_files)} 只训练标的")
print(f"找到 {len(test_files)} 只测试标的")

if kehua_file:
    print(f"✅ 核心标的: 科华数据002335 - {kehua_file}")
else:
    print(f"⚠️  警告: 未找到科华数据002335的训练数据！")
...
# 加载元数据
metadata_file = 'stockdata_v7_002335/metadata_v7_002335.csv'
if os.path.exists(metadata_file):
    ...
    # 检查科华数据002335
    kehua_meta = metadata[metadata['code'] == 'sz.002335']
    if len(kehua_meta) > 0:
        print(f"\n[核心] 科华数据002335元数据:")
        print(kehua_meta[['name', 'category', 'volatility', 'style']].to_string(index=False))
else:
    print(f"\n[警告] 元数据文件不存在: {metadata_file}")

# V11_002335特殊配置：初始资金5万元（匹配实盘）
INITIAL_BALANCE_V11_002335 = 50000  # 5万初始资金，匹配实盘操作

def make_env():
    """随机选择标的创建环境，优先使用科华数据002335"""
    max_retries = 10  # 最多重试10次
    for _ in range(max_retries):
        try:
            # 30%概率使用科华数据002335，70%概率使用其他股票
            if kehua_file and random.random() < 0.3:
                selected_file = kehua_file
            else:
                selected_file = random.choice(stock_files)
            env = StockTradingEnv(selected_file, initial_balance=INITIAL_BALANCE_V11_002335)
            return env
    ...
    # 如果所有重试都失败，使用第一个有效文件
    if kehua_file:
        return StockTradingEnv(kehua_file, initial_balance=INITIAL_BALANCE_V11_002335)
    return StockTradingEnv(stock_files[0], initial_balance=INITIAL_BALANCE_V11_002335)
...
print("\n" + "=" * 70)
print("开始训练【V11 科华数据002335专用版】")
print("=" * 70)
print("核心特点：")
print("  [核心] 专门针对科华数据002335优化")
print("  [配置] 初始资金: 5万元（匹配实盘/半实盘操作）")
print("  [配置] 包含科华数据及相关充电桩/电力设备/新能源/大盘标的")
print("  [策略] 训练时30%概率使用科华数据002335")
print("  [策略] 评估时优先使用科华数据002335")
...
# 回调
os.makedirs('./models_v7_002335/', exist_ok=True)
os.makedirs('./logs_v7_002335/eval/', exist_ok=True)

checkpoint_callback = CheckpointCallback(
    save_freq=100000 // 16,
    save_path='./models_v7_002335/',
    name_prefix='ppo_stock_v7_002335'
)

eval_callback = EvalCallback(
    eval_env,
    best_model_save_path='./models_v7_002335/best/',
    log_path='./logs_v7_002335/eval/',
    eval_freq=50000 // 16,
    deterministic=True,
    render=False
)

# PPO模型（针对科华数据002335优化，兼容V11）
model = PPO(
    "MlpPolicy",
    train_env,
    verbose=1,
    ...
    vf_coef=0.5,
    max_grad_norm=0.5,
    tensorboard_log="./logs_v7_002335/"
)

print("开始训练 2,500,000 步...")
print("💡 提示: 训练过程中会优先使用科华数据002335数据")
...
model.save("ppo_stock_v7_002335.zip")
print("\n[成功] 训练完成！模型已保存：ppo_stock_v7_002335.zip")
...
if kehua_file:
    ...
    for test_file in test_files:
        if '002335' in test_file or '科华数据' in test_file:
            kehua_test = test_file
            test_files_sorted.append(test_file)
            break
    
    # 其他文件
    for test_file in test_files:
        if test_file != kehua_test:
            test_files_sorted.append(test_file)
else:
...
for test_file in test_files_sorted:
    ...
    try:
        env = StockTradingEnv(test_file, initial_balance=INITIAL_BALANCE_V11_002335)
        obs, _ = env.reset()
        done = False
        ...
        # 检查是否是科华数据002335
        is_kehua = ('002335' in test_file or '科华数据' in test_file)
        if is_kehua:
            stats['is_core'] = True
            kehua_stats = stats
            print("=" * 70)
            print("🎯 [核心标的] 科华数据002335回测结果")
            print("=" * 70)
        else:
            stats['is_core'] = False
...
if len(all_stats) > 0:
    ...
    if kehua_stats:
        print("\n" + "=" * 70)
        print("🎯 [核心标的专项统计] 科华数据002335")
        print("=" * 70)
        print(f"最终净值: {kehua_stats['final_net_worth']:,.2f} 元")
        print(f"总收益率: {kehua_stats['total_return']:+.2f}%")
        print(f"最大回撤: {kehua_stats['max_drawdown']:.2f}%")
        print(f"夏普比率: {kehua_stats['sharpe_ratio']:.2f}")
        print(f"交易次数: {kehua_stats['num_trades']}")
        print(f"胜率: {kehua_stats['win_rate']:.2f}%")
        print(f"风险事件: {kehua_stats['risk_events']} 次")
...
print("[完成] 所有测试完成！")
print("=" * 70)
print(f"[保存] 模型: ppo_stock_v7_002335.zip")
print(f"[日志] 训练日志: ./logs_v7_002335/")
print(f"[模型] 检查点: ./models_v7_002335/")
print(f"\n[提示] 使用 tensorboard --logdir=./logs_v7_002335/ 查看训练曲线")
print("\n[V11_002335特色] 专门针对科华数据002335优化，初始资金5万，匹配实盘/半实盘操作！")
print("\n[使用] 训练完成后，可以使用以下命令进行实时预测：")
print("  python real_time_predict_v11_002335.py")
