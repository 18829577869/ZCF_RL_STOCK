# train_v11_603698.py - V11航天工程603698专用训练
# -*- coding: utf-8 -*-
"""
V11 航天工程603698专用版特点：
1. 专门针对航天工程603698进行训练优化
2. 初始资金5万元（匹配实盘操作）
3. 优先使用航天工程603698进行训练和评估
4. 包含相关股票确保更好的泛化能力
...
# 扫描V7_603698训练数据（V11使用V7的数据格式）
train_dir = 'stockdata_v7_603698/train'
...
# 优先找到航天工程603698的文件
hangtian_gongcheng_file = None
for f in stock_files:
    if '603698' in f or '航天工程' in f:
        hangtian_gongcheng_file = f
        break

print("="*70)
print("V11 航天工程603698专用版 - 训练启动")
print("="*70)
print(f"找到 {len(stock_files)} 只训练标的")
print(f"找到 {len(test_files)} 只测试标的")

if hangtian_gongcheng_file:
    print(f"✅ 核心标的: 航天工程603698 - {hangtian_gongcheng_file}")
else:
    print(f"⚠️  警告: 未找到航天工程603698的训练数据！")
...
# 加载元数据
metadata_file = 'stockdata_v7_603698/metadata_v7_603698.csv'
if os.path.exists(metadata_file):
    ...
    # 检查航天工程603698
    hangtian_gongcheng_meta = metadata[metadata['code'] == 'sh.603698']
    if len(hangtian_gongcheng_meta) > 0:
        print(f"\n[核心] 航天工程603698元数据:")
        print(hangtian_gongcheng_meta[['name', 'category', 'volatility', 'style']].to_string(index=False))
else:
    print(f"\n[警告] 元数据文件不存在: {metadata_file}")

# V11_603698特殊配置：初始资金5万元（匹配实盘）
INITIAL_BALANCE_V11_603698 = 50000  # 5万初始资金，匹配实盘操作

def make_env():
    """随机选择标的创建环境，优先使用航天工程603698"""
    # 30%概率使用航天工程603698，70%概率使用其他股票
    if hangtian_gongcheng_file and random.random() < 0.3:
        selected_file = hangtian_gongcheng_file
    else:
        selected_file = random.choice(stock_files)
    env = StockTradingEnv(selected_file, initial_balance=INITIAL_BALANCE_V11_603698)
    return env

def make_eval_env():
    """评估环境（优先使用航天工程603698）"""
    if hangtian_gongcheng_file:
        return StockTradingEnv(hangtian_gongcheng_file, initial_balance=INITIAL_BALANCE_V11_603698)
    else:
        return StockTradingEnv(stock_files[0], initial_balance=INITIAL_BALANCE_V11_603698)

print("\n" + "="*70)
print("开始训练【V11 航天工程603698专用版】")
print("="*70)
print("核心特点：")
print("  [核心] 专门针对航天工程603698优化")
print("  [配置] 初始资金: 5万元（匹配实盘操作）")
print("  [配置] 包含航天工程603698及相关股票")
print("  [策略] 训练时30%概率使用航天工程603698")
print("  [策略] 评估时优先使用航天工程603698")
...
# 回调
os.makedirs('./models_v7_603698/', exist_ok=True)
os.makedirs('./logs_v7_603698/eval/', exist_ok=True)

checkpoint_callback = CheckpointCallback(
    save_freq=100000 // 16,
    save_path='./models_v7_603698/',
    name_prefix='ppo_stock_v7_603698'
)

eval_callback = EvalCallback(
    eval_env,
    best_model_save_path='./models_v7_603698/best/',
    log_path='./logs_v7_603698/eval/',
    eval_freq=50000 // 16,
    deterministic=True,
    render=False
)

# PPO模型（针对航天工程603698优化，兼容V11）
model = PPO(
    "MlpPolicy",
    train_env,
    verbose=1,
    ...
    vf_coef=0.5,
    max_grad_norm=0.5,
    tensorboard_log="./logs_v7_603698/"
)

print("开始训练 2,500,000 步...")
print("💡 提示: 训练过程中会优先使用航天工程603698数据")
...
model.save("ppo_stock_v7_603698.zip")
print("\n[成功] 训练完成！模型已保存：ppo_stock_v7_603698.zip")
...
for test_file in test_files_sorted:
    ...
    try:
        env = StockTradingEnv(test_file, initial_balance=INITIAL_BALANCE_V11_603698)
        obs, _ = env.reset()
        done = False
...
print("[完成] 所有测试完成！")
print("="*70)
print(f"[保存] 模型: ppo_stock_v7_603698.zip")
print(f"[日志] 训练日志: ./logs_v7_603698/")
print(f"[模型] 检查点: ./models_v7_603698/")
print(f"\n[提示] 使用 tensorboard --logdir=./logs_v7_603698/ 查看训练曲线")
print("\n[V11_603698特色] 专门针对航天工程603698优化，初始资金5万，匹配实盘操作！")
print("\n[使用] 训练完成后，可以使用以下命令进行实时预测：")
print("  python real_time_predict_v11_603698.py")
print("  python real_time_predict_v16_603698.py")
