# get_stock_data_v11_000831.py - V11中国稀土000831专用版
# -*- coding: utf-8 -*-
"""
V11 中国稀土000831专用版：
1. 专门针对中国稀土000831进行数据获取和训练
2. 包含中国稀土000831及其相关股票，确保更好的针对性
3. 初始资金2万元（匹配实盘操作）
4. 数据用于V11全功能集成版训练
"""
...
# V11 中国稀土000831专用股票列表
# 包含中国稀土000831及其相关股票，确保更好的针对性
stocks = [
    # === 核心标的：中国稀土000831 ===
    {"code": "sz.000831", "name": "中国稀土", "start_date": "1998-09-11", 
     "category": "稀土", "volatility": "高", "style": "周期", "priority": "核心"},
...
print("="*70)
print("V11 中国稀土000831专用版 - 数据获取")
print("="*70)
print(f"总共 {len(stocks)} 只标的")
print(f"  核心标的: 中国稀土000831")
...
success_count = 0
fail_count = 0
china_rare_earth_success = False
...
for stock in stocks:
    ...
    try:
        ...
        if len(train_data) < 100:
            ...
            fail_count += 1
            if code == "sz.000831":
                print(f"  [严重] 中国稀土000831训练数据不足！")
            continue
        
        # 保存到专用目录（V11可以使用V7的数据格式）
        os.makedirs('stockdata_v7_000831/train', exist_ok=True)
        os.makedirs('stockdata_v7_000831/test', exist_ok=True)
        
        train_file = f'stockdata_v7_000831/train/{code}.{name}.csv'
        test_file = f'stockdata_v7_000831/test/{code}.{name}.csv'
        ...
        if code == "sz.000831":
            china_rare_earth_success = True
            print(f"  [核心] ✅ 中国稀土000831数据获取成功！")
...
if china_rare_earth_success:
    print(f"\n[核心] ✅ 中国稀土000831数据获取成功！")
else:
    print(f"\n[严重] ❌ 中国稀土000831数据获取失败！请检查！")
...
if success_count > 0:
    ...
    metadata_df.to_csv('stockdata_v7_000831/metadata_v7_000831.csv', index=False, encoding='utf-8-sig')
    print(f"\n元数据已保存: stockdata_v7_000831/metadata_v7_000831.csv")
    
    print("\n[完成] 可以开始训练：")
    print("  python train_v11_000831.py")
    print("\n[说明] 本版本专门针对中国稀土000831优化，初始资金2万元")
