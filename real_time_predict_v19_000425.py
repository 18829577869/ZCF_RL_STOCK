...
- 数据查找优先级优化：优先查找股票自己的目录和模型专用目录
- 第二天预测涨幅限制：限制第二天预测涨幅在±10%以内（A股涨跌停限制）

设计理念：多模型协同工作，智能融合决策，自动选择最优模型，风险控制优先，科学资金管理，丰富数据源
专用标的：徐工机械000425（工程机械板块）- 使用自身模型
"""
...
# 基础配置
MODEL_PATH = "ppo_stock_v7_000425.zip"  # 使用徐工机械专用PPO模型
# 支持通过环境变量或命令行参数指定股票代码
STOCK_CODE = os.getenv('STOCK_CODE', 'sz.000425')  # 股票代码（默认：徐工机械000425）
# 如果通过命令行参数指定，优先使用命令行参数
if len(sys.argv) > 1:
...
try:
    ...
    STOCKAPI_AVAILABLE = False
CANDIDATE_MODELS = [  # 候选模型列表（按优先级排序）- 徐工机械000425优选自身模型
    {
        'name': '000425模型（优选）',
        'paths': [
            'ppo_stock_v7_000425.zip',
            'models_v7_000425/best/best_model.zip'
        ],
        'description': '徐工机械000425模型（自身模型，工程机械板块）'
    },
    {
        'name': '通用模型',
        'paths': [
...
print("\n" + "=" * 70)
print("V18 实时预测系统 - 日K线均线信息版（良信股份002706专用）")
print("=" * 70)
print("整合功能:")
...
print("   V18: 日K线均线信息（5日、10日、15日、20日、30日均线）")
print("=" * 70)
print("⭐ V15新增功能:")
...
if ppo_model and PPO_AVAILABLE:
    try:
        ...
        if os.path.exists(test_data_dir):
            ...
            print(f"股票名称: {stock_name}")
            print(f"股票代码: {STOCK_CODE}")
            print(f"使用模型: 徐工机械000425模型 (ppo_stock_v7_000425.zip)")
            print(f"模型说明: 徐工机械000425使用自身模型（工程机械板块）")
            print("-" * 70)
            print("🎯 核心回测指标:")
            print(f"   💡 提示: 回测数据需要运行对应的训练脚本生成")
            print(f"   📊 模型: 徐工机械000425专用模型")
            print("-" * 70)
            print("💡 提示: 以上回测结果基于历史测试数据，仅供参考")
            print("=" * 70)
...
def calculate_rsrs_beta(df, window=18):
    ...
    Returns:
        float: RSRS斜率值（beta），如果计算失败返回None
    """
    ...
    try:
        ...
        # 使用numpy的polyfit进行线性回归（更简单高效）
        # 拟合 y = beta * x + alpha，其中x是最低价，y是最高价
...
def calculate_rsrs_std_score(beta_series, window=600, min_periods=20):
    ...
    Args:
        beta_series: RSRS斜率值的时间序列
        window: 观察周期，默认600个交易日
        min_periods: 最小观察期，默认20个交易日
    
    Returns:
        float: RSRS标准分值，如果计算失败返回None
    """
...
def calculate_rsrs_indicator(stock_code, daily_df=None, window=18, observe_window=600):
    ...
    Args:
        stock_code: 股票代码
        daily_df: 日K线DataFrame（如果已计算，避免重复获取）
        window: 计算斜率时的窗口大小，默认18日
        observe_window: 计算标准分时的观察周期，默认600个交易日
    ...
    Returns:
        dict: 包含RSRS指标信息的字典
        {
            'beta': RSRS斜率值,
            'std_score': RSRS标准分值,
            'buy_signal': bool,  # 标准分 > 0.7
            'sell_signal': bool,  # 标准分 < -0.7
            'data_available': bool
        }
    """
    result = {
    ...
    try:
        ...
        if 'high' not in daily_df.columns or 'low' not in daily_df.columns:
            # 如果没有high/low，尝试用close近似
            if 'close' in daily_df.columns:
                daily_df = daily_df.copy()  # 避免SettingWithCopyWarning
        ...
        # 计算RSRS标准分
        if len(beta_series) >= 20:  # 至少需要20个数据点
            std_score = calculate_rsrs_std_score(beta_series, window=observe_window, min_periods=20)
...
def get_30min_kline_data(stock_code, days=30):
    ...
    Args:
        stock_code: 股票代码，例如 "sz.002706"
        days: 需要获取的历史天数，默认30天
    
    Returns:
        DataFrame: 包含30分钟K线数据的DataFrame，如果获取失败返回None
    """
    try:
        import akshare as ak
        code_info = convert_stock_code(stock_code)
        ...
        try:
            ...
            if lg.error_code == '0':
                try:
                    end_date_bs = datetime.datetime.now().strftime('%Y-%m-%d')
                    start_date_bs = (datetime.datetime.now() - datetime.timedelta(days=days)).strftime('%Y-%m-%d')
                    
                    # baostock可能不支持30分钟K线，这里返回None
                    # 实际使用时可能需要其他数据源
                    pass
                finally:
                    with suppress_stdout():
...
def get_daily_kline_df(stock_code, days=250):
    """
    V18新增：获取日K线DataFrame（用于技术指标计算）
    优先从CSV历史数据文件读取，如果不存在或数据不足，则从在线数据源获取
    ...
    Returns:
        DataFrame: 包含日K线数据的DataFrame，列包含open, high, low, close, volume
    """
    df = None
    
    # 方法1：尝试从CSV历史数据文件读取（优先）
    try:
        # 提取股票代码和名称
        code_info = convert_stock_code(stock_code)
        stock_code_num = code_info.get('akshare', '')
        stock_name = get_stock_name(stock_code)
        
        # 尝试多个可能的CSV文件路径（按优先级排序）
        # 注意：000425是徐工机械模型代码，使用自身模型数据
        possible_csv_paths = [
            ...
            f'stockdata_v7_000425/test/{stock_code}.{stock_name}.csv',  # 000425徐工机械模型目录（优选）
            f'stockdata_v7_000425/train/{stock_code}.{stock_name}.csv',
                ...
                try:
                    ...
                    if all(col in df_csv.columns for col in required_cols):
                        ...
                        column_mapping = {
                            ...
                            '成交量': 'volume',
                            'volume': 'volume',
                            'amount': 'volume'  # 如果没有volume，用amount近似
                        }
                        for old_col, new_col in column_mapping.items():
                        ...
                        # 确保high/low/open列存在（如果没有则用close填充）
                        ...
                        if len(df_csv) > 0:
                            ...
                            if len(df_csv) >= 18:  # 至少需要18条数据用于RSRS计算
                                return df_csv
                except Exception:
                    continue
    except Exception:
    ...
    try:
        ...
        if df is not None and len(df) > 0:
            if '收盘' in df.columns:
                df = df.rename(columns={'收盘': 'close', '开盘': 'open',
                                       '最高': 'high', '最低': 'low', '成交量': 'volume', '日期': 'date'})
            ...
            for col in ['open', 'high', 'low', 'close', 'volume']:
                if col in df.columns:
                    df[col] = pd.to_numeric(df[col], errors='coerce')
            # 确保必需的列存在，如果缺少high/low/open，用close填充
            if 'high' not in df.columns:
                df['high'] = df['close']
            if 'low' not in df.columns:
                df['low'] = df['close']
...
def check_stock_conditions(stock_code, daily_df=None, min30_df=None, tech_indicators=None):
    ...
    Args:
        stock_code: 股票代码
        daily_df: 日K线DataFrame（如果已计算，避免重复获取）
        min30_df: 30分钟K线DataFrame（如果已计算）
        tech_indicators: TechnicalIndicators实例
    
    Returns:
        dict: 包含4种条件检测结果的字典
        {
            'condition1': bool,  # 日K线金叉+日MACD线金叉
            'condition2': bool,  # 30分钟K线金叉+30分钟MACD线金叉
            'condition3': bool,  # 日均线多头排列+日MACD线金叉
            'condition4': bool,  # 30分钟均线多头排列+30分钟MACD线金叉
        }
    """
    result = {
        'condition1': False,  # 日K线金叉+日MACD线金叉
        'condition2': False,  # 30分钟K线金叉+30分钟MACD线金叉
        'condition3': False,  # 日均线多头排列+日MACD线金叉
        'condition4': False,  # 30分钟均线多头排列+30分钟MACD线金叉
    }
...
def get_daily_kline_ma_info(stock_code, days=60):
    ...
    Args:
        stock_code: 股票代码，例如 "sz.002706"
        days: 需要获取的历史天数，默认60天（确保能计算30日均线）
    ...
    Returns:
        dict: 包含均线信息的字典，格式如下：
        {
            ...
            'data_source': 'akshare'/'baostock'/'tushare'/'unknown'
        }
    """
    result = {
        'ma5': None,
        ...
        try:
            ...
            if df is not None and len(df) > 0:
                ...
                if 'close' in df.columns:
                    ...
                    if len(closes) >= 30:  # 确保有足够数据计算30日均线
                        # 计算均线
                ...
                try:
                    ...
                    if ppo_action is not None:
                        ...
                        if v7_suggestions:
                            ...
                            current_pos_pct = v7_suggestions.get('current_position_pct', v7_suggestions.get('suggested_position_pct', 50.0))
                        ...
                        # 注意：V17的明确买卖价格建议会在LSTM和Transformer预测完成后统一显示
                except Exception as e:
                    print(f"   ⚠️  PPO预测失败: {e}")
        elif ppo_model:
            # V12兼容模式：单一模型
...
# V12优化：显示冲突检测和调整信息
        ...
        # ========== V18: 日K线均线信息（在文件结尾显示） ==========
        if daily_kline_info and daily_kline_info.get('data_available'):
            stock_name = get_stock_name(STOCK_CODE)
            print(f"\n   📊 V18日K线均线信息 [{stock_name}({STOCK_CODE})]:")
            print(f"      📅 数据日期: {daily_kline_info.get('latest_date', '未知')}")
            print(f"      📈 数据来源: {daily_kline_info.get('data_source', '未知')}")
            current_price_display = daily_kline_info.get('current_price')
            ...
            # 显示各均线信息
            ma_info_lines = []
            if daily_kline_info.get('ma5') is not None:
                ma5 = daily_kline_info['ma5']
                current_price = daily_kline_info.get('current_price', 0)
                if current_price > 0:
                    diff_pct = ((current_price - ma5) / ma5 * 100) if ma5 > 0 else 0
                    ma_info_lines.append(f"      📈 MA5 (5日均线): {ma5:.2f}元 (当前价格相对MA5: {diff_pct:+.2f}%)")
                else:
                    ma_info_lines.append(f"      📈 MA5 (5日均线): {ma5:.2f}元")
            
            if daily_kline_info.get('ma10') is not None:
                ma10 = daily_kline_info['ma10']
                current_price = daily_kline_info.get('current_price', 0)
                if current_price > 0:
                    diff_pct = ((current_price - ma10) / ma10 * 100) if ma10 > 0 else 0
                    ma_info_lines.append(f"      📈 MA10 (10日均线): {ma10:.2f}元 (当前价格相对MA10: {diff_pct:+.2f}%)")
                else:
                    ma_info_lines.append(f"      📈 MA10 (10日均线): {ma10:.2f}元")
            
            if daily_kline_info.get('ma15') is not None:
                ma15 = daily_kline_info['ma15']
                current_price = daily_kline_info.get('current_price', 0)
                if current_price > 0:
                    diff_pct = ((current_price - ma15) / ma15 * 100) if ma15 > 0 else 0
                    ma_info_lines.append(f"      📈 MA15 (15日均线): {ma15:.2f}元 (当前价格相对MA15: {diff_pct:+.2f}%)")
                else:
                    ma_info_lines.append(f"      📈 MA15 (15日均线): {ma15:.2f}元")
            
            if daily_kline_info.get('ma20') is not None:
                ma20 = daily_kline_info['ma20']
                current_price = daily_kline_info.get('current_price', 0)
                if current_price > 0:
                    diff_pct = ((current_price - ma20) / ma20 * 100) if ma20 > 0 else 0
                    ma_info_lines.append(f"      📈 MA20 (20日均线): {ma20:.2f}元 (当前价格相对MA20: {diff_pct:+.2f}%)")
                else:
                    ma_info_lines.append(f"      📈 MA20 (20日均线): {ma20:.2f}元")
            
            if daily_kline_info.get('ma30') is not None:
                ma30 = daily_kline_info['ma30']
                current_price = daily_kline_info.get('current_price', 0)
                if current_price > 0:
                    diff_pct = ((current_price - ma30) / ma30 * 100) if ma30 > 0 else 0
            ...
            # 均线排列分析
            if all([daily_kline_info.get('ma5'), daily_kline_info.get('ma10'), 
                   daily_kline_info.get('ma20'), daily_kline_info.get('ma30')]):
                ma5 = daily_kline_info['ma5']
                ma10 = daily_kline_info['ma10']
                ma20 = daily_kline_info['ma20']
                ma30 = daily_kline_info['ma30']
                ...
                if current_price > 0:
                    above_count = sum([
                        current_price > ma5 if ma5 else False,
                        current_price > ma10 if ma10 else False,
                        current_price > ma20 if ma20 else False,
                        current_price > ma30 if ma30 else False
                    ])
                    if above_count >= 3:
        ...
        # ========== V18: RSRS指标显示 ==========
        if rsrs_info and rsrs_info.get('data_available'):
            stock_name = get_stock_name(STOCK_CODE)
            print(f"\n   📈 V18 RSRS指标 [{stock_name}({STOCK_CODE})]:")
            
            beta = rsrs_info.get('beta')
            std_score = rsrs_info.get('std_score')
            buy_signal = rsrs_info.get('buy_signal', False)
            sell_signal = rsrs_info.get('sell_signal', False)
            ...
            if std_score is not None:
                ...
                if beta is not None:
                    print(f"      ⚠️  RSRS标准分: 数据不足，无法计算（需要至少600个交易日的历史数据）")
        else:
            stock_name = get_stock_name(STOCK_CODE)
