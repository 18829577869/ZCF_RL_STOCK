# train_v11_301413.py - V11安培龙301413专用训练
# -*- coding: utf-8 -*-
"""
V11 安培龙301413专用版特点：
1. 专门针对安培龙301413进行训练优化
2. 初始资金2万元（匹配实盘操作）
3. 优先使用安培龙301413进行训练和评估
4. 包含相关股票确保更好的泛化能力
...
# 扫描V7_301413训练数据（V11使用V7的数据格式）
train_dir = 'stockdata_v7_301413/train'
test_dir = 'stockdata_v7_301413/test'

if not os.path.exists(train_dir):
    print(f"[错误] 训练数据目录不存在: {train_dir}")
    print("请先运行: python get_stock_data_v11_301413.py")
    exit(1)
...
for f in stock_files:
    if '301413' in f or '安培龙' in f:
        anpeilong_file = f
        break

print("="*70)
print("V11 安培龙301413专用版 - 训练启动")
print("="*70)
print(f"找到 {len(stock_files)} 只训练标的")
print(f"找到 {len(test_files)} 只测试标的")

if anpeilong_file:
    print(f"✅ 核心标的: 安培龙301413 - {anpeilong_file}")
else:
    print(f"⚠️  警告: 未找到安培龙301413的训练数据！")
...
# 加载元数据
metadata_file = 'stockdata_v7_301413/metadata_v7_301413.csv'
if os.path.exists(metadata_file):
    ...
    # 检查安培龙301413
    anpeilong_meta = metadata[metadata['code'] == 'sz.301413']
    if len(anpeilong_meta) > 0:
        print(f"\n[核心] 安培龙301413元数据:")
        print(anpeilong_meta[['name', 'category', 'volatility', 'style']].to_string(index=False))
else:
    print(f"\n[警告] 元数据文件不存在: {metadata_file}")

# V11_301413特殊配置：初始资金2万元（匹配实盘）
INITIAL_BALANCE_V11_301413 = 20000  # 2万初始资金，匹配实盘操作

def make_env():
    """随机选择标的创建环境，优先使用安培龙301413"""
    # 30%概率使用安培龙301413，70%概率使用其他股票
    if anpeilong_file and random.random() < 0.3:
        selected_file = anpeilong_file
    else:
        selected_file = random.choice(stock_files)
    env = StockTradingEnv(selected_file, initial_balance=INITIAL_BALANCE_V11_301413)
    return env

def make_eval_env():
    """评估环境（优先使用安培龙301413）"""
    if anpeilong_file:
        return StockTradingEnv(anpeilong_file, initial_balance=INITIAL_BALANCE_V11_301413)
    else:
        return StockTradingEnv(stock_files[0], initial_balance=INITIAL_BALANCE_V11_301413)

print("\n" + "="*70)
print("开始训练【V11 安培龙301413专用版】")
print("="*70)
print("核心特点：")
print("  [核心] 专门针对安培龙301413优化")
print("  [配置] 初始资金: 2万元（匹配实盘操作）")
print("  [配置] 包含安培龙301413及相关股票")
print("  [策略] 训练时30%概率使用安培龙301413")
print("  [策略] 评估时优先使用安培龙301413")
...
# 回调
os.makedirs('./models_v7_301413/', exist_ok=True)
os.makedirs('./logs_v7_301413/eval/', exist_ok=True)

checkpoint_callback = CheckpointCallback(
    save_freq=100000 // 16,
    save_path='./models_v7_301413/',
    name_prefix='ppo_stock_v7_301413'
)

eval_callback = EvalCallback(
    eval_env,
    best_model_save_path='./models_v7_301413/best/',
    log_path='./logs_v7_301413/eval/',
    eval_freq=50000 // 16,
    deterministic=True,
    render=False
)

# PPO模型（针对安培龙301413优化，兼容V11）
model = PPO(
    "MlpPolicy",
    train_env,
    verbose=1,
    ...
    vf_coef=0.5,
    max_grad_norm=0.5,
    tensorboard_log="./logs_v7_301413/"
)

print("开始训练 2,500,000 步...")
print("💡 提示: 训练过程中会优先使用安培龙301413数据")
...
model.save("ppo_stock_v7_301413.zip")
print("\n[成功] 训练完成！模型已保存：ppo_stock_v7_301413.zip")
...
if anpeilong_file:
    ...
    for test_file in test_files:
        if '301413' in test_file or '安培龙' in test_file:
            anpeilong_test = test_file
            test_files_sorted.append(test_file)
            break
    
    # 其他文件
    for test_file in test_files:
        if test_file != anpeilong_test:
            test_files_sorted.append(test_file)
else:
...
for test_file in test_files_sorted:
    ...
    try:
        env = StockTradingEnv(test_file, initial_balance=INITIAL_BALANCE_V11_301413)
        obs, _ = env.reset()
        done = False
        ...
        # 检查是否是安培龙301413
        is_anpeilong = ('301413' in test_file or '安培龙' in test_file)
        if is_anpeilong:
            stats['is_core'] = True
            anpeilong_stats = stats
            print("="*70)
            print("🎯 [核心标的] 安培龙301413回测结果")
            print("="*70)
        else:
            stats['is_core'] = False
        ...
        core_mark = "🎯 [核心]" if is_anpeilong else ""
...
if len(all_stats) > 0:
    ...
    if anpeilong_stats:
        print("\n" + "="*70)
        print("🎯 [核心标的专项统计] 安培龙301413")
        print("="*70)
        print(f"最终净值: {anpeilong_stats['final_net_worth']:,.2f} 元")
        print(f"总收益率: {anpeilong_stats['total_return']:+.2f}%")
        print(f"最大回撤: {anpeilong_stats['max_drawdown']:.2f}%")
        print(f"夏普比率: {anpeilong_stats['sharpe_ratio']:.2f}")
        print(f"交易次数: {anpeilong_stats['num_trades']}")
        print(f"胜率: {anpeilong_stats['win_rate']:.2f}%")
        print(f"风险事件: {anpeilong_stats['risk_events']} 次")
...
print("[完成] 所有测试完成！")
print("="*70)
print(f"[保存] 模型: ppo_stock_v7_301413.zip")
print(f"[日志] 训练日志: ./logs_v7_301413/")
print(f"[模型] 检查点: ./models_v7_301413/")
print(f"\n[提示] 使用 tensorboard --logdir=./logs_v7_301413/ 查看训练曲线")
print("\n[V11_301413特色] 专门针对安培龙301413优化，初始资金2万，匹配实盘操作！")
print("\n[使用] 训练完成后，可以使用以下命令进行实时预测：")
print("  python real_time_predict_v11_301413.py")
