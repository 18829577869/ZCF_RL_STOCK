# get_stock_data_v11_000547.py - V11航天发展000547专用版
# -*- coding: utf-8 -*-
"""
V11 航天发展000547专用版：
1. 专门针对航天发展000547进行数据获取和训练
2. 包含航天发展000547及其相关股票，确保更好的针对性
3. 初始资金5万元（匹配实盘操作）
4. 数据用于V11全功能集成版训练
"""
...
# V11 航天发展000547专用股票列表
# 包含航天发展000547及其相关股票，确保更好的针对性
stocks = [
    # === 核心标的：航天发展000547 ===
...
print("="*70)
print("V11 航天发展000547专用版 - 数据获取")
print("="*70)
print(f"总共 {len(stocks)} 只标的")
print(f"  核心标的: 航天发展000547")
...
for stock in stocks:
    ...
    try:
        ...
        if len(train_data) < 100:
            ...
            fail_count += 1
            if code == "sz.000547":
                print(f"  [严重] 航天发展000547训练数据不足！")
            continue
        
        # 保存到专用目录（V11可以使用V7的数据格式）
        os.makedirs('stockdata_v7_000547/train', exist_ok=True)
        os.makedirs('stockdata_v7_000547/test', exist_ok=True)
        
        train_file = f'stockdata_v7_000547/train/{code}.{name}.csv'
        test_file = f'stockdata_v7_000547/test/{code}.{name}.csv'
        ...
        if code == "sz.000547":
            hangtian_fazhan_success = True
            print(f"  [核心] ✅ 航天发展000547数据获取成功！")
...
if hangtian_fazhan_success:
    print(f"\n[核心] ✅ 航天发展000547数据获取成功！")
else:
    print(f"\n[严重] ❌ 航天发展000547数据获取失败！请检查！")
...
if success_count > 0:
    ...
    metadata_df.to_csv('stockdata_v7_000547/metadata_v7_000547.csv', index=False, encoding='utf-8-sig')
    print(f"\n元数据已保存: stockdata_v7_000547/metadata_v7_000547.csv")
    
    print("\n[完成] 可以开始训练：")
    print("  python train_v11_000547.py")
    print("\n[说明] 本版本专门针对航天发展000547优化，初始资金5万元")
