...
"""
V12 实时预测系统 - Transformer优化版（航天电器002025专用 - 使用300762模型）
...
模型来源：使用上海瀚讯300762的V7模型（v7_300762）
"""
...
print("\n" + "=" * 70)
print("V12 实时预测系统 - Transformer优化版（航天电器002025专用版 - 使用300762模型）")
print("=" * 70)
print("📌 整合功能:")
...
if PPO_AVAILABLE:
    try:
        ...
        # 本文件使用上海瀚讯300762的模型（v7_300762）来运行航天电器002025
        possible_models = [
            "ppo_stock_v7_300762.zip",  # 300762专用模型（优先，用于002025）
            "models_v7_300762/best/best_model.zip",  # 300762最佳模型（优先，用于002025）
        ...
        if loaded_model_path:
            ...
            if "300762" in MODEL_PATH:
                print(f"✅ PPO模型加载成功: {MODEL_PATH} (使用上海瀚讯300762模型运行航天电器002025)")
            else:
        ...
        if price_suggestions:
            ...
            if suggested_buy_price and final_action >= 4:  # 买入操作
                ...
                if buy_shares > 0:
                    ...
                    if total_cost > current_balance:
                        # 资金不足时，按资金上限重新取整到100股
                        adjust_shares = round_to_lot(max(0, current_balance - MIN_COMMISSION) / adjusted_buy_price) if adjusted_buy_price > 0 else 0
                        if adjust_shares < buy_shares:
