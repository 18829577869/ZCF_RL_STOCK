# get_stock_data_v11_600438.py - V11通威股份600438专用版
# -*- coding: utf-8 -*-
"""
V11 通威股份600438专用版：
1. 专门针对通威股份600438进行数据获取和训练
2. 包含通威股份600438及其相关股票，确保更好的针对性
3. 初始资金2万元（匹配实盘操作）
4. 数据用于V11全功能集成版训练
"""
...
# V11 通威股份600438专用股票列表
# 包含通威股份600438及其相关股票，确保更好的针对性
stocks = [
    # === 核心标的：通威股份600438 ===
    {"code": "sh.600438", "name": "通威股份", "start_date": "2004-03-02", 
...
print("="*70)
print("V11 通威股份600438专用版 - 数据获取")
print("="*70)
print(f"总共 {len(stocks)} 只标的")
print(f"  核心标的: 通威股份600438")
...
for stock in stocks:
    ...
    try:
        ...
        if len(train_data) < 100:
            ...
            fail_count += 1
            if code == "sh.600438":
                print(f"  [严重] 通威股份600438训练数据不足！")
            continue
        
        # 保存到专用目录（V11可以使用V7的数据格式）
        os.makedirs('stockdata_v7_600438/train', exist_ok=True)
        os.makedirs('stockdata_v7_600438/test', exist_ok=True)
        
        train_file = f'stockdata_v7_600438/train/{code}.{name}.csv'
        test_file = f'stockdata_v7_600438/test/{code}.{name}.csv'
        ...
        if code == "sh.600438":
            tongwei_success = True
            print(f"  [核心] ✅ 通威股份600438数据获取成功！")
...
if success_count > 0:
    ...
    metadata_df.to_csv('stockdata_v7_600438/metadata_v7_600438.csv', index=False, encoding='utf-8-sig')
    print(f"\n元数据已保存: stockdata_v7_600438/metadata_v7_600438.csv")
    
    print("\n[完成] 可以开始训练：")
    print("  python train_v11_600438.py")
    print("\n[说明] 本版本专门针对通威股份600438优化，初始资金2万元")
