"""
V12 实时预测系统 - Transformer优化版（鸿远电子603267专用 - 使用002025模型）
整合 V7、V9、V10、V11 的所有功能：
...
设计理念：多模型协同工作，智能融合决策
专用标的：鸿远电子603267（电子元器件板块）
模型来源：使用航天电器002025的V7模型（v7_002025）
"""
...
# 基础配置
MODEL_PATH = "ppo_stock_v7.zip"  # V12使用通用PPO模型，也可以使用专用模型
STOCK_CODE = 'sh.603267'  # 股票代码（鸿远电子）
...
print("\n" + "=" * 70)
print("V12 实时预测系统 - Transformer优化版（鸿远电子603267专用版 - 使用002025模型）")
print("=" * 70)
print("📌 整合功能:")
...
print("=" * 70)
print("⭐ V12新增优化:")
print("   - Transformer预测价格优化（解决预测偏低问题）")
print("   - 增加训练轮数到120轮，提高模型准确性")
print("   - 智能归一化策略（考虑当前价格位置）")
print("   - 趋势感知机制（基于价格动量调整）")
print("   - 预测后处理优化（动态校正预测）")
print("=" * 70)
print("⚠️  版本标识: 这是 V12 版本，优化Transformer预测！")
print("=" * 70 + "\n")
...
if PPO_AVAILABLE:
    try:
        ...
        # 本文件使用航天电器002025的模型（v7_002025）来运行鸿远电子603267
        possible_models = [
            "ppo_stock_v7_002025.zip",  # 002025专用模型（优先，用于603267）
            "models_v7_002025/best/best_model.zip",  # 002025最佳模型（优先，用于603267）
        ...
        if loaded_model_path:
            ...
            if "002025" in MODEL_PATH:
                print(f"✅ PPO模型加载成功: {MODEL_PATH} (使用航天电器002025模型运行鸿远电子603267)")
            else:
                print(f"✅ PPO模型加载成功: {MODEL_PATH} (通用模型)")
        else:
            raise FileNotFoundError("未找到任何可用的PPO模型文件")
    except Exception as e:
    ...
    try:
        ...
        if price_suggestions:
            ...
            if conflict_info and conflict_info.get('has_conflict', False):
                ...
                if final_action == 3 or "调整为持有" in adjustment_reason:  # 已调整为持有
                ...
                elif final_action == 2 or "调整为卖出" in adjustment_reason:  # 已调整为卖出25%
                    action_hint = f"⚠️  由于预测价格明显下跌，已调整为「卖出25%」。建议减仓至25%仓位或更低"
                    consistency_note = f"⚠️  已根据预测方向调整决策（预测{price_suggestions['direction']} {abs(price_suggestions['price_change_pct']):.2f}%）"
                elif final_action == 5 or ("降低买入力度" in adjustment_reason and "买入 50%" in adjustment_reason):  # 已降低买入力度至买入50%
                    action_hint = f"⚠️  由于预测方向与PPO建议冲突，已降低买入力度至「买入50%」。建议买入至50%仓位，而非满仓"
                    consistency_note = f"⚠️  已根据预测方向调整决策（预测{price_suggestions['direction']} {abs(price_suggestions['price_change_pct']):.2f}%）"
                elif "降低买入力度" in adjustment_reason and "买入 25%" in adjustment_reason:  # 已降低买入力度至买入25%
            ...
            elif final_action == 5:  # 买入 50%
                if current_price <= suggestions['50%']:
                    action_hint = f"✅ 融合决策「买入 50%」+ 当前价格 {current_price:.2f}元 在买入区间，建议买入至50%仓位"
                else:
                    action_hint = f"✅ 融合决策「买入 50%」：当前价格 {current_price:.2f}元，建议买入至50%仓位（可等待回调至 {suggestions['50%']:.2f}元 以下）"
                consistency_note = "✅ 与融合决策「买入 50%」一致"
            
            elif final_action == 4:  # 买入 25%
                if current_price <= suggestions['75%']:
            ...
            if final_action == 6:  # 买入100%（目标100%仓位）
                ...
                target_position = "100%"
            elif final_action == 5:  # 买入50%（目标50%仓位）
                suggested_buy_price = suggestions['50%']
                target_position = "50%"
            elif final_action == 4:  # 买入25%（目标75%仓位）
                suggested_buy_price = suggestions['75%']
                target_position = "75%"
            elif final_action == 3:  # 持有（目标50%仓位）
                # 持有操作，显示当前价格对应的合理仓位价格
                target_position = "50%"
                # 不显示买入/卖出价格，因为建议持有
            elif final_action == 2:  # 卖出25%（目标75%仓位，即保留75%）
                suggested_sell_price = suggestions['25%']  # 卖出到25%仓位对应的价格
                target_position = "75%"
            elif final_action == 1:  # 卖出50%（目标50%仓位）
            ...
            # 持有操作显示当前价格对应的合理仓位
            if final_action == 3 and target_position:
            ...
            if suggested_buy_price and final_action >= 4:  # 买入操作
                ...
                if buy_pct > 0:
                    # 使用calc_buy_trade函数计算精确的买入数量和成本（考虑滑点和手续费）
                    shares_bought, total_cost, total_fee, adjusted_price = calc_buy_trade(
                        suggested_buy_price, buy_pct, current_balance
                    )
                    
                    if shares_bought > 0 and total_cost > 0:
                        actual_buy_amount = shares_bought * adjusted_price
                        commission = max(MIN_COMMISSION, actual_buy_amount * COMMISSION_RATE)
                        transfer_fee = actual_buy_amount * TRANSFER_FEE_RATE
                        
                        print(f"      🟢 建议买入:")
                        print(f"         💰 建议价格: {suggested_buy_price:.2f}元（实际成交价考虑滑点: {adjusted_price:.2f}元）")
                        print(f"         📊 建议数量: {int(shares_bought)}股（约{shares_bought:.2f}股）")
                        print(f"         💵 预计金额: {actual_buy_amount:.2f}元")
                        print(f"         💸 手续费: {total_fee:.2f}元（佣金{commission:.2f}元 + 过户费{transfer_fee:.2f}元）")
                        print(f"         💰 总成本: {total_cost:.2f}元")
                        
                        remaining_balance = max(0, current_balance - total_cost)
                        if remaining_balance < 1:  # 剩余资金很少
                            print(f"         ⚠️  接近资金上限（可用资金: {current_balance:.2f}元，使用率: {total_cost/current_balance*100:.1f}%）")
                        print(f"         📈 买入后持仓: {shares_held + shares_bought:.2f}股 | 剩余资金: {remaining_balance:.2f}元")
                        
                        # 如果当前价格与建议价格不同，给出价格提示
                        if abs(current_price - suggested_buy_price) > 0.01:
                            price_diff = suggested_buy_price - current_price
                            price_diff_pct = (price_diff / current_price * 100) if current_price > 0 else 0
                            if price_diff < 0:
                                print(f"         💡 当前价格 {current_price:.2f}元 高于建议价格 {suggested_buy_price:.2f}元（高{abs(price_diff_pct):.2f}%），可等待回调至建议价格再买入")
                            else:
                                print(f"         💡 当前价格 {current_price:.2f}元 低于建议价格 {suggested_buy_price:.2f}元（低{abs(price_diff_pct):.2f}%），可在当前价格买入")
                    elif current_balance < 100:
                        print(f"      ⚠️  建议买入，但可用资金不足（可用资金: {current_balance:.2f}元 < 最小交易金额100元）")
                    else:
                        print(f"      ⚠️  建议买入，但可用资金不足（可用资金: {current_balance:.2f}元，建议价格: {suggested_buy_price:.2f}元）")
            ...
            if shares_held > 0:
                ...
                # 参考回测结果：航天电器002025回测表现（+21.20%收益率，夏普比率1.87）
                # 获取预测价格和波动率
                ...
                # V12优化：参考回测结果调整卖出价格区间
                # 回测结果参考值（基于训练时的回测表现）
                # 航天电器002025回测：+21.20%收益率，夏普比率1.87，胜率5.03%
                backtest_return_pct = 21.20  # 回测收益率
                backtest_sharpe = 1.87  # 回测夏普比率
                backtest_win_rate = 5.03  # 回测胜率（虽然低，但收益率高说明策略有效）
                
                # 根据回测表现调整卖出倍数
                # 如果回测表现好（高收益率、高夏普），可以更激进地设置卖出上限
                # 如果回测表现差，应该更保守
                # 归一化评分：收益率部分（0-30%映射到0-1），夏普比率部分（0-3.0映射到0-1）
                return_score = min(1.0, backtest_return_pct / 30.0)  # 收益率评分，30%为满分
                sharpe_score = min(1.0, backtest_sharpe / 3.0)  # 夏普比率评分，3.0为满分
                backtest_score = return_score * 0.5 + sharpe_score * 0.5  # 综合评分（0-1）
                backtest_multiplier = 1.0 + backtest_score * 1.5  # 倍数范围：1.0-2.5
                
                # 计算卖出价格区间的上限：基于预测价格和波动率
                # V12优化：更合理的卖出上限计算，避免价格过高
                if price_change_pct > 0:
                    # 预测上涨：卖出上限应该基于预测价格，加上合理的涨幅扩展
                    # 方法1：预测价格 * (1 + 预测涨幅% * 扩展倍数)
                    # 扩展倍数：根据回测表现和预测涨幅动态调整，范围1.2-2.0倍
                    # 如果预测涨幅大，扩展倍数可以小一些；如果预测涨幅小，扩展倍数可以大一些
                    if abs(price_change_pct) >= 5.0:
                        # 预测涨幅大（>=5%）：扩展倍数较小，避免过度乐观
                        extension_multiplier = 1.2 + (backtest_multiplier - 1.0) * 0.3  # 1.2-1.65倍
                    elif abs(price_change_pct) >= 2.0:
                        # 预测涨幅中等（2%-5%）：扩展倍数中等
                        extension_multiplier = 1.3 + (backtest_multiplier - 1.0) * 0.4  # 1.3-1.9倍
                    else:
                        # 预测涨幅小（<2%）：扩展倍数较大
                        extension_multiplier = 1.4 + (backtest_multiplier - 1.0) * 0.6  # 1.4-2.2倍
                    
                    # 限制扩展倍数在合理范围（1.2-2.5倍）
                    extension_multiplier = max(1.2, min(2.5, extension_multiplier))
                    
                    # 卖出上限 = 预测价格 * (1 + 预测涨幅% * 扩展倍数 / 100)
                    # 这样即使预测涨幅很大，卖出上限也不会过度夸张
                    sell_max_price = predicted_price * (1 + abs(price_change_pct) / 100 * extension_multiplier)
                else:
                    # 预测下跌：卖出上限基于当前价格，保守卖出（也考虑回测表现）
                    sell_max_price = current_price + current_price * volatility_pct / 100 * 2.0 * backtest_multiplier
                ...
                # V12调试：显示回测结果应用情况（可选，用于验证）
                # print(f"         📊 回测参考: 收益率{backtest_return_pct:.2f}%, 夏普{backtest_sharpe:.2f}, 倍数{backtest_multiplier:.2f}x, 卖出区间: {sell_min_price:.2f}元 - {sell_max_price:.2f}元")
                ...
                for target_position_pct, sell_pct, label in gradient_levels_sell:
                    ...
                    if gradient_price > 0:
                        ...
                        if shares_sold > 0:
                            ...
                            # V12优化：放宽显示条件，基于回测结果扩展的价格区间应该显示更多档位
                            # 如果价格高于当前价格（等待上涨卖出），显示范围扩大到10%
                            # 如果价格低于当前价格（等待更好价格），只显示差异在5%以内的
                            if price_diff > 0:
                                # 价格高于当前价格：显示差异在10%以内的建议（基于回测扩展的区间）
                                show_condition = price_diff_pct <= 10.0
                            else:
                                # 价格低于当前价格：只显示差异在5%以内的
                                show_condition = abs(price_diff_pct) <= 5.0
