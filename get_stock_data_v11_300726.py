# get_stock_data_v11_300726.py - V11宏达电子300726专用版
# -*- coding: utf-8 -*-
"""
V11 宏达电子300726专用版：
1. 专门针对宏达电子300726进行数据获取和训练
2. 包含宏达电子300726及其相关股票，确保更好的针对性
3. 初始资金2万元（匹配实盘操作）
4. 数据用于V11全功能集成版训练
"""
...
# V11 宏达电子300726专用股票列表
# 包含宏达电子300726及其相关股票，确保更好的针对性
stocks = [
    # === 核心标的：宏达电子300726 ===
...
print("="*70)
print("V11 宏达电子300726专用版 - 数据获取")
print("="*70)
print(f"总共 {len(stocks)} 只标的")
print(f"  核心标的: 宏达电子300726")
...
for stock in stocks:
    ...
    try:
        ...
        if len(train_data) < 100:
            ...
            fail_count += 1
            if code == "sz.300726":
                print(f"  [严重] 宏达电子300726训练数据不足！")
            continue
        
        # 保存到专用目录（V11可以使用V7的数据格式）
        os.makedirs('stockdata_v7_300726/train', exist_ok=True)
        os.makedirs('stockdata_v7_300726/test', exist_ok=True)
        
        train_file = f'stockdata_v7_300726/train/{code}.{name}.csv'
        test_file = f'stockdata_v7_300726/test/{code}.{name}.csv'
        ...
        if code == "sz.300726":
            hongda_dianzi_success = True
            print(f"  [核心] ✅ 宏达电子300726数据获取成功！")
...
if hongda_dianzi_success:
    print(f"\n[核心] ✅ 宏达电子300726数据获取成功！")
else:
    print(f"\n[严重] ❌ 宏达电子300726数据获取失败！请检查！")
...
if success_count > 0:
    ...
    metadata_df.to_csv('stockdata_v7_300726/metadata_v7_300726.csv', index=False, encoding='utf-8-sig')
    print(f"\n元数据已保存: stockdata_v7_300726/metadata_v7_300726.csv")
    
    print("\n[完成] 可以开始训练：")
    print("  python train_v11_300726.py")
    print("\n[说明] 本版本专门针对宏达电子300726优化，初始资金2万元")
