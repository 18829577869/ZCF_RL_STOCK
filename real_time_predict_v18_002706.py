"""
-V18 实时预测系统 - 日K线均线信息版（良信股份002706专用）
...
# 基础配置
MODEL_PATH = "ppo_stock_v7_002837.zip"  # V12使用通用PPO模型，也可以使用专用模型
# 支持通过环境变量或命令行参数指定股票代码
STOCK_CODE = os.getenv('STOCK_CODE', 'sz.002706')  # 股票代码（默认：良信股份002706）
...
CANDIDATE_MODELS = [  # 候选模型列表（按优先级排序）
    {
        'name': '002837模型（最佳）',
        'paths': [
            'ppo_stock_v7_002837.zip',
            'models_v7_002837/best/best_model.zip'
        ],
        'description': '英维克002837模型（最佳匹配）'
    },
    {
        'name': '通用模型',
        'paths': [
