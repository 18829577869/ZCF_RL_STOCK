# get_stock_data_v11_000425.py - V11徐工机械000425专用版
# -*- coding: utf-8 -*-
"""
V11 徐工机械000425专用版：
1. 专门针对徐工机械000425进行数据获取和训练
2. 包含徐工机械000425及其相关股票，确保更好的针对性
3. 初始资金2万元（匹配实盘操作）
4. 数据用于V11全功能集成版训练
"""
...
# V11 徐工机械000425专用股票列表
# 包含徐工机械000425及其相关股票，确保更好的针对性
stocks = [
    # === 核心标的：徐工机械000425 ===
    {"code": "sz.000425", "name": "徐工机械", "start_date": "1996-08-28", 
     "category": "工程机械", "volatility": "中", "style": "平衡", "priority": "核心"},
    
    # === 工程机械板块（相关股票）===
    {"code": "sh.600031", "name": "三一重工", "start_date": "2003-07-03", 
     "category": "工程机械", "volatility": "中", "style": "平衡", "priority": "相关"},
    {"code": "sz.000157", "name": "中联重科", "start_date": "2000-10-12", 
     "category": "工程机械", "volatility": "中", "style": "平衡", "priority": "相关"},
    {"code": "sz.000528", "name": "柳工", "start_date": "1993-11-18", 
     "category": "工程机械", "volatility": "中", "style": "平衡", "priority": "相关"},
    {"code": "sh.600761", "name": "安徽合力", "start_date": "1996-10-09", 
     "category": "工程机械", "volatility": "中", "style": "平衡", "priority": "相关"},
    {"code": "sz.002202", "name": "金风科技", "start_date": "2007-12-26", 
     "category": "工程机械", "volatility": "中", "style": "平衡", "priority": "相关"},
    
    # === 机械设备板块（相关股票）===
    {"code": "sz.002050", "name": "三花智控", "start_date": "2005-06-07", 
     "category": "机械设备", "volatility": "中", "style": "平衡", "priority": "相关"},
...
print("="*70)
print("V11 徐工机械000425专用版 - 数据获取")
print("="*70)
print(f"总共 {len(stocks)} 只标的")
print(f"  核心标的: 徐工机械000425")
...
for stock in stocks:
    ...
    try:
        ...
        if len(train_data) < 100:
            ...
            fail_count += 1
            if code == "sz.000425":
                print(f"  [严重] 徐工机械000425训练数据不足！")
            continue
        
        # 保存到专用目录（V11可以使用V7的数据格式）
        os.makedirs('stockdata_v7_000425/train', exist_ok=True)
        os.makedirs('stockdata_v7_000425/test', exist_ok=True)
        
        train_file = f'stockdata_v7_000425/train/{code}.{name}.csv'
        test_file = f'stockdata_v7_000425/test/{code}.{name}.csv'
        ...
        if code == "sz.000425":
            xugong_jixie_success = True
            print(f"  [核心] ✅ 徐工机械000425数据获取成功！")
...
if xugong_jixie_success:
    print(f"\n[核心] ✅ 徐工机械000425数据获取成功！")
else:
    print(f"\n[严重] ❌ 徐工机械000425数据获取失败！请检查！")
...
if success_count > 0:
    ...
    metadata_df.to_csv('stockdata_v7_000425/metadata_v7_000425.csv', index=False, encoding='utf-8-sig')
    print(f"\n元数据已保存: stockdata_v7_000425/metadata_v7_000425.csv")
    
    print("\n[完成] 可以开始训练：")
    print("  python train_v11_000425.py")
    print("\n[说明] 本版本专门针对徐工机械000425优化，初始资金2万元")
