"""
V18均线多头排列筛选版本 - 基于V18修改，专门用于检测5个周期的均线多头排列
...
- RSRS指标：阻力支撑相对强度指标，显示RSRS斜率和标准分
  ...
  * 买入信号：标准分 > 0.7
  * 卖出信号：标准分 < -0.7
- 5个周期均线多头排列筛选条件检测：检测股票是否满足以下筛选条件
  1. 30分钟均线多头排列（MA5 > MA10 > MA20 > MA30）
  2. 60分钟均线多头排列（MA5 > MA10 > MA20 > MA30）
  3. 120分钟均线多头排列（MA5 > MA10 > MA20 > MA30）
  4. 日均线多头排列（MA5 > MA10 > MA20 > MA30）
  5. 周均线多头排列（MA5 > MA10 > MA20 > MA30）

V18均线多头排列筛选版本功能：
- 批量运行所有股票的预测，所有股票都使用5个周期均线多头排列筛选条件
- 每个股票预测时显示V18新增的日K线均线、RSRS指标和5个周期的均线多头排列筛选条件
...
STOCK_LIST = [
    # V18均线多头排列版本：所有股票都使用5个周期均线多头排列筛选条件
...
print("\n" + "=" * 70)
print("V18均线多头排列筛选版本 - 所有股票都使用5个周期均线多头排列筛选条件")
print("=" * 70)
print("整合功能:")
...
print("   V18均线多头排列版本: 日K线均线信息、RSRS指标、5个周期均线多头排列筛选条件检测")
print("=" * 70)
print("⭐ V15新增功能:")
...
print("   - 5个周期均线多头排列筛选条件检测：30分钟、60分钟、120分钟、日线、周线均线多头排列（MA5 > MA10 > MA20 > MA30）")
print("⭐ V14功能回顾:")
...
print("=" * 70)
print("⚠️  版本标识: 这是 V18均线多头排列筛选版本，所有股票都检测5个周期的均线多头排列（30分钟、60分钟、120分钟、日线、周线）！")
print("=" * 70 + "\n")
...
def get_60min_kline_data(stock_code, days=60):
    """V18均线多头排列版本新增：获取60分钟K线数据"""
    try:
        import akshare as ak
        code_info = convert_stock_code(stock_code)
        symbol = str(code_info['akshare'])
        ...
        start_date = (datetime.datetime.now() - datetime.timedelta(days=days)).strftime('%Y%m%d')
        try:
            df = ak.stock_zh_a_hist_min_em(symbol=symbol, period="60", adjust="qfq", start_date=start_date, end_date=end_date)
            if df is not None and len(df) > 0:
                if '收盘' in df.columns:
                    df = df.rename(columns={'收盘': 'close', '开盘': 'open', '最高': 'high', '最低': 'low', '成交量': 'volume'})
                elif 'close' not in df.columns:
...
def get_120min_kline_data(stock_code, days=120):
    """V18均线多头排列版本新增：获取120分钟K线数据"""
    try:
        import akshare as ak
        code_info = convert_stock_code(stock_code)
        symbol = str(code_info['akshare'])
        ...
        start_date = (datetime.datetime.now() - datetime.timedelta(days=days)).strftime('%Y%m%d')
        try:
            # 注意：akshare可能不支持120分钟，我们尝试使用60分钟数据聚合
            # 或者使用其他方法获取120分钟数据
            df = ak.stock_zh_a_hist_min_em(symbol=symbol, period="60", adjust="qfq", start_date=start_date, end_date=end_date)
            if df is not None and len(df) > 0:
                if '收盘' in df.columns:
...
def get_weekly_kline_data(stock_code, weeks=250):
    """V18均线多头排列版本新增：获取周线K线数据"""
    try:
        import akshare as ak
        code_info = convert_stock_code(stock_code)
        symbol = str(code_info['akshare'])
...
def check_stock_conditions(stock_code, daily_df=None, min30_df=None, min60_df=None, min120_df=None, weekly_df=None, tech_indicators=None):
    """V18均线多头排列版本：检查股票是否满足5个周期的均线多头排列筛选条件"""
    result = {
        'condition1': False,  # 30分钟均线多头排列
        'condition2': False,  # 60分钟均线多头排列
        'condition3': False,  # 120分钟均线多头排列
        'condition4': False,  # 日均线多头排列
        'condition5': False   # 周均线多头排列
    }
    ...
    try:
        ...
        if min30_df is not None and len(min30_df) >= 30:
            ...
            if len(min30_df_with_indicators) >= 1:
                result['condition1'] = check_ma_bullish_alignment(min30_df_with_indicators)
    except Exception:
        pass
    ...
    try:
        if min60_df is None:
            min60_df = get_60min_kline_data(stock_code, days=60)
        if min60_df is not None and len(min60_df) >= 30:
            min60_df_with_indicators = tech_indicators.calculate_all(min60_df)
            if len(min60_df_with_indicators) >= 1:
                result['condition2'] = check_ma_bullish_alignment(min60_df_with_indicators)
    except Exception:
        pass
    
    # 检查120分钟均线多头排列
    try:
        if min120_df is None:
            min120_df = get_120min_kline_data(stock_code, days=120)
        if min120_df is not None and len(min120_df) >= 30:
            min120_df_with_indicators = tech_indicators.calculate_all(min120_df)
            if len(min120_df_with_indicators) >= 1:
                result['condition3'] = check_ma_bullish_alignment(min120_df_with_indicators)
    except Exception:
        pass
    ...
    try:
        ...
        if daily_df is not None and len(daily_df) >= 30:
            ...
            if len(daily_df_with_indicators) >= 1:
                result['condition4'] = check_ma_bullish_alignment(daily_df_with_indicators)
    except Exception:
        pass
    ...
    try:
        if weekly_df is None:
            weekly_df = get_weekly_kline_data(stock_code, weeks=250)
        if weekly_df is not None and len(weekly_df) >= 30:
            weekly_df_with_indicators = tech_indicators.calculate_all(weekly_df)
            if len(weekly_df_with_indicators) >= 1:
                result['condition5'] = check_ma_bullish_alignment(weekly_df_with_indicators)
    except Exception:
        pass
...
# V18均线多头排列版本：所有股票的最优模型配置（所有股票都使用5个周期均线多头排列筛选条件）
optimal_models_info = [
...
print("⚠️  重要提示: 这是 V18均线多头排列筛选版本，每个股票只运行一次预测，并显示5个周期的均线多头排列筛选条件（30分钟、60分钟、120分钟、日线、周线）！")
    ...
    for stock_info in STOCK_LIST:
        ...
        with OutputCapture() as output_capture:
            ...
            try:
                ...
                # ========== V18均线多头排列版本: 检测5个周期均线多头排列筛选条件 ==========
                stock_conditions = None
                if tech_indicators:
                    try:
                        daily_df_for_conditions = get_daily_kline_df(STOCK_CODE, days=250)
                        min30_df_for_conditions = get_30min_kline_data(STOCK_CODE, days=30)
                        min60_df_for_conditions = get_60min_kline_data(STOCK_CODE, days=60)
                        min120_df_for_conditions = get_120min_kline_data(STOCK_CODE, days=120)
                        weekly_df_for_conditions = get_weekly_kline_data(STOCK_CODE, weeks=250)
                        stock_conditions = check_stock_conditions(
                            STOCK_CODE, 
                            daily_df=daily_df_for_conditions,
                            min30_df=min30_df_for_conditions,
                            min60_df=min60_df_for_conditions,
                            min120_df=min120_df_for_conditions,
                            weekly_df=weekly_df_for_conditions,
                ...
                if price_suggestions:
                    ...
                    if ENABLE_BACKTEST:
                        ...
                        if v17_suggestions:
                            ...
                            # ========== V18均线多头排列版本: 5个周期均线多头排列筛选条件检测结果 ==========
                            stock_conditions_val = locals().get('stock_conditions', None)
                            if stock_conditions_val:
                                print(f"\n   🔍 V18均线多头排列筛选条件检测 [{stock_name}({STOCK_CODE})]:")
                                condition_names = {
                                    'condition1': '条件1: 30分钟均线多头排列',
                                    'condition2': '条件2: 60分钟均线多头排列',
                                    'condition3': '条件3: 120分钟均线多头排列',
                                    'condition4': '条件4: 日均线多头排列',
                                    'condition5': '条件5: 周均线多头排列'
                                }
                                matched_conditions = []
                                ...
                                if matched_conditions:
                                    print(f"\n      📊 满足的筛选条件: {len(matched_conditions)}/5")
                                    print(f"      💡 该股票满足以下筛选条件: {', '.join([c.split(':')[0] for c in matched_conditions])}")
                                else:
                                    print(f"\n      📊 当前不满足任何筛选条件 (0/5)")
                            else:
                                print(f"\n   ⚠️  V18均线多头排列筛选条件检测 [{stock_name}({STOCK_CODE})]: 检测失败，无法显示筛选条件结果")
