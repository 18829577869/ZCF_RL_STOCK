"""
V17批量预测系统 - 批量运行指定股票（汉威科技等7只股票）
整合 V7、V9、V10、V11、V12、V13、V14、V15、V16 的所有功能：
...
V17新增功能：
- 明确买卖价格建议：基于预测模型给出具体的买入价格、第二天卖出价格和长期卖出价格
- 更清晰的交易建议：以良信为例，明确说明在XXX价位买入，在XXX价位卖出（分为第二天和长期）

V17批量预测功能（V17_2版本）：
- 批量运行指定7只股票的预测，每个股票只运行一次预测
- 包含股票：汉威科技300007、安培龙301413、中核科技000777、永鼎股份600105、兆威机电003021、大业股份603278、中国卫星600118
- 每个股票预测时显示明确的买卖价格建议
...
# 批量预测：股票列表（V17_2版本：汉威科技等7只股票）
STOCK_LIST = [
    # 1   sz.300007 汉威科技 → 使用安培龙301413模型进行回测（核心标的）
    {'code': 'sz.300007', 'name': '汉威科技', 'model': 'ppo_stock_v7_301413.zip', 'rank': 1, 'sharpe': None, 'return': None, 'drawdown': None, 'strategy': '🟢 均衡型'},  # 核心标的：传感器行业，使用安培龙301413模型
    # 2   sz.301413 安培龙 → 使用通用模型
    {'code': 'sz.301413', 'name': '安培龙', 'model': 'ppo_stock_v7.zip', 'rank': 2, 'sharpe': None, 'return': None, 'drawdown': None, 'strategy': '🟢 均衡型'},  # 传感器行业
    # 3   sz.000777 中核科技 → 使用通用模型
    {'code': 'sz.000777', 'name': '中核科技', 'model': 'ppo_stock_v7.zip', 'rank': 3, 'sharpe': None, 'return': None, 'drawdown': None, 'strategy': '🟢 均衡型'},  # 核电设备
    # 4   sh.600105 永鼎股份 → 使用通用模型
    {'code': 'sh.600105', 'name': '永鼎股份', 'model': 'ppo_stock_v7.zip', 'rank': 4, 'sharpe': None, 'return': None, 'drawdown': None, 'strategy': '🟢 均衡型'},  # 通信设备
    # 5   sz.003021 兆威机电 → 使用通用模型
    {'code': 'sz.003021', 'name': '兆威机电', 'model': 'ppo_stock_v7.zip', 'rank': 5, 'sharpe': None, 'return': None, 'drawdown': None, 'strategy': '🟢 均衡型'},  # 精密制造
    # 6   sh.603278 大业股份 → 使用通用模型
    {'code': 'sh.603278', 'name': '大业股份', 'model': 'ppo_stock_v7.zip', 'rank': 6, 'sharpe': None, 'return': None, 'drawdown': None, 'strategy': '🟢 均衡型'},  # 金属制品
    # 7   sh.600118 中国卫星 → 使用通用模型
    {'code': 'sh.600118', 'name': '中国卫星', 'model': 'ppo_stock_v7.zip', 'rank': 7, 'sharpe': None, 'return': None, 'drawdown': None, 'strategy': '🟢 均衡型'},  # 航天军工
]
...
print("\n" + "=" * 70)
print("V17 批量预测系统 V17_2 - 明确买卖价格建议版（汉威科技等7只股票）")
print("=" * 70)
print("整合功能:")
...
print("=" * 70)
print("⚠️  版本标识: 这是 V17 版本，新增明确买卖价格建议功能，并保留 V16 所有功能！")
print("=" * 70 + "\n")
...
print("\n" + "=" * 70)
print("🚀 V17批量预测系统 V17_2 - 批量运行汉威科技等7只股票")
print("=" * 70)
print(f"📊 股票数量: {len(STOCK_LIST)}")
# ==================== 打印最优模型列表 ====================
print("\n" + "=" * 120)
print(" " * 30 + "📊 V17 最优模型配置列表")
print("=" * 120)
...
for stock in STOCK_LIST:
    ...
    sharpe = stock.get('sharpe')
    return_pct = stock.get('return')
    drawdown = stock.get('drawdown')
    ...
    print(f"   排名{rank:2d}: {name}({code}) | 夏普{sharpe_str} | 收益{return_str} | 回撤{drawdown_str} | {strategy}")
print("⚠️  重要提示: 这是 V17 批量预测版本，每个股票只运行一次预测，并显示明确的买卖价格建议！")
...
with open(log_file, 'w', encoding='utf-8') as f:
    f.write("=" * 70 + "\n")
    f.write(f"V17批量预测日志 - {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("=" * 70 + "\n")
    f.write(f"股票数量: {len(STOCK_LIST)}\n")
    
    # 写入最优模型配置列表
    f.write("\n" + "=" * 120 + "\n")
    f.write(" " * 30 + "📊 V17 最优模型配置列表\n")
    f.write("=" * 120 + "\n")
    ...
    for stock in STOCK_LIST:
        ...
        return_str = f"{return_pct:+.2f}%" if return_pct is not None else "N/A"
        drawdown_str = f"{drawdown:.2f}%" if drawdown is not None else "N/A"
        f.write(f"   排名{rank:2d}: {name}({code}) | 夏普{sharpe_str} | 收益{return_str} | 回撤{drawdown_str} | {strategy}\n")
    f.write("=" * 70 + "\n\n")
    ...
    for stock_info in STOCK_LIST:
        ...
        with OutputCapture() as output_capture:
            ...
            try:
                ...
                data_source_used = "未知"
                latest_time = None
                # 批量预测：直接使用stock_info中的name，确保显示正确的股票名称
                # stock_name 已在循环开始时从 stock_info['name'] 获取（第5234行）
                print(f"\n{'='*70}")
