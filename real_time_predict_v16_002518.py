"""
V16 实时预测系统 - DeepSeek 轮次复盘增强版（科士达002518专用）
...
V16新增功能：
- 历史截面模式：支持指定历史日期进行回测分析
- 实时预测优化：增强实时行情获取和价格处理逻辑

设计理念：多模型协同工作，智能融合决策，自动选择最优模型，风险控制优先，科学资金管理，丰富数据源
专用标的：科士达002518（充电桩/电力设备）
模型来源：使用中恒电气002364训练的模型进行预测
"""
...
# 基础配置
MODEL_PATH = "ppo_stock_v7_002364.zip"  # V16使用中恒电气002364模型预测科士达002518
STOCK_CODE = 'sz.002518'  # 股票代码（科士达）

# V16仿真配置：可选历史截面模式；此处恢复实时预测
FORCE_USE_MANUAL_PRICE = False         # False 使用实时价优先级；True 时优先手动价
DISABLE_REALTIME_PRICE = False         # False 启用实时行情
SNAPSHOT_DATE_STR = ""                 # 无截面提示
SNAPSHOT_CUTOFF = ""                   # 为空则不截断数据
...
print("\n" + "=" * 70)
print("V16 实时预测系统 - 趋势/震荡双策略 + DeepSeek复盘 + StockAPI（超捷301005专用）")
print("=" * 70)
print("整合功能:")
...
if PPO_AVAILABLE and ENABLE_AUTO_MODEL_SELECTION:
    ...
    if len(candidate_ppo_models) == 0:
        ...
        # 回退到默认模型加载逻辑
        possible_models = [
            "ppo_stock_v7_603698.zip",  # 航天工程603698专用模型（优先）
            "models_v7_603698/best/best_model.zip",
...
if ppo_model and PPO_AVAILABLE:
    try:
        ...
        # 优先使用本标的专用目录（如 stockdata_v7_688208/test），如果不存在则回退到118013专用数据集
        primary_test_data_dir = f'stockdata_v7_{stock_code_num}/test'
        fallback_test_data_dir = 'stockdata_v7_118013/test'
        
        if os.path.exists(primary_test_data_dir):
            test_data_dir = primary_test_data_dir
        elif os.path.exists(fallback_test_data_dir):
            test_data_dir = fallback_test_data_dir
            print(f"⚠️  测试数据目录 {primary_test_data_dir} 不存在，回退使用 {fallback_test_data_dir} 中的测试数据")
        else:
            test_data_dir = None
        ...
        if test_data_dir and os.path.exists(test_data_dir):
            ...
            if test_files:
                ...
                try:
                    # 初始化环境（使用与训练时相同的初始资金）
                    initial_balance = 50000.0  # 科士达专用：初始资金5万元
            ...
            print(f"⚠️  未找到任何可用的测试数据目录，跳过回测统计")
...
while True:
    try:
        ...
        # 截断到指定历史截面
        if SNAPSHOT_CUTOFF and 'time' in df.columns:
            df = df[df['time'].astype(str) <= SNAPSHOT_CUTOFF].copy()
            if df is None or len(df) == 0:
                print(f"⏸️  截断到 {SNAPSHOT_CUTOFF} 后无数据，结束本轮")
                time.sleep(60)
                continue
        ...
        # 减少重试次数，避免频繁失败请求；可通过DISABLE_REALTIME_PRICE跳过
        realtime_price = None
        if not DISABLE_REALTIME_PRICE:
            try:
        ...
        # 如果启用仅用历史截面(FORCE_USE_MANUAL_PRICE)，则跳过实时行情，直接使用持仓/手动价格或数据源价格
        # 先读取持仓编辑器中的价格，用于比较
        manual_price = None
        ...
        # 确定最终使用的价格
        if FORCE_USE_MANUAL_PRICE:
            if manual_price and manual_price > 0:
                current_price = manual_price
                price_source = "持仓编辑器(手动输入/历史截面)"
                print(f"   ✅ (历史截面) 使用持仓编辑器中的价格: {current_price:.2f} (日期提示: {SNAPSHOT_DATE_STR})")
            else:
                current_price = data_source_price
                price_source = "数据源(历史截面)"
                print(f"   ⚠️ (历史截面) 未找到手动价，使用数据源价格: {current_price:.2f} (日期提示: {SNAPSHOT_DATE_STR})")
        else:
        ...
        # 实时换手率（基于 AkShare 实时行情），仅用于提示，不参与决策计算
        try:
            turnover = get_realtime_turnover(STOCK_CODE, debug=False)
        except Exception:
            turnover = None
        
        if turnover is not None and turnover >= 0:
            if turnover >= 5.0:
                print(f"   🔄 实时换手率: {turnover:.2f}%  ⚠️ 换手率偏高，注意短线博弈和筹码快速轮动风险")
            else:
                print(f"   🔄 实时换手率: {turnover:.2f}%")
        else:
            print(f"   🔄 实时换手率: 暂无数据（数据源未提供或查询失败）")
