"""
V13 实时预测系统 - 自动模型选择优化版（歌尔股份002241专用）
整合 V7、V9、V10、V11、V12 的所有功能：
...
V13新增功能：
- 自动模型选择：基于回测结果自动选择最优模型
- 多模型回测：对多个候选模型进行实时回测
- 综合评分系统：综合MAE、RMSE、MAPE、方向准确率计算模型评分
- 动态模型切换：根据回测结果动态切换到最优模型
- 模型性能追踪：记录每个模型的历史表现

设计理念：多模型协同工作，智能融合决策，自动选择最优模型
...
STOCK_CODE = 'sz.002241'  # 股票代码（歌尔股份）
...
CANDIDATE_MODELS = [  # 候选模型列表（按优先级排序）
    ...
    },
    {
        'name': '300726模型',
        'paths': [
            'ppo_stock_v7_300726.zip',
            'models_v7_300726/best/best_model.zip'
        ],
        'description': '宏达电子300726模型'
    },
    {
        'name': '300762模型',
        'paths': [
        ...
        'description': '上海瀚讯300762模型'
    },
    {
        'name': '603267模型',
        'paths': [
            'ppo_stock_v7_603267.zip',
            'models_v7_603267/best/best_model.zip'
        ],
        'description': '鸿远电子603267模型'
    },
    {
        'name': '002025模型',
        'paths': [
...
print("\n" + "=" * 70)
print("V13 实时预测系统 - 自动模型选择优化版（歌尔股份002241专用版）")
print("=" * 70)
print("📌 整合功能:")
...
print("=" * 70)
print("⚠️  版本标识: 这是 V13 版本，支持自动模型选择、止损止盈风险控制和凯利公式资金管理！")
print("=" * 70 + "\n")
...
def check_stop_loss_take_profit(current_price, cost_price, shares_held):
    """
    检查止损止盈条件
    返回: (triggered, action, reason, profit_loss_pct)
    - triggered: 是否触发止损/止盈
    ...
    if profit_loss_pct >= TAKE_PROFIT_PCT:
        if TAKE_PROFIT_ACTION == 0:
            return True, 0, f"触发止盈：盈利{profit_loss_pct:.2f}% >= {TAKE_PROFIT_PCT}%，建议全部卖出", profit_loss_pct
        elif TAKE_PROFIT_ACTION == 1:
            return True, 1, f"触发止盈：盈利{profit_loss_pct:.2f}% >= {TAKE_PROFIT_PCT}%，建议卖出50%锁定利润", profit_loss_pct
        elif TAKE_PROFIT_ACTION == 2:
            return True, 2, f"触发止盈：盈利{profit_loss_pct:.2f}% >= {TAKE_PROFIT_PCT}%，建议卖出25%锁定部分利润", profit_loss_pct
    
    # 检查止损条件
    if profit_loss_pct <= STOP_LOSS_PCT:
        return True, STOP_LOSS_ACTION, f"触发止损：亏损{abs(profit_loss_pct):.2f}% <= {abs(STOP_LOSS_PCT)}%，建议全部卖出止损", profit_loss_pct
    
    # 检查部分止损条件（如果启用）
    if ENABLE_PARTIAL_STOP_LOSS and profit_loss_pct <= PARTIAL_STOP_LOSS_PCT and profit_loss_pct > STOP_LOSS_PCT:
        return True, PARTIAL_STOP_LOSS_ACTION, f"触发部分止损：亏损{abs(profit_loss_pct):.2f}% <= {abs(PARTIAL_STOP_LOSS_PCT)}%，建议卖出50%降低风险", profit_loss_pct
    
    return False, None, None, profit_loss_pct

def apply_stop_loss_take_profit(final_action, current_price, cost_price, shares_held):
    """
    应用止损止盈逻辑，调整最终决策
    返回: (adjusted_action, stop_loss_info)
    """
    triggered, action, reason, profit_loss_pct = check_stop_loss_take_profit(
        current_price, cost_price, shares_held
    )
