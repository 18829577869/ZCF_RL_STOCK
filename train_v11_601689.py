# train_v11_601689.py - V11拓普集团601689专用训练
# -*- coding: utf-8 -*-
"""
V11 拓普集团601689专用版特点：
1. 专门针对拓普集团601689进行训练优化
2. 初始资金2万元（匹配实盘操作）
3. 优先使用拓普集团601689进行训练和评估
4. 包含相关股票确保更好的泛化能力
...
# 扫描V7_601689训练数据（V11使用V7的数据格式）
train_dir = 'stockdata_v7_601689/train'
test_dir = 'stockdata_v7_601689/test'
...
# 优先找到拓普集团601689的文件
tuopu_jituan_file = None
for f in stock_files:
    if '601689' in f or '拓普集团' in f:
        tuopu_jituan_file = f
        break

print("="*70)
print("V11 拓普集团601689专用版 - 训练启动")
print("="*70)
print(f"找到 {len(stock_files)} 只训练标的")
print(f"找到 {len(test_files)} 只测试标的")

if tuopu_jituan_file:
    print(f"✅ 核心标的: 拓普集团601689 - {tuopu_jituan_file}")
else:
    print(f"⚠️  警告: 未找到拓普集团601689的训练数据！")
...
# 加载元数据
metadata_file = 'stockdata_v7_601689/metadata_v7_601689.csv'
if os.path.exists(metadata_file):
    ...
    # 检查拓普集团601689
    tuopu_jituan_meta = metadata[metadata['code'] == 'sh.601689']
    if len(tuopu_jituan_meta) > 0:
        print(f"\n[核心] 拓普集团601689元数据:")
        print(tuopu_jituan_meta[['name', 'category', 'volatility', 'style']].to_string(index=False))
else:
    print(f"\n[警告] 元数据文件不存在: {metadata_file}")

# V11_601689特殊配置：初始资金2万元（匹配实盘）
INITIAL_BALANCE_V11_601689 = 20000  # 2万初始资金，匹配实盘操作

def make_env():
    """随机选择标的创建环境，优先使用拓普集团601689"""
    # 30%概率使用拓普集团601689，70%概率使用其他股票
    if tuopu_jituan_file and random.random() < 0.3:
        selected_file = tuopu_jituan_file
    else:
        selected_file = random.choice(stock_files)
    env = StockTradingEnv(selected_file, initial_balance=INITIAL_BALANCE_V11_601689)
    return env

def make_eval_env():
    """评估环境（优先使用拓普集团601689）"""
    if tuopu_jituan_file:
        return StockTradingEnv(tuopu_jituan_file, initial_balance=INITIAL_BALANCE_V11_601689)
    else:
        return StockTradingEnv(stock_files[0], initial_balance=INITIAL_BALANCE_V11_601689)

print("\n" + "="*70)
print("开始训练【V11 拓普集团601689专用版】")
print("="*70)
print("核心特点：")
print("  [核心] 专门针对拓普集团601689优化")
print("  [配置] 初始资金: 2万元（匹配实盘操作）")
print("  [配置] 包含拓普集团601689及相关股票")
print("  [策略] 训练时30%概率使用拓普集团601689")
print("  [策略] 评估时优先使用拓普集团601689")
...
# 回调
os.makedirs('./models_v7_601689/', exist_ok=True)
os.makedirs('./logs_v7_601689/eval/', exist_ok=True)

checkpoint_callback = CheckpointCallback(
    save_freq=100000 // 16,
    save_path='./models_v7_601689/',
    name_prefix='ppo_stock_v7_601689'
)

eval_callback = EvalCallback(
    eval_env,
    best_model_save_path='./models_v7_601689/best/',
    log_path='./logs_v7_601689/eval/',
    eval_freq=50000 // 16,
    deterministic=True,
    render=False
)

# PPO模型（针对拓普集团601689优化，兼容V11）
model = PPO(
    "MlpPolicy",
    train_env,
    verbose=1,
    ...
    vf_coef=0.5,
    max_grad_norm=0.5,
    tensorboard_log="./logs_v7_601689/"
)

print("开始训练 2,500,000 步...")
print("💡 提示: 训练过程中会优先使用拓普集团601689数据")
...
model.save("ppo_stock_v7_601689.zip")
print("\n[成功] 训练完成！模型已保存：ppo_stock_v7_601689.zip")
...
all_stats = []
category_stats = {}
tuopu_jituan_stats = None

# 优先测试拓普集团601689
test_files_sorted = []
if tuopu_jituan_file:
    # 找到对应的测试文件
    tuopu_jituan_test = None
    for test_file in test_files:
        if '601689' in test_file or '拓普集团' in test_file:
            tuopu_jituan_test = test_file
            test_files_sorted.append(test_file)
            break
    
    # 其他文件
    for test_file in test_files:
        if test_file != tuopu_jituan_test:
            test_files_sorted.append(test_file)
else:
...
for test_file in test_files_sorted:
    ...
    try:
        env = StockTradingEnv(test_file, initial_balance=INITIAL_BALANCE_V11_601689)
        obs, _ = env.reset()
        done = False
        ...
        # 检查是否是拓普集团601689
        is_tuopu_jituan = ('601689' in test_file or '拓普集团' in test_file)
        if is_tuopu_jituan:
            stats['is_core'] = True
            tuopu_jituan_stats = stats
            print("="*70)
            print("🎯 [核心标的] 拓普集团601689回测结果")
            print("="*70)
        else:
            stats['is_core'] = False
        ...
        core_mark = "🎯 [核心]" if is_tuopu_jituan else ""
...
if len(all_stats) > 0:
    ...
    # 拓普集团601689专项统计
    if tuopu_jituan_stats:
        print("\n" + "="*70)
        print("🎯 [核心标的专项统计] 拓普集团601689")
        print("="*70)
        print(f"最终净值: {tuopu_jituan_stats['final_net_worth']:,.2f} 元")
        print(f"总收益率: {tuopu_jituan_stats['total_return']:+.2f}%")
        print(f"最大回撤: {tuopu_jituan_stats['max_drawdown']:.2f}%")
        print(f"夏普比率: {tuopu_jituan_stats['sharpe_ratio']:.2f}")
        print(f"交易次数: {tuopu_jituan_stats['num_trades']}")
        print(f"胜率: {tuopu_jituan_stats['win_rate']:.2f}%")
        print(f"风险事件: {tuopu_jituan_stats['risk_events']} 次")
...
print("[完成] 所有测试完成！")
print("="*70)
print(f"[保存] 模型: ppo_stock_v7_601689.zip")
print(f"[日志] 训练日志: ./logs_v7_601689/")
print(f"[模型] 检查点: ./models_v7_601689/")
print(f"\n[提示] 使用 tensorboard --logdir=./logs_v7_601689/ 查看训练曲线")
print("\n[V11_601689特色] 专门针对拓普集团601689优化，初始资金2万，匹配实盘操作！")
print("\n[使用] 训练完成后，可以使用以下命令进行实时预测：")
print("  python real_time_predict_v11_601689.py")
