...
# 初始资金（与训练时一致）
INITIAL_BALANCE = 100000  # 10万元

print("="*70)
print("V11 上证指数1A0001专用回测脚本")
print("="*70)
...
# 排序：上证指数优先
test_files_sorted = []
if shangzheng_index_test:
    test_files_sorted.append(shangzheng_index_test)
    for test_file in test_files:
...
for test_file in test_files_sorted:
    ...
    try:
        ...
        print(f"[回测] {os.path.basename(test_file)} (min_trade_unit={min_trade_unit})")
        
        env = StockTradingEnv(test_file, initial_balance=INITIAL_BALANCE, min_trade_unit=min_trade_unit)
        obs, _ = env.reset()
        done = False
