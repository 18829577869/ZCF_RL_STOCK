...
def map_action_to_operation_text(action):
    """将动作映射为操作文本（用于融合决策显示，避免"买入100%"等歧义）"""
    operations = {
        0: "看空",
        1: "看空",
        2: "看空",
    ...
    return operations.get(action, "未知")
...
# 基础配置
MODEL_PATH = "ppo_stock_v7_300762.zip"  # V12使用通用PPO模型，也可以使用专用模型
...
print("\n" + "=" * 70)
print("V16 实时预测系统 - 趋势/震荡双策略 + DeepSeek复盘 + StockAPI（航天电器002025专用）")
print("=" * 70)
print("整合功能:")
...
# V12优化：显示冲突检测和调整信息
            if conflict_info and conflict_info.get('has_conflict', False):
                ...
                original_action = conflict_info.get('original_action', ppo_action)
                print(f"      🎯 PPO模型信号: {map_action_to_direction(original_action)}")
                ...
                if price_diff_from_pred >= 3.0:
                    ...
                    if current_price > price_suggestions['predicted_price']:
                        ...
                        if current_position_pct <= 25:
                            ...
                            action_hint = f"✅ 融合决策「看多」但当前价格 {current_price:.2f}元 高于预测价格（偏离{price_diff_from_pred:.2f}%），建议保持{current_position}仓位"
                        consistency_note = f"⚠️  价格偏离预测价格{price_diff_from_pred:.2f}%，已动态调整建议仓位"
                    else:
                        # 当前价格低于预测价格，建议加仓
                        action_hint = f"✅ 融合决策「看多」+ 当前价格 {current_price:.2f}元 低于预测价格（偏离{price_diff_from_pred:.2f}%），建议加仓至{current_position}仓位"
                        consistency_note = "✅ 与融合决策「看多」一致"
                else:
                    # 价格偏离较小，遵循融合决策
                    if current_price <= suggestions['75%']:
                        action_hint = f"✅ 融合决策「看多」+ 当前价格 {current_price:.2f}元 在买入区间，建议满仓买入"
                    elif current_price <= suggestions['50%']:
                        action_hint = f"✅ 融合决策「看多」+ 当前价格 {current_price:.2f}元 接近买入区间，建议高仓位买入（目标100%仓位）"
                    else:
                        action_hint = f"✅ 融合决策「看多」：虽然当前价格 {current_price:.2f}元 略高于预测价格，但模型建议买入，可考虑分批买入或等待回调至 {suggestions['75%']:.2f}元 以下"
                    consistency_note = "✅ 与融合决策「看多」一致"
