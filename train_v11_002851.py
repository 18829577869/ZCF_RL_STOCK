# train_v11_002851.py - V11麦格米特002851专用训练
# -*- coding: utf-8 -*-
"""
V11 麦格米特002851专用版特点：
1. 专门针对麦格米特002851进行训练优化
2. 初始资金2万元（匹配实盘操作）
3. 优先使用麦格米特002851进行训练和评估
4. 包含相关股票确保更好的泛化能力
...
# 扫描V7_002851训练数据（V11使用V7的数据格式）
train_dir = 'stockdata_v7_002851/train'
test_dir = 'stockdata_v7_002851/test'

if not os.path.exists(train_dir):
    print(f"[错误] 训练数据目录不存在: {train_dir}")
    print("请先运行: python get_stock_data_v11_002851.py")
    exit(1)
...
for f in stock_files:
    if '002851' in f or '麦格米特' in f:
        maigemite_file = f
        break

print("="*70)
print("V11 麦格米特002851专用版 - 训练启动")
print("="*70)
print(f"找到 {len(stock_files)} 只训练标的")
print(f"找到 {len(test_files)} 只测试标的")

if maigemite_file:
    print(f"✅ 核心标的: 麦格米特002851 - {maigemite_file}")
else:
    print(f"⚠️  警告: 未找到麦格米特002851的训练数据！")
...
# 加载元数据
metadata_file = 'stockdata_v7_002851/metadata_v7_002851.csv'
if os.path.exists(metadata_file):
    ...
    # 检查麦格米特002851
    maigemite_meta = metadata[metadata['code'] == 'sz.002851']
    if len(maigemite_meta) > 0:
        print(f"\n[核心] 麦格米特002851元数据:")
        print(maigemite_meta[['name', 'category', 'volatility', 'style']].to_string(index=False))
else:
    print(f"\n[警告] 元数据文件不存在: {metadata_file}")

# V11_002851特殊配置：初始资金2万元（匹配实盘）
INITIAL_BALANCE_V11_002851 = 20000  # 2万初始资金，匹配实盘操作

def make_env():
    """随机选择标的创建环境，优先使用麦格米特002851"""
    # 30%概率使用麦格米特002851，70%概率使用其他股票
    if maigemite_file and random.random() < 0.3:
        selected_file = maigemite_file
    else:
        selected_file = random.choice(stock_files)
    env = StockTradingEnv(selected_file, initial_balance=INITIAL_BALANCE_V11_002851)
    return env

def make_eval_env():
    """评估环境（优先使用麦格米特002851）"""
    if maigemite_file:
        return StockTradingEnv(maigemite_file, initial_balance=INITIAL_BALANCE_V11_002851)
    else:
        return StockTradingEnv(stock_files[0], initial_balance=INITIAL_BALANCE_V11_002851)

print("\n" + "="*70)
print("开始训练【V11 麦格米特002851专用版】")
print("="*70)
print("核心特点：")
print("  [核心] 专门针对麦格米特002851优化")
print("  [配置] 初始资金: 2万元（匹配实盘操作）")
print("  [配置] 包含麦格米特002851及相关股票")
print("  [策略] 训练时30%概率使用麦格米特002851")
print("  [策略] 评估时优先使用麦格米特002851")
...
# 回调
os.makedirs('./models_v7_002851/', exist_ok=True)
os.makedirs('./logs_v7_002851/eval/', exist_ok=True)

checkpoint_callback = CheckpointCallback(
    save_freq=100000 // 16,
    save_path='./models_v7_002851/',
    name_prefix='ppo_stock_v7_002851'
)

eval_callback = EvalCallback(
    eval_env,
    best_model_save_path='./models_v7_002851/best/',
    log_path='./logs_v7_002851/eval/',
    eval_freq=50000 // 16,
    deterministic=True,
    render=False
)

# PPO模型（针对麦格米特002851优化，兼容V11）
model = PPO(
    "MlpPolicy",
    train_env,
    verbose=1,
    ...
    vf_coef=0.5,
    max_grad_norm=0.5,
    tensorboard_log="./logs_v7_002851/"
)

print("开始训练 2,500,000 步...")
print("💡 提示: 训练过程中会优先使用麦格米特002851数据")
...
model.save("ppo_stock_v7_002851.zip")
print("\n[成功] 训练完成！模型已保存：ppo_stock_v7_002851.zip")
...
if maigemite_file:
    ...
    for test_file in test_files:
        if '002851' in test_file or '麦格米特' in test_file:
            maigemite_test = test_file
            test_files_sorted.append(test_file)
            break
    
    # 其他文件
    for test_file in test_files:
        if test_file != maigemite_test:
            test_files_sorted.append(test_file)
else:
...
for test_file in test_files_sorted:
    ...
    try:
        env = StockTradingEnv(test_file, initial_balance=INITIAL_BALANCE_V11_002851)
        obs, _ = env.reset()
        done = False
        ...
        # 检查是否是麦格米特002851
        is_maigemite = ('002851' in test_file or '麦格米特' in test_file)
        if is_maigemite:
            stats['is_core'] = True
            maigemite_stats = stats
            print("="*70)
            print("🎯 [核心标的] 麦格米特002851回测结果")
            print("="*70)
        else:
            stats['is_core'] = False
        ...
        core_mark = "🎯 [核心]" if is_maigemite else ""
...
if len(all_stats) > 0:
    ...
    if maigemite_stats:
        print("\n" + "="*70)
        print("🎯 [核心标的专项统计] 麦格米特002851")
        print("="*70)
        print(f"最终净值: {maigemite_stats['final_net_worth']:,.2f} 元")
        print(f"总收益率: {maigemite_stats['total_return']:+.2f}%")
        print(f"最大回撤: {maigemite_stats['max_drawdown']:.2f}%")
        print(f"夏普比率: {maigemite_stats['sharpe_ratio']:.2f}")
        print(f"交易次数: {maigemite_stats['num_trades']}")
        print(f"胜率: {maigemite_stats['win_rate']:.2f}%")
        print(f"风险事件: {maigemite_stats['risk_events']} 次")
...
print("[完成] 所有测试完成！")
print("="*70)
print(f"[保存] 模型: ppo_stock_v7_002851.zip")
print(f"[日志] 训练日志: ./logs_v7_002851/")
print(f"[模型] 检查点: ./models_v7_002851/")
print(f"\n[提示] 使用 tensorboard --logdir=./logs_v7_002851/ 查看训练曲线")
print("\n[V11_002851特色] 专门针对麦格米特002851优化，初始资金2万，匹配实盘操作！")
print("\n[使用] 训练完成后，可以使用以下命令进行实时预测：")
print("  python real_time_predict_v11_002851.py")
