# train_v11_688102.py - V11斯瑞新材688102专用训练
# -*- coding: utf-8 -*-
"""
V11 斯瑞新材688102专用版特点：
1. 专门针对斯瑞新材688102进行训练优化
2. 初始资金5万元（匹配实盘/半实盘配置）
3. 优先使用斯瑞新材688102进行训练和评估
4. 包含相关高端材料/新能源/军工/大盘标的，确保更好的泛化能力
...
# 扫描V7_688102训练数据（V11使用V7的数据格式）
train_dir = 'stockdata_v7_688102/train'
test_dir = 'stockdata_v7_688102/test'

if not os.path.exists(train_dir):
    print(f"[错误] 训练数据目录不存在: {train_dir}")
    print("请先运行: python get_stock_data_v11_688102.py")
    exit(1)
...
for f in stock_files:
    if '688102' in f or '斯瑞新材' in f:
        sirui_file = f
        break

print("=" * 70)
print("V11 斯瑞新材688102专用版 - 训练启动")
print("=" * 70)
print(f"找到 {len(stock_files)} 只训练标的")
print(f"找到 {len(test_files)} 只测试标的")

if sirui_file:
    print(f"✅ 核心标的: 斯瑞新材688102 - {sirui_file}")
else:
    print(f"⚠️  警告: 未找到斯瑞新材688102的训练数据！")
...
# 加载元数据
metadata_file = 'stockdata_v7_688102/metadata_v7_688102.csv'
if os.path.exists(metadata_file):
    ...
    # 检查斯瑞新材688102
    sirui_meta = metadata[metadata['code'] == 'sh.688102']
    if len(sirui_meta) > 0:
        print(f"\n[核心] 斯瑞新材688102元数据:")
        print(sirui_meta[['name', 'category', 'volatility', 'style']].to_string(index=False))
else:
    print(f"\n[警告] 元数据文件不存在: {metadata_file}")

# V11_688102特殊配置：初始资金5万元（匹配实盘）
INITIAL_BALANCE_V11_688102 = 50000  # 5万初始资金，匹配实盘操作

def make_env():
    """随机选择标的创建环境，优先使用斯瑞新材688102"""
    # 30%概率使用斯瑞新材688102，70%概率使用其他股票
    if sirui_file and random.random() < 0.3:
        selected_file = sirui_file
    else:
        selected_file = random.choice(stock_files)
    env = StockTradingEnv(selected_file, initial_balance=INITIAL_BALANCE_V11_688102)
    return env

def make_eval_env():
    """评估环境（优先使用斯瑞新材688102）"""
    if sirui_file:
        return StockTradingEnv(sirui_file, initial_balance=INITIAL_BALANCE_V11_688102)
    else:
        return StockTradingEnv(stock_files[0], initial_balance=INITIAL_BALANCE_V11_688102)

print("\n" + "=" * 70)
print("开始训练【V11 斯瑞新材688102专用版】")
print("=" * 70)
print("核心特点：")
print("  [核心] 专门针对斯瑞新材688102优化")
print("  [配置] 初始资金: 5万元（匹配实盘/半实盘操作）")
print("  [配置] 包含斯瑞新材及相关高端材料/新能源/军工/大盘标的")
print("  [策略] 训练时30%概率使用斯瑞新材688102")
print("  [策略] 评估时优先使用斯瑞新材688102")
...
# 回调
os.makedirs('./models_v7_688102/', exist_ok=True)
os.makedirs('./logs_v7_688102/eval/', exist_ok=True)

checkpoint_callback = CheckpointCallback(
    save_freq=100000 // 16,
    save_path='./models_v7_688102/',
    name_prefix='ppo_stock_v7_688102'
)

eval_callback = EvalCallback(
    eval_env,
    best_model_save_path='./models_v7_688102/best/',
    log_path='./logs_v7_688102/eval/',
    eval_freq=50000 // 16,
    deterministic=True,
    render=False
)

# PPO模型（针对斯瑞新材688102优化，兼容V11）
model = PPO(
    "MlpPolicy",
    train_env,
    verbose=1,
    ...
    vf_coef=0.5,
    max_grad_norm=0.5,
    tensorboard_log="./logs_v7_688102/"
)

print("开始训练 2,500,000 步...")
print("💡 提示: 训练过程中会优先使用斯瑞新材688102数据")
...
model.save("ppo_stock_v7_688102.zip")
print("\n[成功] 训练完成！模型已保存：ppo_stock_v7_688102.zip")
...
if sirui_file:
    ...
    for test_file in test_files:
        if '688102' in test_file or '斯瑞新材' in test_file:
            sirui_test = test_file
            test_files_sorted.append(test_file)
            break

    for test_file in test_files:
        if test_file != sirui_test:
            test_files_sorted.append(test_file)
else:
...
for test_file in test_files_sorted:
    ...
    try:
        env = StockTradingEnv(test_file, initial_balance=INITIAL_BALANCE_V11_688102)
        obs, _ = env.reset()
        done = False
        ...
        is_sirui = ('688102' in test_file or '斯瑞新材' in test_file)
        if is_sirui:
            stats['is_core'] = True
            sirui_stats = stats
            print("=" * 70)
            print("🎯 [核心标的] 斯瑞新材688102回测结果")
            print("=" * 70)
        else:
            stats['is_core'] = False
...
if len(all_stats) > 0:
    ...
    if sirui_stats:
        print("\n" + "=" * 70)
        print("🎯 [核心标的专项统计] 斯瑞新材688102")
        print("=" * 70)
        print(f"最终净值: {sirui_stats['final_net_worth']:,.2f} 元")
        print(f"总收益率: {sirui_stats['total_return']:+.2f}%")
        print(f"最大回撤: {sirui_stats['max_drawdown']:.2f}%")
        print(f"夏普比率: {sirui_stats['sharpe_ratio']:.2f}")
        print(f"交易次数: {sirui_stats['num_trades']}")
        print(f"胜率: {sirui_stats['win_rate']:.2f}%")
        print(f"风险事件: {sirui_stats['risk_events']} 次")
...
print("[完成] 所有测试完成！")
print("=" * 70)
print(f"[保存] 模型: ppo_stock_v7_688102.zip")
print(f"[日志] 训练日志: ./logs_v7_688102/")
print(f"[模型] 检查点: ./models_v7_688102/")
print(f"\n[提示] 使用 tensorboard --logdir=./logs_v7_688102/ 查看训练曲线")
print("\n[V11_688102特色] 专门针对斯瑞新材688102优化，初始资金5万，匹配实盘/半实盘操作！")
print("\n[使用] 训练完成后，可以使用以下命令进行实时预测：")
print("  python real_time_predict_v11_688102.py")
