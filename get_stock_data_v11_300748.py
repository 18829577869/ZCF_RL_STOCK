# get_stock_data_v11_300748.py - V11金力永磁300748专用版
# -*- coding: utf-8 -*-
"""
V11 金力永磁300748专用版：
1. 专门针对金力永磁300748进行数据获取和训练
2. 包含金力永磁300748及其相关股票，确保更好的针对性
3. 初始资金2万元（匹配实盘操作）
4. 数据用于V11全功能集成版训练
"""
...
# V11 金力永磁300748专用股票列表
# 包含金力永磁300748及其相关股票，确保更好的针对性
stocks = [
    # === 核心标的：金力永磁300748 ===
    {"code": "sz.300748", "name": "金力永磁", "start_date": "2018-09-21",
     "category": "稀土永磁", "volatility": "高", "style": "周期", "priority": "核心"},

    # === 稀土/永磁材料板块（相关股票）===
    {"code": "sz.000831", "name": "中国稀土", "start_date": "1998-09-11",
     "category": "稀土", "volatility": "高", "style": "周期", "priority": "相关"},
    {"code": "sh.600111", "name": "北方稀土", "start_date": "1997-09-02",
     "category": "稀土", "volatility": "高", "style": "周期", "priority": "相关"},
    {"code": "sh.600392", "name": "盛和资源", "start_date": "2004-03-18",
     "category": "稀土", "volatility": "高", "style": "周期", "priority": "相关"},

    # === 稀有金属/新能源材料板块（相关股票）===
    {"code": "sz.002460", "name": "赣锋锂业", "start_date": "2010-08-10",
     "category": "稀有金属", "volatility": "高", "style": "周期", "priority": "相关"},
    {"code": "sz.002466", "name": "天齐锂业", "start_date": "2010-08-31",
     "category": "稀有金属", "volatility": "高", "style": "周期", "priority": "相关"},
    {"code": "sh.603993", "name": "洛阳钼业", "start_date": "2012-10-09",
     "category": "稀有金属", "volatility": "高", "style": "周期", "priority": "相关"},
    {"code": "sh.603799", "name": "华友钴业", "start_date": "2015-01-29",
     "category": "稀有金属", "volatility": "高", "style": "周期", "priority": "相关"},

    # === 有色金属板块（相关股票）===
    {"code": "sh.000792", "name": "盐湖股份", "start_date": "1997-09-04",
     "category": "有色金属", "volatility": "高", "style": "周期", "priority": "相关"},
    {"code": "sz.002738", "name": "中矿资源", "start_date": "2014-12-30",
     "category": "有色金属", "volatility": "高", "style": "周期", "priority": "相关"},
...
print("=" * 70)
print("V11 金力永磁300748专用版 - 数据获取")
print("=" * 70)
print(f"总共 {len(stocks)} 只标的")
print(f"  核心标的: 金力永磁300748")
...
for stock in stocks:
    ...
    try:
        ...
        if len(train_data) < 100:
            ...
            fail_count += 1
            if code == "sz.300748":
                print(f"  [严重] 金力永磁300748训练数据不足！")
            continue

        # 保存到专用目录（V11可以使用V7的数据格式）
        os.makedirs('stockdata_v7_300748/train', exist_ok=True)
        os.makedirs('stockdata_v7_300748/test', exist_ok=True)

        train_file = f'stockdata_v7_300748/train/{code}.{name}.csv'
        test_file = f'stockdata_v7_300748/test/{code}.{name}.csv'
        ...
        if code == "sz.300748":
            jinli_yongci_success = True
            print(f"  [核心] ✅ 金力永磁300748数据获取成功！")
...
if jinli_yongci_success:
    print(f"\n[核心] ✅ 金力永磁300748数据获取成功！")
else:
    print(f"\n[严重] ❌ 金力永磁300748数据获取失败！请检查！")
...
if success_count > 0:
    ...
    metadata_df.to_csv('stockdata_v7_300748/metadata_v7_300748.csv', index=False, encoding='utf-8-sig')
    print(f"\n元数据已保存: stockdata_v7_300748/metadata_v7_300748.csv")

    print("\n[完成] 可以开始训练：")
    print("  python train_v11_300748.py")
    print("\n[说明] 本版本专门针对金力永磁300748优化，初始资金2万元")
