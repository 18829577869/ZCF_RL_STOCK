"""
V11 实时预测系统 - 全功能集成版（上证指数1A0001专用）
整合 V7、V9、V10 的所有功能：
...
def convert_stock_code(code):
    ...
    if '.' in code:
        ...
        # 处理指数代码（1A0001等）
        if num.startswith('1A') or num == '000001':
            # 上证指数：baostock使用sh.000001，akshare使用000001
            return {
                'baostock': 'sh.000001',  # baostock中上证指数是sh.000001
                'tushare': '000001.SH',
                'akshare': '000001',  # akshare中上证指数是000001
                'market': 'sh',
                'is_index': True
            }
        return {
...
def fetch_akshare_5min(code_info, days=7):
    """使用 AkShare 获取5分钟K线数据（支持指数）"""
    try:
        import akshare as ak
        symbol = code_info.get('akshare', '')
        is_index = code_info.get('is_index', False)
...
# 基础配置
MODEL_PATH = "ppo_stock_v7_1A0001.zip"  # 上证指数专用模型
STOCK_CODE = 'sh.000001'  # 上证指数代码（baostock格式：sh.000001，也支持1A0001）
