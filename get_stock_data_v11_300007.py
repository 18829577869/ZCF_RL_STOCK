# get_stock_data_v11_300007.py - V11汉威科技300007专用版
# -*- coding: utf-8 -*-
"""
V11 汉威科技300007专用版：
1. 专门针对汉威科技300007进行数据获取和训练
2. 包含汉威科技300007及其相关股票，确保更好的针对性
3. 初始资金2万元（匹配实盘操作）
4. 数据用于V11全功能集成版训练
"""
...
# V11 汉威科技300007专用股票列表
# 包含汉威科技300007及其相关股票，确保更好的针对性
stocks = [
    # === 核心标的：汉威科技300007 ===
...
print("="*70)
print("V11 汉威科技300007专用版 - 数据获取")
print("="*70)
print(f"总共 {len(stocks)} 只标的")
print(f"  核心标的: 汉威科技300007")
...
for stock in stocks:
    ...
    try:
        ...
        if len(train_data) < 100:
            ...
            fail_count += 1
            if code == "sz.300007":
                print(f"  [严重] 汉威科技300007训练数据不足！")
            continue
        
        # 保存到专用目录（V11可以使用V7的数据格式）
        os.makedirs('stockdata_v7_300007/train', exist_ok=True)
        os.makedirs('stockdata_v7_300007/test', exist_ok=True)
        
        train_file = f'stockdata_v7_300007/train/{code}.{name}.csv'
        test_file = f'stockdata_v7_300007/test/{code}.{name}.csv'
        ...
        if code == "sz.300007":
            hanwei_keji_success = True
            print(f"  [核心] ✅ 汉威科技300007数据获取成功！")
...
if hanwei_keji_success:
    print(f"\n[核心] ✅ 汉威科技300007数据获取成功！")
else:
    print(f"\n[严重] ❌ 汉威科技300007数据获取失败！请检查！")
...
if success_count > 0:
    ...
    metadata_df.to_csv('stockdata_v7_300007/metadata_v7_300007.csv', index=False, encoding='utf-8-sig')
    print(f"\n元数据已保存: stockdata_v7_300007/metadata_v7_300007.csv")
    
    print("\n[完成] 可以开始训练：")
    print("  python train_v11_300007.py")
    print("\n[说明] 本版本专门针对汉威科技300007优化，初始资金2万元")
