# get_stock_data_v11_600730_301005.py - V11中国高科和超捷股份专用版
# -*- coding: utf-8 -*-
"""
V11 中国高科和超捷股份专用版：
1. 专门针对中国高科(sh.600730)和超捷股份(sz.301005)进行数据获取和训练
2. 包含相关股票，确保更好的针对性
3. 初始资金10万元
4. 数据用于V11全功能集成版训练
"""
...
stocks = [
    # === 核心标的：中国高科600730 ===
    {"code": "sh.600730", "name": "中国高科", "start_date": "1993-12-20", 
     "category": "教育", "volatility": "中", "style": "平衡", "priority": "核心"},
    
    # === 核心标的：超捷股份301005 ===
    {"code": "sz.301005", "name": "超捷股份", "start_date": "2021-06-01", 
     "category": "汽车零部件", "volatility": "高", "style": "成长", "priority": "核心"},
    
    # === 教育板块（相关股票）===
    {"code": "sz.002607", "name": "中公教育", "start_date": "2011-08-10", 
     "category": "教育", "volatility": "高", "style": "成长", "priority": "相关"},
    {"code": "sz.300359", "name": "全通教育", "start_date": "2014-01-21", 
     "category": "教育", "volatility": "高", "style": "成长", "priority": "相关"},
...
print("="*70)
print("V11 中国高科和超捷股份专用版 - 数据获取")
print("="*70)
print(f"总共 {len(stocks)} 只标的")
print(f"  核心标的: 中国高科(sh.600730)、超捷股份(sz.301005)")
    ...
    try:
        ...
        if len(train_data) < 100:
            ...
            if code == "sh.600730":
                ...
                print(f"  [严重] 超捷股份301005训练数据不足！")
            continue
        
        # 保存到专用目录（V11可以使用V7的数据格式）
        os.makedirs('stockdata_v7_600730_301005/train', exist_ok=True)
        os.makedirs('stockdata_v7_600730_301005/test', exist_ok=True)
        
        train_file = f'stockdata_v7_600730_301005/train/{code}.{name}.csv'
        test_file = f'stockdata_v7_600730_301005/test/{code}.{name}.csv'
        ...
        if code == "sh.600730":
            ...
            chaojie_success = True
            print(f"  [核心] ✅ 超捷股份301005数据获取成功！")
            print(f"  [核心] 训练数据: {len(train_data)}条，测试数据: {len(test_data)}条")
...
if chaojie_success:
    ...
    print(f"[严重] ❌ 超捷股份301005数据获取失败！请检查！")
...
if success_count > 0:
    ...
    metadata_df.to_csv('stockdata_v7_600730_301005/metadata_v7_600730_301005.csv', index=False, encoding='utf-8-sig')
    print(f"\n元数据已保存: stockdata_v7_600730_301005/metadata_v7_600730_301005.csv")
    
    print("\n[完成] 可以开始训练：")
    print("  python train_v11_600730_301005.py")
    print("\n[说明] 本版本专门针对中国高科600730和超捷股份301005优化，初始资金10万元")
