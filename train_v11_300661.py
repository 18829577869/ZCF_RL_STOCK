# train_v11_300661.py - V11圣邦股份专用训练
# -*- coding: utf-8 -*-
"""
V11 圣邦股份专用版特点：
1. 专门针对圣邦股份(sz.300661)进行训练优化
2. 初始资金10万元
3. 优先使用圣邦股份进行训练和评估
4. 包含相关股票确保更好的泛化能力
...
# 扫描V7_300661训练数据（V11使用V7的数据格式）
train_dir = 'stockdata_v7_300661/train'
test_dir = 'stockdata_v7_300661/test'

if not os.path.exists(train_dir):
    print(f"[错误] 训练数据目录不存在: {train_dir}")
    print("请先运行: python get_stock_data_v11_300661.py")
    exit(1)
...
for f in stock_files:
    if '300661' in f or '圣邦股份' in f:
        shengbang_file = f
        break

print("="*70)
print("V11 圣邦股份专用版 - 训练启动")
print("="*70)
print(f"找到 {len(stock_files)} 只训练标的")
print(f"找到 {len(test_files)} 只测试标的")

if shengbang_file:
    print(f"✅ 核心标的: 圣邦股份300661 - {shengbang_file}")
else:
    print(f"⚠️  警告: 未找到圣邦股份300661的训练数据！")
...
# 加载元数据
metadata_file = 'stockdata_v7_300661/metadata_v7_300661.csv'
if os.path.exists(metadata_file):
    ...
    # 检查圣邦股份300661
    shengbang_meta = metadata[metadata['code'] == 'sz.300661']
    if len(shengbang_meta) > 0:
        print(f"\n[核心] 圣邦股份300661元数据:")
        print(shengbang_meta[['name', 'category', 'volatility', 'style']].to_string(index=False))
else:
    print(f"\n[警告] 元数据文件不存在: {metadata_file}")

# V11_300661特殊配置：初始资金10万元
INITIAL_BALANCE_V11 = 100000  # 10万初始资金

def make_env():
    """随机选择标的创建环境，优先使用圣邦股份300661"""
    # 30%概率使用圣邦股份300661，70%概率使用其他股票
    if shengbang_file and random.random() < 0.3:
        selected_file = shengbang_file
    else:
...
def make_eval_env():
    """评估环境（优先使用圣邦股份300661）"""
    if shengbang_file:
        return StockTradingEnv(shengbang_file, initial_balance=INITIAL_BALANCE_V11)
    else:
        return StockTradingEnv(stock_files[0], initial_balance=INITIAL_BALANCE_V11)

print("\n" + "="*70)
print("开始训练【V11 圣邦股份专用版】")
print("="*70)
print("核心特点：")
print("  [核心] 专门针对圣邦股份300661优化")
print("  [配置] 初始资金: 10万元")
print("  [配置] 包含圣邦股份及相关股票")
print("  [策略] 训练时30%概率使用圣邦股份300661")
print("  [策略] 评估时优先使用圣邦股份300661")
...
# 回调
os.makedirs('./models_v7_300661/', exist_ok=True)
os.makedirs('./logs_v7_300661/eval/', exist_ok=True)

checkpoint_callback = CheckpointCallback(
    save_freq=100000 // 16,
    save_path='./models_v7_300661/',
    name_prefix='ppo_stock_v7_300661'
)

eval_callback = EvalCallback(
    eval_env,
    best_model_save_path='./models_v7_300661/best/',
    log_path='./logs_v7_300661/eval/',
    eval_freq=50000 // 16,
    deterministic=True,
    render=False
)

# PPO模型（针对圣邦股份优化，兼容V11）
model = PPO(
    "MlpPolicy",
    train_env,
    verbose=1,
    ...
    vf_coef=0.5,
    max_grad_norm=0.5,
    tensorboard_log="./logs_v7_300661/"
)

print("开始训练 2,500,000 步...")
print("💡 提示: 训练过程中会优先使用圣邦股份300661数据")
...
model.save("ppo_stock_v7_300661.zip")
print("\n[成功] 训练完成！模型已保存：ppo_stock_v7_300661.zip")
...
if shengbang_file:
    ...
    for test_file in test_files:
        if '300661' in test_file or '圣邦股份' in test_file:
            shengbang_test = test_file
            test_files_sorted.append(test_file)
            break
    
    # 其他文件
    for test_file in test_files:
        if test_file != shengbang_test:
            test_files_sorted.append(test_file)
else:
...
for test_file in test_files_sorted:
    ...
    try:
        ...
        # 检查是否是圣邦股份300661
        is_shengbang = ('300661' in test_file or '圣邦股份' in test_file)
        if is_shengbang:
            stats['is_core'] = True
            shengbang_stats = stats
            print("="*70)
            print("🎯 [核心标的] 圣邦股份300661回测结果")
            print("="*70)
        else:
            stats['is_core'] = False
        ...
        core_mark = "🎯 [核心]" if is_shengbang else ""
...
if len(all_stats) > 0:
    ...
    if shengbang_stats:
        print("\n" + "="*70)
        print("🎯 [核心标的专项统计] 圣邦股份300661")
        print("="*70)
        print(f"最终净值: {shengbang_stats['final_net_worth']:,.2f} 元")
        print(f"总收益率: {shengbang_stats['total_return']:+.2f}%")
        print(f"最大回撤: {shengbang_stats['max_drawdown']:.2f}%")
        print(f"夏普比率: {shengbang_stats['sharpe_ratio']:.2f}")
        print(f"交易次数: {shengbang_stats['num_trades']}")
        print(f"胜率: {shengbang_stats['win_rate']:.2f}%")
        print(f"风险事件: {shengbang_stats['risk_events']} 次")
...
print("[完成] 所有测试完成！")
print("="*70)
print(f"[保存] 模型: ppo_stock_v7_300661.zip")
print(f"[日志] 训练日志: ./logs_v7_300661/")
print(f"[模型] 检查点: ./models_v7_300661/")
print(f"\n[提示] 使用 tensorboard --logdir=./logs_v7_300661/ 查看训练曲线")
print("\n[V11_300661特色] 专门针对圣邦股份300661优化，初始资金10万！")
print("\n[使用] 训练完成后，可以使用以下命令进行实时预测：")
print("  python real_time_predict_v16_300661.py")
