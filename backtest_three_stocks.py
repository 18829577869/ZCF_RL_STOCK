"""
两只股票回测脚本
针对：中国高科(sh.600730)、超捷股份(sz.301005)
...
# 两只股票配置（V11版本）
TARGET_STOCKS = [
    {'code': 'sh.600730', 'name': '中国高科', 'model': 'ppo_stock_v7_600730.zip', 'data_dir': 'stockdata_v7_600730_301005'},
    {'code': 'sz.301005', 'name': '超捷股份', 'model': 'ppo_stock_v7_301005.zip', 'data_dir': 'stockdata_v7_600730_301005'},
]
...
def get_stock_data_file(stock_code):
    ...
    尝试多个可能的路径
    """
    # 提取股票代码的数字部分（如 sh.600730 -> 600730）
    code_parts = stock_code.split('.')
    code_number = code_parts[-1] if len(code_parts) > 1 else stock_code.replace('.', '')
    
    # 可能的路径列表（按优先级排序）
    possible_paths = [
        # 格式1: stockdata_v7_600730_301005/train/{stock_code}.{name}.csv (V11专用目录)
        f"stockdata_v7_600730_301005/train/{stock_code}.*.csv",
        f"stockdata_v7_600730_301005/test/{stock_code}.*.csv",
...
def calculate_max_drawdown(net_worth_history):
    ...
    peak = np.maximum.accumulate(net_worth_array)  # 累积最大值
    drawdown = (net_worth_array - peak) / peak  # 回撤
    ...
    for i in range(max_dd_idx, -1, -1):
        if net_worth_array[i] == peak[max_dd_idx]:
            peak_idx = i
            break
    ...
    return {
        ...
        'trough_value': float(net_worth_array[max_dd_idx]),
        'peak_index': int(peak_idx),
        'trough_index': int(max_dd_idx)
    }

def run_backtest(stock_code, stock_name, model_path, data_file):
    """
    对单只股票进行回测
    ...
    try:
        ...
        # 夏普比率
        sharpe_ratio = calculate_sharpe_ratio(np.array(daily_returns)) if len(daily_returns) > 0 else 0.0
        
        # 年化收益率
        trading_days = len(env.df)
        if trading_days > 0:
            annual_return = total_return * (252 / trading_days) if trading_days < 252 else total_return
        else:
            annual_return = 0.0
...
def main():
    ...
    for stock in TARGET_STOCKS:
        ...
        if not data_file:
            ...
            code_parts = stock_code.split('.')
            code_number = code_parts[-1] if len(code_parts) > 1 else stock_code.replace('.', '')
            print(f"      - stockdata_v7_{code_number}/train/{stock_code}.*.csv")
            print(f"      - stockdata_v7_{code_number}/test/{stock_code}.*.csv")
            print(f"      - stockdata_v7/train/{stock_code}.csv")
            print(f"      - 以及其他可能的路径...")
            results.append({
                'stock_code': stock_code,
                'stock_name': stock_name,
        ...
        # 执行回测
        result = run_backtest(stock_code, stock_name, model_path, data_file)
        results.append(result)
    
    # 保存结果
    timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
    result_file = f"backtest_three_stocks_{timestamp}.json"
    ...
    print(f"\n{'股票名称':<12} {'股票代码':<15} {'总收益率':<12} {'年化收益率':<12} {'最大回撤':<12} {'夏普比率':<12} {'状态':<10}")
    print("-" * 90)
    
    for result in results:
        if result['status'] == 'success':
            print(f"{result['stock_name']:<12} {result['stock_code']:<15} "
                  f"{result['total_return']:>+10.2f}% {result['annual_return']:>+10.2f}% "
                  f"{result['max_drawdown_pct']:>10.2f}% {result['sharpe_ratio']:>10.4f} "
                  f"{'✅ 成功':<10}")
        else:
            print(f"{result['stock_name']:<12} {result['stock_code']:<15} "
                  f"{'N/A':<12} {'N/A':<12} {'N/A':<12} {'N/A':<12} "
                  f"{'❌ 失败':<10} ({result.get('error', '未知错误')})")
