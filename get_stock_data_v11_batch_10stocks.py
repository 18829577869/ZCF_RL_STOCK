# get_stock_data_v11_batch_10stocks.py - V11批量10只股票数据获取
# -*- coding: utf-8 -*-
"""
V11 批量10只股票数据获取：
1. 获取用户指定的10只股票数据
...
stocks = [
    ...
    {"code": "sz.000893", "name": "亚钾国际", "start_date": "1998-11-27", 
...
for i, stock in enumerate(stocks, 1):
    print(f"  {i}. {stock['code']} {stock['name']} - {stock['category']} - {stock['business']}")
...
for stock in stocks:
    ...
    try:
        ...
        # 保存到专用目录
        os.makedirs('stockdata_v7_batch_10stocks/train', exist_ok=True)
        os.makedirs('stockdata_v7_batch_10stocks/test', exist_ok=True)
        
        train_file = f'stockdata_v7_batch_10stocks/train/{code}.{name}.csv'
        test_file = f'stockdata_v7_batch_10stocks/test/{code}.{name}.csv'
...
if success_count > 0:
    ...
    metadata_df.to_csv('stockdata_v7_batch_10stocks/metadata_v7_batch_10stocks.csv', index=False, encoding='utf-8-sig')
    print(f"\n元数据已保存: stockdata_v7_batch_10stocks/metadata_v7_batch_10stocks.csv")
    
    print("\n[完成] 可以开始训练：")
    print("  python train_v11_batch_10stocks.py")
else:
    print("\n[失败] 没有成功下载任何数据")
