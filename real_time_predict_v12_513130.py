"""
V12 实时预测系统 - Transformer优化版（恒生科技ETF513130专用）
...
STOCK_CODE = 'sh.513130'  # 股票代码（恒生科技ETF）
...
print("\n" + "=" * 70)
print("V12 实时预测系统 - Transformer优化版（恒生科技ETF513130专用版）")
print("=" * 70)
print("📌 整合功能:")
...
if PPO_AVAILABLE:
    try:
        ...
        possible_models = [
            "ppo_stock_v7_513130.zip",  # 513130专用模型（优先）
            "models_v7_513130/best/best_model.zip",  # 513130最佳模型（优先）
        ...
        if loaded_model_path:
            ...
            if "513130" in MODEL_PATH:
                print(f"✅ PPO模型加载成功: {MODEL_PATH} (中宠股份513130专用模型)")
            else:
    ...
    try:
        ...
        if price_suggestions:
            ...
            if shares_held > 0:
                ...
                # 参考回测结果：恒生科技ETF513130回测表现（+25.67%收益率，夏普比率2.20）
                # 获取预测价格和波动率
                ...
                # 回测结果参考值（基于训练时的回测表现）
                # 恒生科技ETF513130回测：+25.67%收益率，夏普比率2.20，胜率5.03%
                backtest_return_pct = 25.67  # 回测收益率
                backtest_sharpe = 2.20  # 回测夏普比率
                ...
                if zero_price:
                    ...
                    if shares_sold_all > 0:
                        ...
                        if profit_loss_all is not None:
                            profit_loss_sign = "📈" if profit_loss_all >= 0 else "📉"
                            print(f"         🔥 0%仓位 (全卖): {zero_price:6.2f}元 | 当前{current_price:.2f}元 (差异{price_diff:+.2f}元, {price_diff_pct:+.2f}%) | 卖出全部{int(shares_sold_all):4d}股 → 保留 0股 | {profit_loss_sign}盈亏{profit_loss_all:+.2f}元(基于成本价{current_cost_price:.4f}元计算)")
                        else:
