# get_stock_data_v11_603267.py - V11鸿远电子603267专用版
# -*- coding: utf-8 -*-
"""
V11 鸿远电子603267专用版：
1. 专门针对鸿远电子603267进行数据获取和训练
2. 包含鸿远电子603267及其相关股票，确保更好的针对性
3. 初始资金2万元（匹配实盘操作）
4. 数据用于V11全功能集成版训练
"""
...
# V11 鸿远电子603267专用股票列表
# 包含鸿远电子603267及其相关股票，确保更好的针对性
stocks = [
    # === 核心标的：鸿远电子603267 ===
    {"code": "sh.603267", "name": "鸿远电子", "start_date": "2019-05-15", 
     "category": "电子元器件", "volatility": "中", "style": "平衡", "priority": "核心"},
    ...
    {"code": "sz.002241", "name": "歌尔股份", "start_date": "2008-05-22", 
     "category": "电子元器件", "volatility": "高", "style": "激进", "priority": "相关"},
...
print("="*70)
print("V11 鸿远电子603267专用版 - 数据获取")
print("="*70)
print(f"总共 {len(stocks)} 只标的")
print(f"  核心标的: 鸿远电子603267")
...
for stock in stocks:
    ...
    try:
        ...
        if len(train_data) < 100:
            ...
            fail_count += 1
            if code == "sh.603267":
                print(f"  [严重] 鸿远电子603267训练数据不足！")
            continue
        
        # 保存到专用目录（V11可以使用V7的数据格式）
        os.makedirs('stockdata_v7_603267/train', exist_ok=True)
        os.makedirs('stockdata_v7_603267/test', exist_ok=True)
        
        train_file = f'stockdata_v7_603267/train/{code}.{name}.csv'
        test_file = f'stockdata_v7_603267/test/{code}.{name}.csv'
        ...
        if code == "sh.603267":
            hongyuan_dianzi_success = True
            print(f"  [核心] ✅ 鸿远电子603267数据获取成功！")
...
if hongyuan_dianzi_success:
    print(f"\n[核心] ✅ 鸿远电子603267数据获取成功！")
else:
    print(f"\n[严重] ❌ 鸿远电子603267数据获取失败！请检查！")
...
if success_count > 0:
    ...
    metadata_df.to_csv('stockdata_v7_603267/metadata_v7_603267.csv', index=False, encoding='utf-8-sig')
    print(f"\n元数据已保存: stockdata_v7_603267/metadata_v7_603267.csv")
    
    print("\n[完成] 可以开始训练：")
    print("  python train_v11_603267.py")
    print("\n[说明] 本版本专门针对鸿远电子603267优化，初始资金2万元")
