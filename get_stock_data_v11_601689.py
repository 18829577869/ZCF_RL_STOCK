# get_stock_data_v11_601689.py - V11拓普集团601689专用版
# -*- coding: utf-8 -*-
"""
V11 拓普集团601689专用版：
1. 专门针对拓普集团601689进行数据获取和训练
2. 包含拓普集团601689及其相关股票，确保更好的针对性
3. 初始资金2万元（匹配实盘操作）
4. 数据用于V11全功能集成版训练
"""
...
# V11 拓普集团601689专用股票列表
# 包含拓普集团601689及其相关股票，确保更好的针对性
stocks = [
    # === 核心标的：拓普集团601689 ===
    {"code": "sh.601689", "name": "拓普集团", "start_date": "2015-03-19", 
     "category": "汽车零部件", "volatility": "中", "style": "平衡", "priority": "核心"},
    
    # === 汽车零部件板块（相关股票）===
    {"code": "sz.002050", "name": "三花智控", "start_date": "2005-06-07", 
     "category": "汽车零部件", "volatility": "中", "style": "平衡", "priority": "相关"},
    {"code": "sz.000700", "name": "模塑科技", "start_date": "1997-02-28", 
     "category": "汽车零部件", "volatility": "中", "style": "平衡", "priority": "相关"},
    {"code": "sz.002920", "name": "德赛西威", "start_date": "2017-12-26", 
     "category": "汽车零部件", "volatility": "中", "style": "平衡", "priority": "相关"},
    {"code": "sz.300750", "name": "宁德时代", "start_date": "2018-06-11", 
     "category": "新能源汽车", "volatility": "中", "style": "平衡", "priority": "相关"},
    
    # === 新能源汽车板块（相关股票）===
    {"code": "sz.002594", "name": "比亚迪", "start_date": "2011-06-30", 
     "category": "新能源汽车", "volatility": "中", "style": "平衡", "priority": "相关"},
    {"code": "sz.002466", "name": "天齐锂业", "start_date": "2010-08-31", 
     "category": "新能源汽车", "volatility": "高", "style": "周期", "priority": "相关"},
...
print("="*70)
print("V11 拓普集团601689专用版 - 数据获取")
print("="*70)
print(f"总共 {len(stocks)} 只标的")
print(f"  核心标的: 拓普集团601689")
...
for stock in stocks:
    ...
    try:
        ...
        if len(train_data) < 100:
            ...
            fail_count += 1
            if code == "sh.601689":
                print(f"  [严重] 拓普集团601689训练数据不足！")
            continue
        
        # 保存到专用目录（V11可以使用V7的数据格式）
        os.makedirs('stockdata_v7_601689/train', exist_ok=True)
        os.makedirs('stockdata_v7_601689/test', exist_ok=True)
        
        train_file = f'stockdata_v7_601689/train/{code}.{name}.csv'
        test_file = f'stockdata_v7_601689/test/{code}.{name}.csv'
        ...
        if code == "sh.601689":
            tuopu_jituan_success = True
            print(f"  [核心] ✅ 拓普集团601689数据获取成功！")
...
if tuopu_jituan_success:
    print(f"\n[核心] ✅ 拓普集团601689数据获取成功！")
else:
    print(f"\n[严重] ❌ 拓普集团601689数据获取失败！请检查！")
...
if success_count > 0:
    ...
    metadata_df.to_csv('stockdata_v7_601689/metadata_v7_601689.csv', index=False, encoding='utf-8-sig')
    print(f"\n元数据已保存: stockdata_v7_601689/metadata_v7_601689.csv")
    
    print("\n[完成] 可以开始训练：")
    print("  python train_v11_601689.py")
    print("\n[说明] 本版本专门针对拓普集团601689优化，初始资金2万元")
