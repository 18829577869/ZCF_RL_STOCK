"""
V16 实时预测系统 - DeepSeek 轮次复盘增强版（高澜股份300499专用）
...
# 基础配置
MODEL_PATH = "ppo_stock_v7_300499.zip"  # V12使用通用PPO模型，也可以使用专用模型
# 支持通过环境变量或命令行参数指定股票代码
STOCK_CODE = os.getenv('STOCK_CODE', 'sz.300499')  # 股票代码（默认：高澜股份300499）
...
CANDIDATE_MODELS = [  # 候选模型列表（按优先级排序）
    {
        'name': '300499模型（最佳）',
        'paths': [
            'ppo_stock_v7_300499.zip',
            'models_v7_300499/best/best_model.zip'
        ],
        'description': '高澜股份300499模型（最佳匹配）'
    },
    {
        'name': '通用模型',
        'paths': [
...
print("\n" + "=" * 70)
print("V16 实时预测系统 - 趋势/震荡双策略 + DeepSeek复盘 + StockAPI（高澜股份300499专用）")
print("=" * 70)
print("📌 整合功能:")
...
def calculate_v7_price_suggestions(current_price, ppo_action, historical_prices=None):
    ...
    base_desc_suffix = f"（按价格带测算，当前采用价格对应的合理仓位约为 {current_position_pct:.0f}%）"
