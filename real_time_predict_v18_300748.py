"""
-V18 实时预测系统 - 日K线均线信息版（金力永磁300748专用）
...
# 基础配置
MODEL_PATH = "ppo_stock_v7_300748.zip"  # V11使用金力永磁专用PPO模型
# 支持通过环境变量或命令行参数指定股票代码
STOCK_CODE = os.getenv('STOCK_CODE', 'sz.300748')  # 股票代码（默认：金力永磁300748）
...
CANDIDATE_MODELS = [  # 候选模型列表（按优先级排序）
    {
        ...
        'description': '金力永磁300748专用模型（最佳匹配）'
    },
    {
        'name': '000831模型（相关）',
        'paths': [
        ...
        'description': '中国稀土000831模型（稀土板块相关）'
    },
    {
        'name': '通用模型',
        'paths': [
...
def get_daily_kline_df(stock_code, days=250):
    ...
    try:
        ...
        possible_csv_paths = [
            ...
            f'stockdata_v7_002837/test/{stock_code}.{stock_name}.csv',  # 002837模型目录
