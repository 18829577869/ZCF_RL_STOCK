...
- 包含所有股票的完整版本（batch_predict_v18_1.py）
- 每个股票预测时显示V18新增的日K线均线、RSRS指标和筛选条件

V19新增功能：
- 两融余额监测：自动监测两融余额占股市总市值的比例
  * 3.3%警告线：当占比达到3.3%时，显示风险警告提示
  * 3.8%警戒线：当占比达到3.8%时，显示严重风险警告提示
  * 提供详细的风险说明和投资建议
- 数据查找优先级优化：修复了历史数据查找逻辑
  * 优先级1：股票自己的目录（stockdata_v7_{stock_code}/）
  * 优先级2：模型专用目录（stockdata_v7_{model_code}/）
  * 优先级3：相关模型目录（板块内互相备用）
  * 优先级4：通用目录（stockdata_v7/）
- 实时价格获取优化：增强了调试输出，便于诊断问题

V20新增功能：
- 牛市牛股训练：针对牛市市场环境优化模型
  * 使用历史牛市牛股数据进行模型训练，提高模型在牛市中的表现
  * 包含2007年、2015年、2019-2021年等历史牛市的牛股数据
  * 数据目录：stockdata_v20_bull_market/
  * 训练数据优先使用牛市牛股数据，提高模型在牛市中的预测准确性
- 数据查找优先级优化（V20）：
  * 优先级0：牛市牛股数据目录（stockdata_v20_bull_market/）- V20新增
  * 优先级1：股票自己的目录（stockdata_v7_{stock_code}/）
  * 优先级2：模型专用目录（stockdata_v7_{model_code}/）
  * 优先级3：相关模型目录（板块内互相备用）
  * 优先级4：通用目录（stockdata_v7/）

设计理念：多模型协同工作，智能融合决策，自动选择最优模型，风险控制优先，科学资金管理，丰富数据源，牛市优化
"""
...
STOCK_METRICS_DATA = {
    ...
    'sz.002025': {'total_return': 41.87, 'max_drawdown': 7.80, 'sharpe_ratio': 2.24},  # 航天电器 - 使用航天发展模型，夏普2.24，收益率+41.87%
    # 以下股票不在排名列表中，保留原配置（模型保留，但不进行预测）
    'sh.603698': {'total_return': 51.53, 'max_drawdown': 6.69, 'sharpe_ratio': 2.76},  # 航天工程 - 优秀表现
    'sh.600343': {'total_return': 190.73, 'max_drawdown': 19.70, 'sharpe_ratio': 2.82},  # 航天动力 - 表现卓越
    'sz.000547': {'total_return': 196.60, 'max_drawdown': 18.61, 'sharpe_ratio': 2.97},  # 航天发展 - 🏆 核心训练标的，夏普2.97，收益率+196.60%
    ...
    'sh.600118': {'total_return': 77.94, 'max_drawdown': 16.34, 'sharpe_ratio': 2.04},  # 中国卫星 - 使用航天发展模型，夏普2.04，收益率+77.94%
    ...
    'sh.600879': {'total_return': 87.15, 'max_drawdown': 16.85, 'sharpe_ratio': 2.05},  # 航天电子 - 使用航天发展模型，夏普2.05，收益率+87.15%
}
...
def load_historical_data_for_prediction(stock_code, min_length=126):
    ...
    Returns:
        tuple: (pd.DataFrame, dict) 或 None
            - pd.DataFrame: 包含 time, open, high, low, close, volume 等列的DataFrame
            - dict: 数据来源信息，包含 'data_dir', 'model_code', 'is_own_data' 等字段
            失败返回None
    """
    try:
        # 尝试从多个目录加载历史数据（按优先级排序）
        possible_dirs = []
        ...
        # V18优化：根据STOCK_LIST中的模型配置，查找对应的数据目录
        # 优先级：股票自己的目录 > 模型专用目录 > 通用目录
        if not is_index and 'STOCK_LIST' in globals():
            stock_name = get_stock_name(stock_code)
            # 提取股票代码数字部分（如 "sh.600392" -> "600392"）
            stock_code_num = stock_code.split('.')[1] if '.' in stock_code else stock_code
            ...
            if stock_config:
                model_path = stock_config.get('model', '')
                # 从模型路径中提取股票代码（如 "ppo_stock_v7_300748.zip" -> "300748"）
                if 'ppo_stock_v7_' in model_path:
                    model_code = model_path.replace('ppo_stock_v7_', '').replace('.zip', '').replace('_', '').replace('steps', '')
                    # 如果是1A0001，提取出来
                    if '1A0001' in model_code:
                        model_code = '1A0001'
                    
                    # 优先级1：股票自己的数据目录
                    possible_dirs.extend([
                        f"stockdata_v7_{stock_code_num}/test",
                        f"stockdata_v7_{stock_code_num}/train"
                    ])
                    
                    # 优先级2：模型专用目录（如果模型不是自身模型）
                    if model_code and model_code != stock_code_num:
                        possible_dirs.extend([
                            f"stockdata_v7_{model_code}/test",
                            f"stockdata_v7_{model_code}/train"
                        ])
                    
                    # 优先级3：相关模型目录（根据板块和模型关系，添加备用目录）
                    # 稀土/有色金属板块：金力永磁(300748)和中国稀土(000831)模型可以互相作为备用
                    if model_code == '300748':  # 使用金力永磁模型
                        possible_dirs.extend([
                            "stockdata_v7_000831/test",  # 中国稀土模型目录（备用）
                            "stockdata_v7_000831/train"
                        ])
                    elif model_code == '000831':  # 使用中国稀土模型
                        possible_dirs.extend([
                            "stockdata_v7_300748/test",  # 金力永磁模型目录（备用）
                            "stockdata_v7_300748/train"
                        ])
                    
                    # 注意：其他板块的模型目录已经在优先级2中添加，这里不需要重复
        
        # V20新增：优先级0：牛市牛股数据目录（最高优先级）
        possible_dirs.insert(0, "stockdata_v20_bull_market/test")
        possible_dirs.insert(1, "stockdata_v20_bull_market/train")
                ...
                if file.endswith('.csv'):
                    ...
                    if file_match:
                        ...
                        try:
                            ...
                            # 从目录名提取模型代码（如 "stockdata_v7_300748" -> "300748"）
                            if f'stockdata_v7_{stock_code_num}' in data_dir:
                                # 使用自己的数据
                                data_source_info['is_own_data'] = True
                                data_source_info['model_code'] = stock_code_num
                                data_source_info['model_name'] = get_stock_name(stock_code)
                                data_source_info['source_type'] = '自身数据'
                            else:
                                # 使用其他模型的数据
                                data_source_info['is_own_data'] = False
                                # 从目录名提取模型代码
                                for part in data_dir.split('/'):
                                    if part.startswith('stockdata_v7_') and len(part) > len('stockdata_v7_'):
                                        model_code = part.replace('stockdata_v7_', '')
                                        if model_code and model_code != stock_code_num:
                                            data_source_info['model_code'] = model_code
                                            # 尝试获取模型对应的股票名称
                                            try:
                                                if model_code == '1A0001':
                                                    data_source_info['model_name'] = '上证指数'
                                                else:
                                                    # 构造股票代码
                                                    if model_code.startswith('6'):
                                                        model_stock_code = f"sh.{model_code}"
                                                    else:
                                                        model_stock_code = f"sz.{model_code}"
                                                    data_source_info['model_name'] = get_stock_name(model_stock_code)
                                            except:
                                                data_source_info['model_name'] = f"{model_code}模型"
                                            data_source_info['source_type'] = f'{data_source_info.get("model_name", model_code)}模型数据'
                                            break
                            
                            print(f"   ✅ 已从历史数据文件加载: {file} ({len(result_df)} 条数据)")
                            if not data_source_info.get('is_own_data', True):
                                print(f"   📊 数据来源: {data_source_info.get('source_type', '其他模型数据')} (目录: {data_dir})")
                            
                            return result_df, data_source_info
...
def load_historical_data_v18_style(stock_code, min_length=126):
    """
    V19新增：V18风格的历史数据加载函数（只查找通用目录，用于对比）
    从本地历史数据文件加载数据用于预测（当日线数据不足时使用）
    将日线数据转换为预测所需的格式（模拟5分钟数据）
    ...
    Returns:
        tuple: (pd.DataFrame, dict) 或 None
            - pd.DataFrame: 包含 time, open, high, low, close, volume 等列的DataFrame
            - dict: 数据来源信息，包含 'data_dir', 'is_own_data' 等字段
            失败返回None
    """
    try:
        # V18风格：只查找通用目录（不区分模型）
                    ...
                    if file_match:
                        ...
                        try:
                            ...
                            data_source_info = {
                                ...
                                'file_path': file_path,
                                'data_count': len(result_df),
                                'is_own_data': True,  # V18风格只查找通用目录，视为自身数据
                                'source_type': 'V18风格（通用目录）',
                                'method': 'V18'  # 标记为V18风格
                            }
                            
                            print(f"   ✅ [V18风格] 已从历史数据文件加载: {file} ({len(result_df)} 条数据)")
                            print(f"   📊 [V18风格] 数据来源: {data_source_info['source_type']} (目录: {data_dir})")
...
def display_alternative_models_comparison(stock_code, stock_name, stock_info):
    """
    V19新增：显示使用其他模型的历史数据对比
    
    Args:
        stock_code: 当前股票代码
        stock_name: 当前股票名称
        stock_info: 当前股票的配置信息（包含model, sharpe, return, drawdown等）
    """
    if not stock_info:
        return
    
    current_model = stock_info.get('model', '')
    current_sharpe = stock_info.get('sharpe')
    current_return = stock_info.get('return')
    current_drawdown = stock_info.get('drawdown')
    
    # 从当前模型路径中提取模型代码
    current_model_code = None
    if current_model:
        if 'ppo_stock_v7_' in current_model:
            current_model_code = current_model.replace('ppo_stock_v7_', '').replace('.zip', '').replace('_', '').replace('steps', '')
            if '1A0001' in current_model_code:
                current_model_code = '1A0001'
        elif 'ppo_stock_v11_custom' in current_model:
            current_model_code = 'v11_custom'  # V11自定义模型
    
    # 收集使用相同模型的其他股票（用于对比）
    same_model_stocks = []
    # 收集使用相关模型的其他股票（同一板块或相关模型）
    related_model_stocks = []
    ...
    for stock in STOCK_LIST:
        ...
        # 提取模型代码
        stock_model_code = None
        if 'ppo_stock_v7_' in stock_model:
            stock_model_code = stock_model.replace('ppo_stock_v7_', '').replace('.zip', '').replace('_', '').replace('steps', '')
            if '1A0001' in stock_model_code:
                stock_model_code = '1A0001'
        elif 'ppo_stock_v11_custom' in stock_model:
            stock_model_code = 'v11_custom'  # V11自定义模型
        
        # 判断是否使用相同模型
        if stock_model_code and current_model_code and stock_model_code == current_model_code:
            same_model_stocks.append(stock)
        # 判断是否使用相关模型（根据板块和模型关系）
        elif stock_model_code and current_model_code:
            # 稀土/有色金属板块：金力永磁(300748)和中国稀土(000831)模型相关
            if (current_model_code == '300748' and stock_model_code == '000831') or \
               (current_model_code == '000831' and stock_model_code == '300748'):
                related_model_stocks.append(stock)
            # 其他板块：如果模型代码相似（前3位相同），也视为相关
            elif len(current_model_code) >= 3 and len(stock_model_code) >= 3:
                if current_model_code[:3] == stock_model_code[:3]:
                    related_model_stocks.append(stock)
    
    # 如果有其他使用相同模型或相关模型的股票，显示对比
    if same_model_stocks or related_model_stocks:
        print(f"\n   📊 V19其他模型历史数据对比 [{stock_name}({stock_code})]:")
        print(f"   {'-'*68}")
        
        # 显示当前使用的模型
        if current_model_code == 'v11_custom':
            model_name = "V11自定义模型"
        elif current_model_code and stock_code.split('.')[-1] == current_model_code:
            model_name = "专用模型"
        elif current_model_code:
            model_name = f"{current_model_code}模型"
        else:
            model_name = "当前模型"
        print(f"   🎯 当前使用模型: {model_name} ({current_model})")
        if current_sharpe is not None and current_return is not None and current_drawdown is not None:
            print(f"      夏普比率: {current_sharpe:.2f} | 总收益率: {current_return:+.2f}% | 最大回撤: {current_drawdown:.2f}%")
        print(f"   {'-'*68}")
        
        # 显示使用相同模型的其他股票（最多显示3个）
        if same_model_stocks:
            print(f"   📈 使用相同模型的其他股票（历史回测数据）:")
            displayed_count = 0
            for stock in same_model_stocks[:3]:  # 最多显示3个
                alt_name = stock.get('name', stock.get('code'))
                alt_sharpe = stock.get('sharpe')
                alt_return = stock.get('return')
                alt_drawdown = stock.get('drawdown')
                alt_strategy = stock.get('strategy', '')
                
                if alt_sharpe is not None and alt_return is not None and alt_drawdown is not None:
                    print(f"      • {alt_name}({stock.get('code')}): "
                          f"夏普{alt_sharpe:.2f} | 收益{alt_return:+.2f}% | 回撤{alt_drawdown:.2f}% {alt_strategy}")
                    displayed_count += 1
            
            if len(same_model_stocks) > 3:
                print(f"      ... 还有 {len(same_model_stocks) - 3} 个股票使用相同模型")
        
        # 显示使用相关模型的其他股票（最多显示2个）
        if related_model_stocks:
            print(f"   🔗 使用相关模型的其他股票（历史回测数据）:")
            displayed_count = 0
            for stock in related_model_stocks[:2]:  # 最多显示2个
                alt_name = stock.get('name', stock.get('code'))
                alt_model = stock.get('model', '')
                alt_model_code = None
                if 'ppo_stock_v7_' in alt_model:
                    alt_model_code = alt_model.replace('ppo_stock_v7_', '').replace('.zip', '').replace('_', '').replace('steps', '')
                    if '1A0001' in alt_model_code:
                        alt_model_code = '1A0001'
                elif 'ppo_stock_v11_custom' in alt_model:
                    alt_model_code = 'v11_custom'  # V11自定义模型
                
                if alt_model_code == 'v11_custom':
                    alt_model_name = "V11自定义模型"
                elif alt_model_code:
                    alt_model_name = f"{alt_model_code}模型"
                else:
                    alt_model_name = "相关模型"
                ...
                if alt_sharpe is not None and alt_return is not None and alt_drawdown is not None:
                    print(f"      • {alt_name}({stock.get('code')}) [{alt_model_name}]: "
                          f"夏普{alt_sharpe:.2f} | 收益{alt_return:+.2f}% | 回撤{alt_drawdown:.2f}% {alt_strategy}")
                    displayed_count += 1
            
            if len(related_model_stocks) > 2:
                print(f"      ... 还有 {len(related_model_stocks) - 2} 个股票使用相关模型")
        
        print(f"   {'-'*68}")
        print(f"   💡 提示: 以上数据基于历史回测结果（测试集：2025-01-01至今），仅供参考")
    else:
        # 如果没有其他使用相同模型或相关模型的股票，提示用户
        print(f"\n   📊 V19其他模型历史数据对比 [{stock_name}({stock_code})]:")
        print(f"   {'-'*68}")
        print(f"   ℹ️  暂无其他使用相同模型或相关模型的股票历史数据")
        print(f"   💡 提示: 当前股票使用 {current_model}，未找到其他股票的对比数据")
        print(f"   {'-'*68}")
...
STOCK_LIST = [
    ...
    {'code': 'sz.000547', 'name': '航天发展', 'model': 'ppo_stock_v7_000547.zip', 'rank': 1, 'sharpe': 2.97, 'return': 196.60, 'drawdown': 18.61, 'strategy': '🟡 进取型'},  # 🏆 核心训练标的，自身模型，夏普2.97，收益率+196.60%，回撤18.61%
    {'code': 'sh.600343', 'name': '航天动力', 'model': 'ppo_stock_v11_custom.zip', 'rank': 2, 'sharpe': 2.82, 'return': 190.73, 'drawdown': 19.70, 'strategy': '🟡 进取型'},  # V11自定义模型，夏普2.82，收益率+190.73%，回撤19.70%
    {'code': 'sh.603698', 'name': '航天工程', 'model': 'ppo_stock_v7_603698.zip', 'rank': 3, 'sharpe': 2.76, 'return': 51.53, 'drawdown': 6.69, 'strategy': '🔵 稳健型'},  # 核心训练标的，自身模型，夏普2.76，收益率+51.53%，回撤6.69%
    {'code': 'sz.300811', 'name': '铂科新材', 'model': 'ppo_stock_v7_300811.zip', 'rank': 4, 'sharpe': 2.61, 'return': 51.14, 'drawdown': 6.40, 'strategy': '🔵 稳健型'},  # 自身模型，夏普2.61，收益率+51.14%，回撤6.40%
    ...
    {'code': 'sh.600118', 'name': '中国卫星', 'model': 'ppo_stock_v7_000547.zip', 'rank': 9, 'sharpe': 2.04, 'return': 77.94, 'drawdown': 16.34, 'strategy': '🟢 均衡型'},  # 使用航天发展模型，夏普2.04，收益率+77.94%，回撤16.34%（原V11自定义模型夏普2.15，历史回测显示航天发展模型收益更高）
    {'code': 'sz.002025', 'name': '航天电器', 'model': 'ppo_stock_v7_000547.zip', 'rank': 10, 'sharpe': 2.24, 'return': 41.87, 'drawdown': 7.80, 'strategy': '🟢 均衡型'},  # 使用航天发展模型，夏普2.24，收益率+41.87%，回撤7.80%（原上海瀚讯模型夏普2.12，历史回测中航天发展模型夏普与收益均更优）
    ...
    {'code': 'sh.600879', 'name': '航天电子', 'model': 'ppo_stock_v7_000547.zip', 'rank': 13, 'sharpe': 2.05, 'return': 87.15, 'drawdown': 16.85, 'strategy': '🟢 均衡型'},  # 使用航天发展模型，夏普2.05，收益率+87.15%，回撤16.85%（原V11自定义模型夏普1.75，历史回测中航天发展模型夏普与收益大幅领先）
    ...
    # ===== 工程机械板块 =====
    {'code': 'sz.000425', 'name': '徐工机械', 'model': 'ppo_stock_v7_000425.zip', 'rank': 16, 'sharpe': 2.11, 'return': 71.75, 'drawdown': 15.76, 'strategy': '🟢 均衡型'},  # 自身模型，夏普2.11，收益率+71.75%，回撤15.76%
...
CANDIDATE_MODELS = [  # 候选模型列表（包含所有股票的专用模型）
    ...
    {
        'name': '000547模型',
        'paths': ['ppo_stock_v7_000547.zip', 'models_v7_000547/best/best_model.zip'],
        'description': '航天发展000547专用模型 - 🏆 核心训练标的，夏普2.97，收益率+196.60%'
    },
    {
        'name': '603698模型',
        'paths': ['ppo_stock_v7_603698.zip', 'models_v7_603698/best/best_model.zip'],
        'description': '航天工程603698专用模型 - ⭐ 优秀表现（夏普2.76，收益率+51.53%）'
    },
    {
        'name': '002025模型',
            ...
            if spot_df is not None and len(spot_df) > 0:
                ...
                if code_col and price_col:
                    ...
                    if len(stock_row) > 0:
                        ...
                        # 如果找不到股票，输出调试信息
                        if debug:
                            print(f"[实时价格] ⚠️  在实时行情中未找到股票代码: {symbol} (目标: {symbol_digits})")
                else:
                    # 如果找不到必需的列，输出调试信息
                    if debug:
                        print(f"[实时价格] ⚠️  无法找到必需的列: code_col={code_col}, price_col={price_col}")
                        if spot_df is not None:
                            print(f"[实时价格] 可用列: {list(spot_df.columns)}")
        else:
            # 指数跳过方法1，直接使用方法2或3
            if debug:
...
def calculate_position_price_suggestions(current_price, lstm_prediction=None, transformer_prediction=None, 
    ...
    # V19改进：确保价格区间百分比不超过合理范围（10%）
    # 如果因为动态调整导致区间过大，重新调整价格区间
    max_interval_pct = 10.0  # 最大价格区间10%
    if interval_pct > max_interval_pct:
        # 如果实际价格区间超过10%，重新调整，以当前价格为中心
        center_price = (min_price + max_price) / 2
        # 优先以当前价格为中心（更实用）
        if current_price >= min_price and current_price <= max_price:
            center_price = current_price
        
        adjusted_interval_size = current_price * max_interval_pct / 100
        min_price = center_price - adjusted_interval_size / 2
        max_price = center_price + adjusted_interval_size / 2
        ...
        interval_pct = (price_interval / current_price * 100) if current_price > 0 else 0
        # 再次限制，确保不超过10%
        interval_pct = min(interval_pct, max_interval_pct)
...
def calculate_v17_explicit_price_suggestions(current_price, lstm_prediction=None, transformer_prediction=None, 
    ...
    # V19新增：限制第二天预测涨幅在合理范围内（A股涨跌停限制为±10%）
    if next_day_prediction is not None and current_price > 0:
        next_day_change_pct = ((next_day_prediction - current_price) / current_price * 100)
        
        # 如果预测涨幅超过±10%，进行限制
        if abs(next_day_change_pct) > 10.0:
            # 限制在±10%范围内
            if next_day_change_pct > 10.0:
                next_day_prediction = current_price * 1.10  # 涨停价
            elif next_day_change_pct < -10.0:
                next_day_prediction = current_price * 0.90  # 跌停价
            
            # 重新计算限制后的涨幅
            next_day_change_pct_limited = ((next_day_prediction - current_price) / current_price * 100)
            # 可选：输出警告信息（如果需要调试）
            # print(f"   ⚠️  第二天预测涨幅({next_day_change_pct:.2f}%)超过±10%，已限制为{next_day_change_pct_limited:.2f}%")
    ...
    if next_day_prediction is not None:
        ...
        if historical_prices is not None and len(historical_prices) >= 30:
            try:
                ...
                # 计算历史波动率（用于最小波动阈值）
                volatility = np.std(returns) if len(returns) > 1 else abs(avg_daily_return) if avg_daily_return != 0 else 0.001
                
                # 结合短期预测和历史趋势（权重：短期40%，历史60%，降低短期预测权重）
                combined_daily_return = day_change_pct * 0.4 + avg_daily_return * 0.6
                
                # V17改进：如果短期和历史趋势都接近0，使用历史波动率作为最小基准
                # 确保长期预测不会完全等于当前价格
                min_daily_return_threshold = volatility * 0.3  # 最小波动阈值：历史波动率的30%
                if abs(combined_daily_return) < min_daily_return_threshold:
                    # 使用历史波动率和趋势方向（即使很小）来估算长期变化
                    if combined_daily_return >= 0:
                        combined_daily_return = max(min_daily_return_threshold, combined_daily_return)
                    else:
                        combined_daily_return = min(-min_daily_return_threshold, combined_daily_return)
                ...
                # V17改进：如果短期预测变化太小，至少假设0.05%的日波动（年化约12.5%）
                min_day_change = 0.0005  # 最小日变化率0.05%
                if abs(day_change_pct) < min_day_change:
                    if day_change_pct >= 0:
                        day_change_pct = min_day_change
                    else:
                        day_change_pct = -min_day_change
            ...
            # V17改进：如果短期预测变化太小，至少假设0.05%的日波动
            min_day_change = 0.0005  # 最小日变化率0.05%
            if abs(day_change_pct) < min_day_change:
                if day_change_pct >= 0:
                    day_change_pct = min_day_change
    ...
    if long_term_prediction is not None:
        ...
        # V17改进：确保长期卖出价格始终明显高于当前价格（至少5%），避免等于当前价格
        # 如果长期预测看跌（低于当前价格），长期卖出价格应该基于当前价格计算
        if long_term_prediction < current_price:
            # 长期预测看跌，但给出合理的卖出价格（假设有反弹或波动）
            # 使用当前价格的一定涨幅（至少5%）作为长期卖出价格
            suggested_sell_price_long_term = current_price * 1.05  # 至少比当前价格高5%
        else:
            # 长期预测看涨或持平，确保卖出价格至少比当前价格高5%
            min_sell_price = current_price * 1.05  # 最小卖出价格：比当前价格高5%
            suggested_sell_price_long_term = max(min_sell_price, suggested_sell_price_long_term)
        
        # 确保长期卖出价格合理（不能太高，也不能等于当前价格）
        # 再次检查，确保不会因为四舍五入导致等于当前价格
        if suggested_sell_price_long_term <= current_price * 1.01:  # 如果差异小于1%，强制设置为至少高5%
            suggested_sell_price_long_term = current_price * 1.05
        
        suggested_sell_price_long_term = round(suggested_sell_price_long_term, 2)
        
        # 最终检查：四舍五入后如果等于当前价格，强制增加
        if suggested_sell_price_long_term <= current_price:
            suggested_sell_price_long_term = round(current_price * 1.05, 2)
    else:
        # 没有长期预测，使用当前价格的合理涨幅
...
def get_stock_sector(stock_code):
    ...
    # 示例：根据股票代码前缀或名称判断
    sector_map = {
        '军工': ['300900', '000547', '002151', '002025', '600343', '603698', '300762'],
        '航空航天': ['300900', '000547', '002025', '600343', '603698'],
...
# 最优模型列表数据（根据STOCK_LIST自动生成，按夏普比率排序）
# 从模型路径提取模型名称的函数
def format_model_name(model_path, stock_code=None):
    """根据模型路径格式化模型名称"""
    if model_path == 'ppo_stock_v11_custom.zip':
        return 'V11自定义模型'
    elif model_path == 'ppo_stock_v7.zip':
        return '通用模型'
    else:
        # 从路径中提取股票代码（如 ppo_stock_v7_002837.zip -> 002837）
        model_code = model_path.replace('ppo_stock_v7_', '').replace('ppo_stock_v11_', '').replace('.zip', '')
        if model_code:
            # 根据股票代码查找股票名称
            model_stock_code = None
            if model_code.startswith('sz.'):
                model_stock_code = model_code
            elif model_code.startswith('sh.'):
                model_stock_code = model_code
            else:
                # 假设是6位数字代码，需要判断是sz还是sh
                if len(model_code) == 6:
                    if model_code.startswith('0') or model_code.startswith('3'):
                        model_stock_code = f'sz.{model_code}'
                    elif model_code.startswith('6'):
                        model_stock_code = f'sh.{model_code}'
                    else:
                        model_stock_code = f'sz.{model_code}'  # 默认sz
            
            if model_stock_code:
                model_name = get_stock_name(model_stock_code)
                if model_name:
                    return f'{model_name}({model_code})模型'
        
        return f'{model_path}模型'

# 根据STOCK_LIST生成optimal_models_info
optimal_models_info = []
for stock in STOCK_LIST:
    code = stock['code']
    name = stock.get('name') or get_stock_name(code)
    model_path = stock.get('model', 'ppo_stock_v7.zip')
    rank = stock.get('rank', 0)
    sharpe = stock.get('sharpe', 0)
    return_pct = stock.get('return', 0)
    drawdown = stock.get('drawdown', 0)
    strategy = stock.get('strategy', '🟢 均衡型')
    
    # 格式化模型名称
    model_name = format_model_name(model_path, code)
    
    optimal_models_info.append({
        'rank': rank,
        'code': code,
        'name': name,
        'model': model_name,
...
# V19新增：初始化预测结果汇总列表（用于汇总第二天预测涨幅≥1%和≥2%的股票）
all_predictions_summary = []  # 存储所有股票的预测结果
# 清空或创建日志文件，写入头部信息
with open(log_file, 'w', encoding='utf-8') as f:
    f.write("=" * 70 + "\n")
    f.write(f"V19批量预测日志 - {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("=" * 70 + "\n")
    f.write(f"股票数量: {len(STOCK_LIST)}\n")
    
    # 写入最优模型配置列表
    f.write("\n" + "=" * 120 + "\n")
    f.write(" " * 30 + "📊 V19 最优模型配置列表\n")
    f.write("=" * 120 + "\n")
...
# ==================== 两融余额监测功能 ====================
def get_margin_balance_ratio():
    """
    获取两融余额占中国股市总市值的比例
    
    Returns:
        tuple: (两融余额(亿元), 股市总市值(亿元), 占比(%), 是否达到3.8%阈值)
        如果获取失败，返回 (None, None, None, False)
    """
    try:
        import akshare as ak
        ...
        try:
            # 方法1：尝试获取沪深两市融资融券汇总数据
            # 使用东方财富的融资融券数据接口（通常是汇总的）
            try:
                df_margin = ak.stock_margin_underlying_info_em()
                if df_margin is not None and len(df_margin) > 0:
                    # 获取最新的两融余额
                    latest_row = df_margin.iloc[-1]
                    # 根据不同列名尝试获取
                    if '融资余额' in df_margin.columns and '融券余额' in df_margin.columns:
                        margin_balance_buy = float(latest_row['融资余额']) if pd.notna(latest_row['融资余额']) else 0
                        margin_balance_sell = float(latest_row['融券余额']) if pd.notna(latest_row['融券余额']) else 0
                        margin_balance = margin_balance_buy + margin_balance_sell
                    elif '两融余额' in df_margin.columns:
                        margin_balance = float(latest_row['两融余额']) if pd.notna(latest_row['两融余额']) else 0
            except:
                pass
            
            # 方法2：如果方法1失败，尝试分别获取上交所和深交所的数据
            if margin_balance is None:
                try:
                    # 尝试获取上交所融资融券数据
                    df_sse = ak.stock_margin_underlying_info_sse()
                    margin_sse = 0
                    if df_sse is not None and len(df_sse) > 0:
                        latest_sse = df_sse.iloc[-1]
                        if '融资余额' in df_sse.columns and '融券余额' in df_sse.columns:
                            margin_sse = float(latest_sse['融资余额'] or 0) + float(latest_sse['融券余额'] or 0)
                    ...
                    if df_szse is not None and len(df_szse) > 0:
                        ...
                        if '融资余额' in df_szse.columns and '融券余额' in df_szse.columns:
                            margin_szse = float(latest_szse['融资余额'] or 0) + float(latest_szse['融券余额'] or 0)
                    
                    if margin_sse > 0 or margin_szse > 0:
                        margin_balance = margin_sse + margin_szse
                except Exception as e:
                    pass
            
            # 方法3：如果以上都失败，尝试使用市场整体数据接口
            if margin_balance is None:
                try:
                    # 尝试使用市场整体融资融券数据（如果有相关接口）
        ...
        try:
            ...
            if df_all is not None and len(df_all) > 0:
                ...
                if '总市值' in df_all.columns:
                    # 总市值已经是元，转换为亿元
                    total_market_cap = df_all['总市值'].astype(float).sum() / 100000000
                elif '市值' in df_all.columns:
                    total_market_cap = df_all['市值'].astype(float).sum() / 100000000
                elif '流通市值' in df_all.columns:
                    # 如果没有总市值列，使用流通市值估算（通常总市值略大于流通市值）
                    # 流通市值也是元，转换为亿元，然后乘以1.2估算总市值
                    total_market_cap = df_all['流通市值'].astype(float).sum() * 1.2 / 100000000
        except Exception as e:
            pass  # 静默失败，稍后会显示警告
        
        # 3. 计算占比
        if margin_balance is not None and margin_balance > 0 and total_market_cap is not None and total_market_cap > 0:
            ratio = (margin_balance / total_market_cap) * 100
            # 检查是否达到3.3%和3.8%阈值
            is_warning_reached = ratio >= 3.3  # 3.3%警告线
            is_threshold_reached = ratio >= 3.8  # 3.8%警戒线
            return margin_balance, total_market_cap, ratio, is_threshold_reached, is_warning_reached
        else:
...
# 批量预测前：检查两融余额占比
print("\n" + "=" * 70)
print("📊 市场风险监测：两融余额占比检查")
print("=" * 70)
margin_balance_val, total_market_cap_val, ratio_val, is_threshold_reached, is_warning_reached = get_margin_balance_ratio()

if margin_balance_val is not None and total_market_cap_val is not None and ratio_val is not None:
    print(f"   两融余额: {margin_balance_val:,.2f} 亿元")
    print(f"   股市总市值: {total_market_cap_val:,.2f} 亿元")
    print(f"   占比: {ratio_val:.2f}%")
    ...
    if is_threshold_reached:
        ...
        print("🚨 【重要风险提示】🚨")
        print("⚠️ " * 35)
        print(f"   两融余额占股市总市值的比例已达到 {ratio_val:.2f}%，超过3.8%的警戒线！")
        print("   💡 历史经验表明，当两融余额占比达到3.8%以上时，市场可能面临以下风险：")
        print("      1. 杠杆资金集中，市场波动可能加大")
        print("      2. 流动性风险增加，可能出现踩踏式下跌")
        print("      3. 投资者情绪可能过度乐观，需警惕回调风险")
        print("   📊 建议：")
        print("      - 适当降低仓位，控制风险")
        print("      - 关注市场情绪变化，设置止损")
        print("      - 避免追高，等待更好的入场时机")
        print("⚠️ " * 35 + "\n")
    elif is_warning_reached:
        # 3.3%到3.8%之间：警告提示
        print("\n" + "⚠️ " * 35)
        print("⚠️  【风险提示】⚠️")
        print("⚠️ " * 35)
        print(f"   两融余额占股市总市值的比例已达到 {ratio_val:.2f}%，超过3.3%的警告线！")
        print("   💡 当两融余额占比达到3.3%以上时，市场杠杆水平已处于较高位置：")
        print("      1. 杠杆资金较为集中，需要关注市场波动")
        print("      2. 投资者情绪可能较为乐观，需保持谨慎")
        print("      3. 建议密切关注市场变化，做好风险控制")
        print("   📊 建议：")
        print("      - 控制仓位，不要盲目追高")
        print("      - 设置止损，保护已有收益")
        print("      - 关注市场情绪变化，及时调整策略")
        print("⚠️ " * 35 + "\n")
    else:
        print(f"   ✅ 当前占比 {ratio_val:.2f}% 低于3.3%警告线，风险可控")
else:
    print("   ⚠️  无法获取两融余额数据，跳过风险监测")
print("=" * 70 + "\n")
    ...
    for stock_info in STOCK_LIST:
        ...
        with OutputCapture() as output_capture:
            ...
            try:
                ...
                stock_name = get_stock_name(STOCK_CODE)
                print(f"\n{'='*70}")
                # V19新增：添加锚点，用于汇总中的超链接跳转
                # 使用Markdown格式的锚点（在支持Markdown的编辑器中可以点击跳转）
                anchor_id = STOCK_CODE.replace('.', '_').replace(':', '_')  # 将股票代码转换为有效的锚点ID
                print(f"<a name=\"{anchor_id}\"></a>")  # HTML锚点（兼容性更好）
                print(f"<!-- {STOCK_CODE} -->")  # 注释标记，便于查找
                ...
                # V19新增：初始化历史数据来源信息变量
                historical_data_source_info = None
                v18_data_source_info = None
                v18_closes = None
                v18_df = None
                v18_prediction_results = None  # V19新增：V18方式预测结果
                ...
                if len(closes) < 126:
                    ...
                    historical_result = load_historical_data_for_prediction(STOCK_CODE, min_length=126)
                    if historical_result is not None:
                        if isinstance(historical_result, tuple):
                            historical_df, historical_data_source_info = historical_result
                        else:
                            # 兼容旧版本（如果返回的不是tuple）
                            historical_df = historical_result
                            historical_data_source_info = None
                        
                        if historical_df is not None and len(historical_df) >= 126:
                            df = historical_df
                            closes = df['close'].astype(float).values
                            print(f"   ✅ [V19方式] 已从历史数据文件加载数据，当前数据条数: {len(closes)}")
                            data_source_used = "历史数据文件（V19优先级）"
                            
                            # V19新增：同时使用V18风格的方式加载数据（用于对比）
                            # 注意：只有在V19方式成功加载数据后，才尝试V18方式
                            v18_result = load_historical_data_v18_style(STOCK_CODE, min_length=126)
                            if v18_result is not None:
                                if isinstance(v18_result, tuple):
                                    v18_df_temp, v18_data_source_info_temp = v18_result
                                    if v18_df_temp is not None and len(v18_df_temp) >= 126:
                                        v18_closes_temp = v18_df_temp['close'].astype(float).values
                                        # 检查V18方式的数据是否与V19方式的数据不同
                                        if not (historical_data_source_info and v18_data_source_info_temp and 
                                               historical_data_source_info.get('data_dir') == v18_data_source_info_temp.get('data_dir')):
                                            v18_df = v18_df_temp
                                            v18_closes = v18_closes_temp
                                            v18_data_source_info = v18_data_source_info_temp
                                            print(f"   ✅ [V18方式] 已从历史数据文件加载数据，当前数据条数: {len(v18_closes)}")
                                        else:
                                            # 如果数据来源相同，不需要对比
                                            print(f"   💡 [V18方式] 数据来源与V19方式相同，跳过对比预测")
                    else:
                        # 使用多数据源合并功能，用历史数据补齐
                ...
                try:
                    ...
                    # 减少重试次数为1次，启用调试输出以便诊断问题
                    realtime_price = get_current_market_price(STOCK_CODE, max_retries=1, debug=True)
                    if realtime_price and realtime_price > 0:
                        print(f"   ✅ 已从实时行情接口获取价格: {realtime_price:.2f}")
                    else:
                        print(f"   ⚠️  实时行情接口返回的价格无效: {realtime_price}")
                except Exception as e:
                    # 输出错误信息以便诊断
                    print(f"   ⚠️  实时行情获取异常: {e}")
                    import traceback
                    # 只在需要详细调试时输出完整堆栈
                    # traceback.print_exc()  # 注释掉，避免输出过多信息
                ...
                # ========== V19新增：如果同时有V18方式的数据，使用V18方式的数据进行对比预测 ==========
                if v18_closes is not None and len(v18_closes) >= 126 and v18_data_source_info is not None:
                    # 使用V18方式的数据进行预测
                    print(f"\n   {'='*70}")
                    print(f"   📊 V19数据来源对比预测 [{stock_name}({STOCK_CODE})]")
                    print(f"   {'='*70}")
                    print(f"   🔄 使用V18风格数据（通用目录）进行对比预测...")
                    
                    try:
                        # 使用V18方式的数据重新进行预测
                        v18_ppo_action = None
                        v18_transformer_prediction = None
                        v18_lstm_prediction = None
                        
                        # V18方式：PPO预测
                        if ppo_model and current_model_name:
                            try:
                                v18_obs = np.array(v18_closes[-126:], dtype=np.float32)
                                if ENABLE_AUTO_MODEL_SELECTION and len(candidate_ppo_models) > 0:
                                    v18_ppo_action = model_actions.get(current_model_name) if 'model_actions' in locals() else None
                                    if v18_ppo_action is None:
                                        # 如果model_actions中没有，直接使用模型预测
                                        action_v18, _states_v18 = ppo_model.predict(v18_obs, deterministic=True)
                                        v18_ppo_action = int(action_v18)
                                else:
                                    action_v18, _states_v18 = ppo_model.predict(v18_obs, deterministic=True)
                                    v18_ppo_action = int(action_v18)
                            except Exception as e:
                                print(f"   ⚠️  [V18方式] PPO预测失败: {e}")
                        
                        # V18方式：Transformer预测（使用V18数据，但可以复用归一化参数）
                        if transformer_model and ENABLE_TRANSFORMER and len(v18_closes) >= TRANSFORMER_MAX_SEQ_LEN:
                            try:
                                # 使用V18数据，但尽量复用V19的归一化参数（如果可用）
                                seq_v18 = v18_closes[-TRANSFORMER_MAX_SEQ_LEN:]
                                
                                # 如果V19已经训练过，尝试使用V19的归一化参数
                                if transformer_trained and transformer_normalization_params:
                                    norm_method_v18 = transformer_normalization_params.get('method', 'minmax')
                                    if norm_method_v18 == 'minmax':
                                        min_val_v18 = transformer_normalization_params['min']
                                        max_val_v18 = transformer_normalization_params['max']
                                        if max_val_v18 - min_val_v18 > 0:
                                            normalized_seq_v18 = (seq_v18 - min_val_v18) / (max_val_v18 - min_val_v18)
                                        else:
                                            normalized_seq_v18 = np.zeros_like(seq_v18)
                                    else:
                                        normalized_seq_v18 = seq_v18
                                    
                                    prediction_norm_v18 = transformer_model.predict_next(normalized_seq_v18)
                                    v18_transformer_prediction = float(transformer_model.denormalize(
                                        np.array([prediction_norm_v18]),
                                        transformer_normalization_params
                                    )[0]) if prediction_norm_v18 is not None else None
                                else:
                                    # 如果V19还没训练，使用V18数据训练
                                    if len(v18_closes) >= TRANSFORMER_MAX_SEQ_LEN * 2:
                                        if USE_SLIDING_WINDOW_NORMALIZE and len(v18_closes) > SLIDING_WINDOW_SIZE:
                                            recent_closes_v18 = v18_closes[-SLIDING_WINDOW_SIZE:]
                                        else:
                                            recent_closes_v18 = v18_closes
                                        
                                        normalized_closes_v18, norm_params_v18 = transformer_model.normalize(recent_closes_v18)
                                        
                                        X_list_v18, y_list_v18 = [], []
                                        for i in range(TRANSFORMER_MAX_SEQ_LEN, len(normalized_closes_v18)):
                                            X_list_v18.append(normalized_closes_v18[i-TRANSFORMER_MAX_SEQ_LEN:i])
                                            y_list_v18.append(normalized_closes_v18[i])
                                        
                                        if len(X_list_v18) > 0:
                                            transformer_model.train(
                                                np.array(X_list_v18), np.array(y_list_v18),
                                                epochs=TRANSFORMER_EPOCHS, batch_size=32,
                                                learning_rate=0.001, validation_split=0.2, verbose=False
                                            )
                                        
                                        normalized_seq_v18 = seq_v18
                                        norm_method_v18 = norm_params_v18.get('method', 'minmax')
                                        if norm_method_v18 == 'minmax':
                                            min_val_v18 = norm_params_v18['min']
                                            max_val_v18 = norm_params_v18['max']
                                            if max_val_v18 - min_val_v18 > 0:
                                                normalized_seq_v18 = (seq_v18 - min_val_v18) / (max_val_v18 - min_val_v18)
                                            else:
                                                normalized_seq_v18 = np.zeros_like(seq_v18)
                                ...
                                print(f"   ⚠️  [V18方式] Transformer预测失败: {e}")
                        
                        # V19新增：限制V18方式的第二天预测涨幅在合理范围内（A股涨跌停限制为±10%）
                        if v18_transformer_prediction is not None and current_price > 0:
                            v18_change_pct = ((v18_transformer_prediction - current_price) / current_price * 100)
                            
                            # 如果预测涨幅超过±10%，进行限制
                            if abs(v18_change_pct) > 10.0:
                                # 限制在±10%范围内
                                if v18_change_pct > 10.0:
                                    v18_transformer_prediction = current_price * 1.10  # 涨停价
                                elif v18_change_pct < -10.0:
                                    v18_transformer_prediction = current_price * 0.90  # 跌停价
                        
                        # 保存V18方式的预测结果
                        if v18_ppo_action is not None or v18_transformer_prediction is not None:
                            v18_prediction_results = {
                                'ppo_action': v18_ppo_action,
                                'transformer_prediction': v18_transformer_prediction,
                                'lstm_prediction': v18_lstm_prediction,
                                'data_source_info': v18_data_source_info,
                                'closes': v18_closes
                            }
                            print(f"   ✅ [V18方式] 预测完成")
                            if v18_ppo_action is not None:
                                print(f"      🎯 PPO动作: {map_action_to_operation(v18_ppo_action)} (动作={v18_ppo_action})")
                            if v18_transformer_prediction is not None:
                                v18_price_diff = v18_transformer_prediction - current_price
                                v18_price_diff_pct = (v18_price_diff / current_price * 100) if current_price > 0 else 0
                                print(f"      🔮 Transformer预测: {v18_transformer_prediction:.2f}元 (差异: {v18_price_diff:+.2f}元, {v18_price_diff_pct:+.2f}%)")
                    except Exception as e:
                        print(f"   ⚠️  [V18方式] 对比预测失败: {e}")
                        import traceback
                        traceback.print_exc()
                        v18_prediction_results = None
            ...
            try:
                ...
                # ========== V19新增：显示历史数据来源对比信息 ==========
                if 'historical_data_source_info' in locals() and historical_data_source_info is not None:
                    if not historical_data_source_info.get('is_own_data', True):
                        # 使用了其他模型的数据
                        model_name_used = historical_data_source_info.get('model_name', '未知模型')
                        model_code_used = historical_data_source_info.get('model_code', '未知')
                        source_type = historical_data_source_info.get('source_type', '其他模型数据')
                        data_dir_used = historical_data_source_info.get('data_dir', '未知目录')
                        
                        print(f"\n   📊 V19历史数据来源对比 [{stock_name}({STOCK_CODE})]:")
                        print(f"      ⚠️  使用了其他模型的历史数据:")
                        print(f"         📦 数据来源模型: {model_name_used} ({model_code_used})")
                        print(f"         📁 数据目录: {data_dir_used}")
                        print(f"         💡 说明: 当前股票使用{model_name_used}模型的历史数据进行预测")
                        ...
                        if stock_config:
                            ...
                            if 'ppo_stock_v7_' in model_path:
                                model_code = model_path.replace('ppo_stock_v7_', '').replace('.zip', '').replace('_', '').replace('steps', '')
                                if model_code and model_code != '1A0001':
                                    try:
                                        if model_code.startswith('6'):
                                            model_stock_code = f"sh.{model_code}"
                                        else:
                                            model_stock_code = f"sz.{model_code}"
                                        model_stock_name = get_stock_name(model_stock_code)
                                        print(f"         🎯 配置模型: {model_stock_name} ({model_code})")
                                        print(f"         📊 对比: 数据来源({model_name_used}) vs 配置模型({model_stock_name})")
                                        
                                        # 如果数据来源模型和配置模型不同，给出提示
                                        if model_code_used != model_code:
                                            print(f"         ⚠️  注意: 数据来源模型与配置模型不一致，可能影响预测准确性")
                                    except:
                                        print(f"         🎯 配置模型: {model_code}")
                    else:
                        # 使用了自己的数据
                        print(f"\n   📊 V19历史数据来源对比 [{stock_name}({STOCK_CODE})]:")
                        print(f"      ✅ 使用自身历史数据:")
                        print(f"         📁 数据目录: {historical_data_source_info.get('data_dir', '未知目录')}")
                        print(f"         💡 说明: 当前股票使用自身的历史数据进行预测，数据一致性最佳")
                
                # ========== V19新增：显示V18方式和V19方式的预测结果对比 ==========
                if v18_prediction_results is not None and historical_data_source_info is not None:
                    print(f"\n   📊 V19数据来源对比预测结果 [{stock_name}({STOCK_CODE})]:")
                    print(f"   {'='*70}")
                    
                    # V19方式的预测结果（使用优先级查找）
                    v19_method = "V19方式（优先级查找）"
                    v19_data_dir = historical_data_source_info.get('data_dir', '未知目录')
                    v19_source_type = historical_data_source_info.get('source_type', '未知来源')
                    
                    print(f"\n   🔵 {v19_method}:")
                    print(f"      📁 数据目录: {v19_data_dir}")
                    print(f"      📊 数据来源: {v19_source_type}")
                    
                    # V18方式的预测结果（只查找通用目录）
                    v18_method = "V18方式（通用目录）"
                    v18_data_dir = v18_data_source_info.get('data_dir', '未知目录') if v18_data_source_info else '未知目录'
                    v18_source_type = v18_data_source_info.get('source_type', '通用目录') if v18_data_source_info else '通用目录'
                    
                    print(f"\n   🟢 {v18_method}:")
                    print(f"      📁 数据目录: {v18_data_dir}")
                    print(f"      📊 数据来源: {v18_source_type}")
                    
                    # 对比显示预测结果
                    print(f"\n   📈 预测结果对比:")
                    
                    # PPO动作对比
                    if 'ppo_action' in locals() and ppo_action is not None:
                        v19_ppo_op = map_action_to_operation(ppo_action)
                        if v18_prediction_results.get('ppo_action') is not None:
                            v18_ppo_op = map_action_to_operation(v18_prediction_results['ppo_action'])
                            if ppo_action == v18_prediction_results['ppo_action']:
                                print(f"      ✅ PPO动作: {v19_ppo_op} (两种方式一致)")
                            else:
                                print(f"      ⚠️  PPO动作: V19方式={v19_ppo_op} (动作={ppo_action}) vs V18方式={v18_ppo_op} (动作={v18_prediction_results['ppo_action']}) (不一致)")
                        else:
                            print(f"      📊 PPO动作: V19方式={v19_ppo_op} (动作={ppo_action}), V18方式=未预测")
                    
                    # Transformer预测对比
                    if 'transformer_prediction' in locals() and transformer_prediction is not None:
                        v19_trans_price = transformer_prediction
                        v19_trans_diff = v19_trans_price - current_price
                        v19_trans_diff_pct = (v19_trans_diff / current_price * 100) if current_price > 0 else 0
                        
                        if v18_prediction_results.get('transformer_prediction') is not None:
                            v18_trans_price = v18_prediction_results['transformer_prediction']
                            v18_trans_diff = v18_trans_price - current_price
                            v18_trans_diff_pct = (v18_trans_diff / current_price * 100) if current_price > 0 else 0
                            
                            price_diff_between_methods = abs(v19_trans_price - v18_trans_price)
                            price_diff_pct_between_methods = (price_diff_between_methods / current_price * 100) if current_price > 0 else 0
                            
                            print(f"\n      🔮 Transformer预测对比:")
                            print(f"         🔵 V19方式: {v19_trans_price:.2f}元 (差异: {v19_trans_diff:+.2f}元, {v19_trans_diff_pct:+.2f}%)")
                            print(f"         🟢 V18方式: {v18_trans_price:.2f}元 (差异: {v18_trans_diff:+.2f}元, {v18_trans_diff_pct:+.2f}%)")
                            
                            if price_diff_pct_between_methods < 0.1:
                                print(f"         ✅ 两种方式预测结果一致（差异仅{price_diff_pct_between_methods:.2f}%）")
                            elif price_diff_pct_between_methods < 1.0:
                                print(f"         📊 两种方式预测结果接近（差异{price_diff_pct_between_methods:.2f}%）")
                            else:
                                print(f"         ⚠️  两种方式预测结果存在差异（差异{price_diff_pct_between_methods:.2f}%）")
                                
                                # 分析差异原因
                                if abs(v19_trans_diff_pct) > abs(v18_trans_diff_pct):
                                    print(f"            💡 V19方式预测变化更大（{v19_trans_diff_pct:+.2f}% vs {v18_trans_diff_pct:+.2f}%）")
                                elif abs(v18_trans_diff_pct) > abs(v19_trans_diff_pct):
                                    print(f"            💡 V18方式预测变化更大（{v18_trans_diff_pct:+.2f}% vs {v19_trans_diff_pct:+.2f}%）")
                        else:
                            print(f"\n      🔮 Transformer预测: V19方式={v19_trans_price:.2f}元 (差异: {v19_trans_diff:+.2f}元, {v19_trans_diff_pct:+.2f}%), V18方式=未预测")
                    
                    # 数据来源对比
                    if historical_data_source_info.get('is_own_data', True):
                        print(f"\n      ✅ V19方式使用自身数据（数据一致性最佳）")
                    else:
                        print(f"\n      ⚠️  V19方式使用其他模型数据: {historical_data_source_info.get('model_name', '未知模型')} ({historical_data_source_info.get('model_code', '未知')})")
                    
                    print(f"      📊 V18方式使用通用目录数据（V18标准处理方式）")
                    
                    # 总结建议
                    print(f"\n   💡 对比建议:")
                    if historical_data_source_info.get('is_own_data', True):
                        print(f"      ✅ V19方式（使用自身数据）数据一致性最佳，建议优先参考V19方式预测结果")
                    else:
                        print(f"      📊 V19方式使用了其他模型数据，可能与配置模型不完全匹配")
                        print(f"      📊 V18方式使用通用目录数据，是V18标准处理方式")
                        print(f"      💡 建议：对比两种方式的预测结果，综合考虑做出决策")
                ...
                # V19新增：收集预测结果用于汇总（V19方式和V18方式，包括涨幅和跌幅）
                current_price_val = prediction_data.get('current_price')
                if current_price_val is not None and current_price_val > 0:
                    # V19方式的预测结果
                    v19_next_day_prediction = prediction_data.get('v17_next_day_prediction')
                    v19_next_day_change_pct = None
                    if v19_next_day_prediction is not None:
                        v19_next_day_change_pct = ((v19_next_day_prediction - current_price_val) / current_price_val * 100)
                    
                    # V18方式的预测结果
                    v18_next_day_prediction = None
                    v18_next_day_change_pct = None
                    if 'v18_prediction_results' in locals() and v18_prediction_results is not None:
                        # V18方式使用Transformer预测作为第二天预测
                        v18_transformer_pred = v18_prediction_results.get('transformer_prediction')
                        if v18_transformer_pred is not None:
                            v18_next_day_prediction = v18_transformer_pred
                            v18_next_day_change_pct = ((v18_next_day_prediction - current_price_val) / current_price_val * 100)
                    
                    # 如果V19或V18任一有预测结果，添加到汇总列表
                    if v19_next_day_prediction is not None or v18_next_day_prediction is not None:
                        summary_item = {
                            'stock_code': STOCK_CODE,
                            'stock_name': stock_name,
                            'current_price': current_price_val,
                            'v19_next_day_prediction': v19_next_day_prediction,
                            'v19_next_day_change_pct': v19_next_day_change_pct,
                            'v18_next_day_prediction': v18_next_day_prediction,
                            'v18_next_day_change_pct': v18_next_day_change_pct,
                            'predicted_change_pct': prediction_data.get('predicted_change_pct'),
                            'predicted_direction': prediction_data.get('predicted_direction'),
                            'ppo_action': prediction_data.get('ppo_action'),
                            'ppo_operation': prediction_data.get('ppo_operation'),
                            'confidence': prediction_data.get('confidence'),
                        }
                        all_predictions_summary.append(summary_item)
            ...
            # ========== V19新增：显示其他模型历史数据对比 ==========
            try:
                # stock_info在for循环作用域内，可以直接使用
                display_alternative_models_comparison(STOCK_CODE, stock_name, stock_info)
            except Exception as e:
                print(f"   ⚠️  显示其他模型对比失败: {e}")
                # 只在调试时输出完整堆栈
                # import traceback
                # traceback.print_exc()
...
# ========== V19新增：汇总第二天预测涨幅和跌幅≥1%和≥2%的股票（V18和V19任一有结果即显示）==========
if all_predictions_summary:
    # 处理每个股票的预测结果，V18和V19任一有结果即显示，两者都有时分别显示
    processed_stocks = []
    for p in all_predictions_summary:
        v19_change = p.get('v19_next_day_change_pct')
        v18_change = p.get('v18_next_day_change_pct')
        v19_prediction = p.get('v19_next_day_prediction')
        v18_prediction = p.get('v18_next_day_prediction')
        
        # 如果V19有预测结果，添加V19的条目
        if v19_change is not None:
            processed_stocks.append({
                'stock_code': p['stock_code'],
                'stock_name': p['stock_name'],
                'current_price': p['current_price'],
                'prediction': v19_prediction,
                'change_pct': v19_change,
                'method': 'V19',
                'has_both': v18_change is not None,  # 标记是否两者都有
                'ppo_operation': p.get('ppo_operation', '未知') or '未知',
            })
        
        # 如果V18有预测结果，添加V18的条目
        if v18_change is not None:
            processed_stocks.append({
                'stock_code': p['stock_code'],
                'stock_name': p['stock_name'],
                'current_price': p['current_price'],
                'prediction': v18_prediction,
                'change_pct': v18_change,
                'method': 'V18',
                'has_both': v19_change is not None,  # 标记是否两者都有
                'ppo_operation': p.get('ppo_operation', '未知') or '未知',
            })
    
    # 筛选涨幅和跌幅
    stocks_up_2pct = [s for s in processed_stocks if s['change_pct'] >= 2.0]
    stocks_up_1pct = [s for s in processed_stocks if s['change_pct'] >= 1.0 and s['change_pct'] < 2.0]
    stocks_down_1pct = [s for s in processed_stocks if s['change_pct'] <= -1.0 and s['change_pct'] > -2.0]
    stocks_down_2pct = [s for s in processed_stocks if s['change_pct'] <= -2.0]
    
    # 按涨幅/跌幅绝对值从高到低排序
    stocks_up_2pct.sort(key=lambda x: x['change_pct'], reverse=True)
    stocks_up_1pct.sort(key=lambda x: x['change_pct'], reverse=True)
    stocks_down_1pct.sort(key=lambda x: x['change_pct'])  # 跌幅从小到大（-1.5% < -1.0%）
    stocks_down_2pct.sort(key=lambda x: x['change_pct'])  # 跌幅从小到大（-3.0% < -2.0%）
    
    # 生成汇总信息
    summary_text = "\n" + "=" * 70 + "\n"
    summary_text += "📊 V19批量预测汇总：第二天预测涨跌幅汇总（V18/V19任一有结果即显示）\n"
    summary_text += "=" * 70 + "\n\n"
    
    # 显示涨幅≥2%的股票
    if stocks_up_2pct:
        summary_text += f"🔥 第二天预测涨幅≥2%的股票（共{len(stocks_up_2pct)}条预测结果）:\n"
        summary_text += "-" * 100 + "\n"
        summary_text += f"{'股票名称':<30} | {'股票代码':<15} | {'当前价格':<12} | {'预测价格':<12} | {'预测涨幅':<12} | {'预测方式':<10} | {'PPO动作':<12}\n"
        summary_text += "-" * 100 + "\n"
        for s in stocks_up_2pct:
            stock_name_display = s['stock_name']
            stock_code_display = s['stock_code']
            # V19新增：将股票名称转换为超链接，跳转到详细信息位置
            anchor_id = stock_code_display.replace('.', '_').replace(':', '_')
            stock_name_link = f"[{stock_name_display}](#{anchor_id})"
            current_price_display = f"{s['current_price']:.2f}"
            pred_display = f"{s['prediction']:.2f}" if s['prediction'] else "N/A"
            change_pct_display = f"{s['change_pct']:+.2f}%"
            method_display = s['method']
            if s['has_both']:
                method_display += " (双)"
            ppo_op_display = s['ppo_operation']
            summary_text += f"{stock_name_link:<30} | {stock_code_display:<15} | {current_price_display:>12} | {pred_display:>12} | {change_pct_display:>12} | {method_display:<10} | {ppo_op_display:<12}\n"
        summary_text += "-" * 100 + "\n\n"
    else:
        summary_text += "⚠️  第二天预测涨幅≥2%的股票: 无\n\n"
    
    # 显示涨幅≥1%但<2%的股票
    if stocks_up_1pct:
        summary_text += f"📈 第二天预测涨幅≥1%但<2%的股票（共{len(stocks_up_1pct)}条预测结果）:\n"
        summary_text += "-" * 100 + "\n"
        summary_text += f"{'股票名称':<30} | {'股票代码':<15} | {'当前价格':<12} | {'预测价格':<12} | {'预测涨幅':<12} | {'预测方式':<10} | {'PPO动作':<12}\n"
        summary_text += "-" * 100 + "\n"
        for s in stocks_up_1pct:
            stock_name_display = s['stock_name']
            stock_code_display = s['stock_code']
            # V19新增：将股票名称转换为超链接，跳转到详细信息位置
            anchor_id = stock_code_display.replace('.', '_').replace(':', '_')
        ...
        summary_text += "⚠️  第二天预测涨幅≥1%但<2%的股票: 无\n\n"
    
    # 显示跌幅≤-1%但>-2%的股票
    if stocks_down_1pct:
        summary_text += f"📉 第二天预测跌幅≥1%但<2%的股票（共{len(stocks_down_1pct)}条预测结果）:\n"
        summary_text += "-" * 100 + "\n"
        summary_text += f"{'股票名称':<30} | {'股票代码':<15} | {'当前价格':<12} | {'预测价格':<12} | {'预测跌幅':<12} | {'预测方式':<10} | {'PPO动作':<12}\n"
        summary_text += "-" * 100 + "\n"
        for s in stocks_down_1pct:
            stock_name_display = s['stock_name']
            stock_code_display = s['stock_code']
            # V19新增：将股票名称转换为超链接，跳转到详细信息位置
            anchor_id = stock_code_display.replace('.', '_').replace(':', '_')
        ...
        summary_text += "⚠️  第二天预测跌幅≥1%但<2%的股票: 无\n\n"
    
    # 显示跌幅≤-2%的股票
    if stocks_down_2pct:
        summary_text += f"🔻 第二天预测跌幅≥2%的股票（共{len(stocks_down_2pct)}条预测结果）:\n"
        summary_text += "-" * 100 + "\n"
        summary_text += f"{'股票名称':<30} | {'股票代码':<15} | {'当前价格':<12} | {'预测价格':<12} | {'预测跌幅':<12} | {'预测方式':<10} | {'PPO动作':<12}\n"
        summary_text += "-" * 100 + "\n"
        for s in stocks_down_2pct:
            stock_name_display = s['stock_name']
            stock_code_display = s['stock_code']
            # V19新增：将股票名称转换为超链接，跳转到详细信息位置
            anchor_id = stock_code_display.replace('.', '_').replace(':', '_')
        ...
        summary_text += "⚠️  第二天预测跌幅≥2%的股票: 无\n\n"
    ...
    try:
        ...
        print(f"✅ 预测汇总已写入日志文件开头: {log_file}")
    except Exception as e:
        print(f"⚠️  写入汇总信息到日志文件失败: {e}")
else:
    print("\n⚠️  未收集到任何预测结果，无法生成汇总")
