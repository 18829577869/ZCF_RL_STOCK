# get_stock_data_v11_002518.py - V11科士达002518专用版
# -*- coding: utf-8 -*-
"""
V11 科士达002518专用版：
1. 专门针对科士达002518进行数据获取和训练
2. 包含科士达及其相关充电桩/电力设备/新能源等股票，确保更好的针对性与泛化能力
...
# V11 科士达002518专用股票列表
# 核心：科士达；配套：同板块/产业链 + 大盘风格股票
stocks = [
    # === 核心标的：科士达002518（充电桩/电力设备）===
...
print("=" * 70)
print("V11 科士达002518专用版 - 数据获取")
print("=" * 70)
print(f"总共 {len(stocks)} 只标的")
print(f"  核心标的: 科士达002518")
...
for stock in stocks:
    ...
    try:
        ...
        if len(train_data) < 100:
            ...
            fail_count += 1
            if code == "sz.002518":
                print(f"  [严重] 科士达002518训练数据不足！")
            continue

        # 保存到专用目录（V11可以使用V7的数据格式）
        os.makedirs('stockdata_v7_002518/train', exist_ok=True)
        os.makedirs('stockdata_v7_002518/test', exist_ok=True)

        train_file = f'stockdata_v7_002518/train/{code}.{name}.csv'
        test_file = f'stockdata_v7_002518/test/{code}.{name}.csv'
...
if success_count > 0:
    ...
    metadata_df.to_csv('stockdata_v7_002518/metadata_v7_002518.csv', index=False, encoding='utf-8-sig')
    print(f"\n元数据已保存: stockdata_v7_002518/metadata_v7_002518.csv")

    print("\n[完成] 可以开始训练：")
    print("  python train_v11_002518.py")
    print("\n[说明] 本版本专门针对科士达002518优化，初始资金5万元")
