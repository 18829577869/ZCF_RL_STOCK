"""
V16 实时预测系统 - DeepSeek 轮次复盘增强版（国机重装601399专用）
...
V16新增功能：
- 趋势/震荡双策略：趋势突破跟随 + 震荡布林均值回归，与模型信号加权融合

设计理念：多模型协同工作，智能融合决策，自动选择最优模型，风险控制优先，科学资金管理，丰富数据源
专用标的：国机重装601399（可控核聚变板块）
"""
...
# 基础配置
MODEL_PATH = "ppo_stock_v7_601399.zip"  # 使用国机重装601399专用模型
STOCK_CODE = 'sh.601399'  # 股票代码（国机重装）
...
CANDIDATE_MODELS = [  # 候选模型列表（按优先级排序）
    {
        'name': '601399模型（最佳）',
        'paths': [
            'ppo_stock_v7_601399.zip',
            'models_v7_601399/best/best_model.zip'
        ],
        'description': '国机重装601399模型（最佳匹配）'
    },
    {
        'name': '通用模型',
        'paths': [
...
print("\n" + "=" * 70)
print("V16 实时预测系统 - 趋势/震荡双策略 + DeepSeek复盘 + StockAPI（国机重装601399专用）")
print("=" * 70)
print("整合功能:")
...
print("=" * 70)
print("⚠️  版本标识: 这是 V15 版本，新增 DeepSeek 轮次复盘并保留 StockAPI 集成！")
print("=" * 70 + "\n")
...
def calculate_v7_price_suggestions(current_price, ppo_action, historical_prices=None):
    """
    为V7预测计算建议价格和仓位（基于PPO动作 + 价格带），并给出更人性化的详细说明
    """
    ...
    if historical_prices is not None and len(historical_prices) >= 20:
        try:
            recent_prices = np.array(historical_prices[-20:], dtype=float)
            returns = np.diff(recent_prices) / recent_prices[:-1]
            volatility_pct = float(np.std(returns) * 100 * np.sqrt(252))
            volatility_pct = max(1.0, min(10.0, volatility_pct))
        except Exception:
            volatility_pct = 2.0
    ...
    price_interval_size = current_price * price_interval_pct / 100.0

    # 3. 根据PPO动作对价格区间中心做偏移：看多→略下移，看空→略上移
    center_offset = 0.0
    ...
    min_price = price_center - price_interval_size / 2.0
    max_price = price_center + price_interval_size / 2.0

    # 保证价格区间不离谱
    min_price = max(current_price * 0.7, min_price)
    max_price = max(min_price + current_price * 0.01, max_price)

    # 4. 构造价格带：价格越低仓位越高
    position_prices = {
        "100%": round(min_price, 2),
        "75%": round(min_price + (max_price - min_price) * 0.25, 2),
        "50%": round(min_price + (max_price - min_price) * 0.50, 2),
        "25%": round(min_price + (max_price - min_price) * 0.75, 2),
        "0%":  round(max_price, 2),
    }

    # 5. 计算当前价格在价格带中对应的“合理仓位”
    price_levels = [
        position_prices["100%"],
        position_prices["75%"],
        position_prices["50%"],
    ...
    if current_price <= price_levels[0]:
        ...
        for i in range(len(price_levels) - 1):
            if price_levels[i] <= current_price <= price_levels[i + 1]:
                span = price_levels[i + 1] - price_levels[i]
                ratio = (current_price - price_levels[i]) / span if span > 0 else 0.0
                current_position_pct = 100.0 - (i * 25.0 + ratio * 25.0)
                break
    ...
    if ppo_action == 6:
        ...
        diff_pct = (
            (current_price - suggested_buy_price) / suggested_buy_price * 100.0
    ...
    # 买入 25%
        ...
        if diff_pct <= -1.0:
            ...
            detailed_position_description = (
                ...
                f"相对理想价{_fmt_pct(diff_pct)}，可尝试小仓位分批买入，整体仓位保持偏中性或略偏多。{base_desc_suffix}"
            )
        elif -1.0 < diff_pct < 1.0:
            position_description = "PPO轻微看多，当前价格接近理想加仓价，可试探性小仓介入"
            detailed_position_description = (
                f"当前价 {current_price:.2f} 元接近理想加仓价 {suggested_buy_price:.2f} 元，"
                f"可以小仓位试探性买入，但不宜一次性大幅加仓。{base_desc_suffix}"
            )
        else:
            position_description = "PPO轻微看多，但当前价格略高于理想加仓价，建议观望或等待回调"
            detailed_position_description = (
                f"理想加仓参考价约 {suggested_buy_price:.2f} 元，而当前价 {current_price:.2f} 元已经偏离，"
                f"相对理想价{_fmt_pct(diff_pct)}，可以以观望为主，等待更合适的价格再考虑加仓。{base_desc_suffix}"
            )

    # 持有
    elif ppo_action == 3:
        suggested_position_pct = current_position_pct
        position_description = "PPO认为以观望为主，保持当前仓位较为稳健"
        detailed_position_description = (
            f"当前价 {current_price:.2f} 元在价格带中对应的合理仓位约为 {current_position_pct:.0f}%，"
            f"模型并未给出明显的加仓或减仓信号，可根据自身风险偏好小幅微调仓位。{base_desc_suffix}"
        )
    ...
    # 卖出 25%
        ...
        diff_pct = (
            (current_price - suggested_sell_price) / suggested_sell_price * 100.0
            if suggested_sell_price > 0
            else 0.0
        )

        position_description = "PPO偏空，建议降低仓位至较低水平，逢高减仓为主"
        detailed_position_description = (
            f"目标减仓价参考约 {suggested_sell_price:.2f} 元，当前价 {current_price:.2f} 元，相对参考价{_fmt_pct(diff_pct)}，"
            f"可以考虑将仓位逐步降至 25% 左右，以控制整体回撤风险。{base_desc_suffix}"
        )
    ...
    # 卖出 50%
        ...
        position_description = "PPO偏空，当前价格处于相对偏高区域，建议明显减仓"
        detailed_position_description = (
            f"减仓参考价约 {suggested_sell_price:.2f} 元，当前价 {current_price:.2f} 元，相对参考价{_fmt_pct(diff_pct)}，"
            f"可以考虑大幅降低仓位，更多以防守为主，仅保留少量底仓。{base_desc_suffix}"
        )
    ...
    # 卖出 100%
        ...
        position_description = "PPO看空，当前区间以防守为主，建议清仓或保留极轻仓位"
        detailed_position_description = (
            f"清仓参考价约 {suggested_sell_price:.2f} 元，当前价 {current_price:.2f} 元，相对清仓参考价{_fmt_pct(diff_pct)}，"
            f"如已有较大浮盈可考虑分批止盈落袋，谨慎应对后续可能的回调风险。{base_desc_suffix}"
        )
...
def refine_holographic_signal(holo_result, closes, indicator_summary, fallback_used=False):
    """
    对全息信号进行保守后处理：
    - 如果无信号或情绪获取失败（fallback_used），输出 hold，低置信度
    - 如果短期动量、RSI、KDJ、布林信号偏空，则输出 sell/hold 并下调置信度
    """
    # 默认值
    ...
    # 空头信号判断：动量为负且 RSI<45 或 KDJ J<0，或价格高于上轨
    ...
    try:
        ...
        if ENABLE_AUTO_MODEL_SELECTION and len(candidate_ppo_models) > 0:
            ...
            if ppo_model and current_model_name:
                try:
                    ...
                    if ppo_action is not None:
                        ...
                        stock_name = get_stock_name(STOCK_CODE)
                        print(f"   🎯 V7 PPO动作 [{stock_name}({STOCK_CODE})]: (动作={ppo_action}, 模型={current_model_name})")
            ...
            try:
                ...
                ppo_operation = map_action_to_operation(ppo_action)
                stock_name = get_stock_name(STOCK_CODE)
                print(f"   🎯 V7 PPO动作 [{stock_name}({STOCK_CODE})]: (动作={ppo_action})")
