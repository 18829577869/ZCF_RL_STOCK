# get_stock_data_v11_002335.py - V11科华数据002335专用版
# -*- coding: utf-8 -*-
"""
V11 科华数据002335专用版：
1. 专门针对科华数据002335进行数据获取和训练
2. 包含科华数据及其相关充电桩/电力设备/数据中心/新能源等股票，确保更好的针对性与泛化能力
...
# V11 科华数据002335专用股票列表
# 核心：科华数据；配套：同板块/产业链 + 大盘风格股票
stocks = [
    # === 核心标的：科华数据002335（数据中心/UPS/电力设备）===
    {"code": "sz.002335", "name": "科华数据", "start_date": "2010-01-13",
     "category": "电力设备", "volatility": "中", "style": "成长", "priority": "核心"},

    # === 充电桩/电力设备板块（相关）===
    {"code": "sz.002364", "name": "中恒电气", "start_date": "2010-03-05",
     "category": "电力设备", "volatility": "中", "style": "成长", "priority": "相关"},
    {"code": "sz.002518", "name": "科士达", "start_date": "2010-12-07",
     "category": "电力设备", "volatility": "中", "style": "成长", "priority": "相关"},
    ...
    {"code": "sh.600875", "name": "东方电气", "start_date": "1995-10-10",
     "category": "电力设备", "volatility": "中", "style": "平衡", "priority": "相关"},
    ...
    {"code": "sz.300750", "name": "宁德时代", "start_date": "2018-06-11",
     "category": "新能源", "volatility": "高", "style": "成长", "priority": "配置"},
...
print("=" * 70)
print("V11 科华数据002335专用版 - 数据获取")
print("=" * 70)
print(f"总共 {len(stocks)} 只标的")
print(f"  核心标的: 科华数据002335")
...
for stock in stocks:
    ...
    try:
        ...
        if len(train_data) < 100:
            ...
            fail_count += 1
            if code == "sz.002335":
                print(f"  [严重] 科华数据002335训练数据不足！")
            continue

        # 保存到专用目录（V11可以使用V7的数据格式）
        os.makedirs('stockdata_v7_002335/train', exist_ok=True)
        os.makedirs('stockdata_v7_002335/test', exist_ok=True)

        train_file = f'stockdata_v7_002335/train/{code}.{name}.csv'
        test_file = f'stockdata_v7_002335/test/{code}.{name}.csv'
...
if success_count > 0:
    ...
    metadata_df.to_csv('stockdata_v7_002335/metadata_v7_002335.csv', index=False, encoding='utf-8-sig')
    print(f"\n元数据已保存: stockdata_v7_002335/metadata_v7_002335.csv")

    print("\n[完成] 可以开始训练：")
    print("  python train_v11_002335.py")
    print("\n[说明] 本版本专门针对科华数据002335优化，初始资金5万元")
