# get_stock_data_v11_601399.py - V11国机重装601399专用版
# -*- coding: utf-8 -*-
"""
V11 国机重装601399专用版：
1. 专门针对国机重装601399进行数据获取和训练
2. 包含国机重装601399及其相关股票，确保更好的针对性
3. 初始资金2万元（匹配实盘操作）
4. 数据用于V11全功能集成版训练
"""
...
# V11 国机重装601399专用股票列表
# 包含国机重装601399及其相关股票，确保更好的针对性
stocks = [
    # === 核心标的：国机重装601399 ===
    {"code": "sh.601399", "name": "国机重装", "start_date": "2020-06-08", 
     "category": "可控核聚变", "volatility": "中", "style": "平衡", "priority": "核心"},
    
    # === 可控核聚变/核电板块（相关股票）===
    {"code": "sh.601985", "name": "中国核电", "start_date": "2015-06-10", 
     "category": "核电", "volatility": "中", "style": "平衡", "priority": "相关"},
    {"code": "sh.601106", "name": "中国一重", "start_date": "2010-02-09", 
     "category": "重型装备", "volatility": "中", "style": "平衡", "priority": "相关"},
    {"code": "sz.002266", "name": "浙富控股", "start_date": "2008-08-06", 
...
print("="*70)
print("V11 国机重装601399专用版 - 数据获取")
print("="*70)
print(f"总共 {len(stocks)} 只标的")
print(f"  核心标的: 国机重装601399")
...
success_count = 0
fail_count = 0
guoji_zhongzhuang_success = False
...
for stock in stocks:
    ...
    try:
        ...
        if len(train_data) < 100:
            ...
            fail_count += 1
            if code == "sh.601399":
                print(f"  [严重] 国机重装601399训练数据不足！")
            continue
        
        # 保存到专用目录（V11可以使用V7的数据格式）
        os.makedirs('stockdata_v7_601399/train', exist_ok=True)
        os.makedirs('stockdata_v7_601399/test', exist_ok=True)
        
        train_file = f'stockdata_v7_601399/train/{code}.{name}.csv'
        test_file = f'stockdata_v7_601399/test/{code}.{name}.csv'
        ...
        if code == "sh.601399":
            guoji_zhongzhuang_success = True
            print(f"  [核心] ✅ 国机重装601399数据获取成功！")
...
if guoji_zhongzhuang_success:
    print(f"\n[核心] ✅ 国机重装601399数据获取成功！")
else:
    print(f"\n[严重] ❌ 国机重装601399数据获取失败！请检查！")
...
if success_count > 0:
    ...
    metadata_df.to_csv('stockdata_v7_601399/metadata_v7_601399.csv', index=False, encoding='utf-8-sig')
    print(f"\n元数据已保存: stockdata_v7_601399/metadata_v7_601399.csv")
    
    print("\n[完成] 可以开始训练：")
    print("  python train_v11_601399.py")
    print("\n[说明] 本版本专门针对国机重装601399优化，初始资金2万元")
